...
<%!
    ...
    sql_statement += ' AND ((airdate <= ' + today + ' AND ('
    if sickbeard.COUNT_SKIPPED_EPISODES:
        sql_statement += 'status = ' + str(SKIPPED) + ' OR '
    sql_statement += '  status = ' + str(WANTED) + ' OR status = ' + str(FAILED) + ')) '

...
<%
    ...
    if ComingEpisodes.is_episode_missed(cur_result):
        show_div = 'listing-overdue'
    elif ComingEpisodes.is_episode_later(cur_result):
        show_div = 'listing-toofar'
    elif ComingEpisodes.is_episode_today(cur_result):
...
<%
    ...
    cur_indexer = int(cur_result['indexer'])
    is_episode_later = ComingEpisodes.is_episode_later(cur_result)
    is_episode_missed = ComingEpisodes.is_episode_missed(cur_result)
    is_episode_today = ComingEpisodes.is_episode_today(cur_result)
%>
    % if sickbeard.COMING_EPS_SORT == 'network':
        ...
        % if is_episode_missed:
            <% show_div = 'ep_listing listing-overdue' %>
        % elif is_episode_later:

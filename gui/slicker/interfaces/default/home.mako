...
<div class="show-container">
	<div class="row clearfix">
		...
		<div class="col-md-8 column">
			<div class="right">
			<a class="btn btn-primary ${('', 'active')[layout == 'poster']}" href="${sbRoot}/setHomeLayout/?layout=poster"><i class="fa fa-th-large"></i></a>
			<a class="btn btn-primary ${('', 'active')[layout == 'small']}" href="${sbRoot}/setHomeLayout/?layout=small"><i class="fa fa-th"></i></a></a>
			<a class="btn btn-primary ${('', 'active')[layout == 'banner']}" href="${sbRoot}/setHomeLayout/?layout=banner"><i class="fa fa-th-list"></i></a>
			<a class="btn btn-primary ${('', 'active')[layout == 'simple']}" href="${sbRoot}/setHomeLayout/?layout=simple"><i class="fa fa-list"></i></a>
			<a class="btn btn-primary ${('', 'active')[layout == 'coverflow']}" href="${sbRoot}/setHomeLayout/?layout=coverflow"><i class="fa fa-random"></i></a>
			% if layout != 'poster':
                % if layout != 'coverflow':
        			<button id="popover" type="button" class="btn btn-primary">Select Column</button>
        			<div class="col-xs-4">
        				<div class="input-group">
        					<span class="input-group-addon"><i class="fa fa-search"></i></span>
    						<input type="text" class="search form-control" type="search" data-column="2" placeholder="Search Show Name">
    						<span class="input-group-btn">
      							<button type="button" class="resetshows resetanime btn btn-primary"><i class="fa fa-eraser"></i></button>
    						</span>
    					</div>
  					</div>
  				% endif
    		...
    		<div class="col-xs-4">
 	  			<div class="input-group">
					...
					<select id="postersort" class="form-control">
			...
			<div class="col-xs-2">
				<div class="input-group">
					...
					<select id="postersortdirection" class="form-control">
...
% if layout == 'poster':
	<div id="container${('', '-anime')[curListType == 'Anime' and layout == 'poster']}" class="clearfix">
...
%>
	<div class="show cap-bot" id="show$curShow.indexerid" data-name="${curShow.name}" data-date="${data_date}" data-network="${curShow.network}" data-progress="${progressbar_percent}">
		<div class="show-image ribbon-container">
			<a href="${sbRoot}/home/displayShow?show=${curShow.indexerid}"><img alt="" class="show-image" src="${sbRoot}/showPoster/?show=${curShow.indexerid}&amp;which=poster_thumb" />
			% if 'nded' in display_status:
			<span class="ribbon ribbon-red"><i class="fa fa-exclamation-triangle"></i> Ended</span>
			% else if 'nknown' in display_status:
			<span class="ribbon ribbon-orange"><i class="fa fa-question-circle"></i> Unknown</span>
			% endif
			</a>
		</div>
		<div class="showDetailsPopup">
		...
		% for curShow in myShowList:
			<div class="cover">
				<a href="${sbRoot}/home/displayShow?show=${curShow.indexerid}">
					<img alt="" class="reflect" src="${sbRoot}/showPoster/?show=${curShow.indexerid}&amp;which=poster_thumb" />
				</a>
			</div>
		% endfor
	</div>
...
% if sickbeard.TRAKT_USE_ROLLING_DOWNLOAD and sickbeard.USE_TRAKT
    <i class="fa fa-thumbs-${('down', 'up')[int(curShow.paused) == 0]}"></i>
% else
    <i class="fa fa-thumbs-${('down', 'up')[int(curShow.paused) == 0 and curShow.status == 'Continuing']}"></i>
% endif
		</td>
...
% if None is not display_status
    % if re.search(r'(?i)(?:new|returning)\s*series', curShow.status):
        <% display_status = '<i class="fa fa-play"></i>' %>
    % else if re.search(r'(?i)(?:nded)', curShow.status):
        <% display_status = '<i class="fa fa-stop"></i>' %>
    % else:
    	<% display_status = '<i class="fa fa-play"></i>' %>
    % endif
% endif
...
% endif
% endfor
<script type="text/javascript" src="${sbRoot}/js/coverflow.js?${sbPID}"></script>
<script type="text/javascript" src="${sbRoot}/js/reflection.js?${sbPID}"></script>
<script>
    $(function() {
        $('.coverflow').coverflow();
    ...
    $.backstretch([
        % for curShow in random.sample(myShowList, len(myShowList)):
            '${sbRoot}/showPoster/?show=${curShow.indexerid}&which=fanart',
        % endfor
    ], {
        duration: 10000,
        fade: 2000

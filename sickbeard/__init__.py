...
from sickbeard.providers import btn, newznab, womble, thepiratebay, torrentleech, kat, iptorrents, torrentz, \
    ...
    scenetime, btdigg, strike, transmitthenet, tvchaosuk, bitcannon, pretome, gftracker
    ...
    with INIT_LOCK:
        ...
        for curTorrentProvider in [curProvider for curProvider in providers.sortedProviderList() if
            ...
            if hasattr(curTorrentProvider, 'mincachetime') and hasattr(curTorrentProvider, 'mincachetimeprovider'):
                curTorrentProvider.mincachetime = check_setting_int(CFG, curTorrentProvider.getID().upper(),
                                                                curTorrentProvider.getID() + '_mincachetime', curTorrentProvider.mincachetimeprovider)
...
def save_config():
    ...
    for curTorrentProvider in [curProvider for curProvider in providers.sortedProviderList() if
        ...
        if hasattr(curTorrentProvider, 'mincachetime'):
            new_config[curTorrentProvider.getID().upper()][curTorrentProvider.getID() + '_mincachetime'] = int(
                curTorrentProvider.mincachetime)
        if hasattr(curTorrentProvider, 'options'):
            new_config[curTorrentProvider.getID().upper()][

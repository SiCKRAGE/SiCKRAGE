...
def initialize(consoleLogging=True):
    with INIT_LOCK:
        ...
        global BRANCH, GIT_RESET, GIT_REMOTE, GIT_REMOTE_URL, CUR_COMMIT_HASH, CUR_COMMIT_BRANCH, ACTUAL_LOG_DIR, LOG_DIR, LOG_NR, LOG_SIZE, WEB_PORT, WEB_LOG, ENCRYPTION_VERSION, ENCRYPTION_SECRET, WEB_ROOT, WEB_USERNAME, WEB_PASSWORD, WEB_HOST, WEB_IPV6, WEB_COOKIE_SECRET, WEB_USE_GZIP, API_KEY, API_ROOT, ENABLE_HTTPS, HTTPS_CERT, HTTPS_KEY, \
            ...
            NEWZNAB_DATA, NZBS, NZBS_UID, NZBS_HASH, INDEXER_DEFAULT, INDEXER_TIMEOUT, USENET_RETENTION, TORRENT_DIR, NO_SHOWUPDATE, \
        ...
        NO_SHOWUPDATE = bool(check_setting_int(CFG, 'General', 'no_showupdate', 0))
...
def start():
    ...
    with INIT_LOCK:
        if __INITIALIZED__:
            ...
            if NO_SHOWUPDATE:
                showUpdateScheduler.silent = True
                showUpdateScheduler.enable = False
            else:
                showUpdateScheduler.silent = False
...
def save_config():
    ...
    new_config['General']['no_showupdate'] = int(NO_SHOWUPDATE)

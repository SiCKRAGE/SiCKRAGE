...
class BlackAndWhiteList(object):
    ...
    def load(self):
        ...
        """
        logger.log(u'Building black and white list for ' +
                   str(self.show_id), logger.DEBUG)
    ...
    def _add_keywords(self, table, values):
        ...
        for value in values:
            myDB.action(
                'INSERT INTO [' + table + '] (show_id, keyword) VALUES (?,?)', [self.show_id, value])
    ...
    def _del_all_keywords(self, table):
        ...
        """
        myDB = db.DBConnection()
        myDB.action('DELETE FROM [' + table +
                    '] WHERE show_id = ?', [self.show_id])
    ...
    def _load_list(self, table):
        ...
        sqlResults = myDB.select(
            'SELECT keyword FROM [' + table + '] WHERE show_id = ?', [self.show_id])
        ...
        logger.log('BWL: ' + str(self.show_id) + ' loaded keywords from ' +
                   table + ': ' + str(groups), logger.DEBUG)
        return groups
    ...
    def is_valid(self, result):
        ...
        if not result.release_group:
            logger.log(
                'Failed to detect release group, invalid result',
                logger.DEBUG)
            return False
        if self.whitelist or self.blacklist:
            ...
            logger.log(
                'Whitelist check passed: %s. Blacklist check passed: %s' %
                (white_result, black_result), logger.DEBUG)
            ...
            logger.log(
                'No Whitelist and  Blacklist defined, check passed.',
                logger.DEBUG)
            return True

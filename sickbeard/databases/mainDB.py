...
class MainSanityCheck(db.DBSanityCheck):
    ...
    def convert_tvrage_to_tvdb(self):
        ...
        if sqlResults:
            logger.log(u"Found %i shows with TVRage ID's, attempting automatic conversion..." % len(sqlResults),
                       logger.WARNING)
        ...
        for tvrage_show in sqlResults:
            ...
            if len(mapping) != 1:
                logger.log(
                    u"Error mapping show from tvrage to tvdb for %s (%s), found %i mapping results. Cannot convert automatically!" %
                    (tvrage_show['show_name'], tvrage_show['location'], len(mapping)), logger.WARNING)
                logger.log(u"Removing the TVRage show and it's episodes from the DB, use 'addExistingShow'",
                           logger.WARNING)
            ...
            if duplicate:
                ...
                self.connection.action("DELETE FROM tv_episodes WHERE showid = %i" % tvrage_show['indexer_id'])
                logger.log(
                    u'Manually move the season folders from %s into %s, and delete %s before rescanning %s and unpausing it' %
                    (tvrage_show['location'], duplicate[0]['location'], tvrage_show['location'],
                     duplicate[0]['show_name']), logger.WARNING)
                continue
    ...
    def fix_episode_statuses(self):
        ...
        for cur_ep in sqlResults:
            logger.log(
                u"MALFORMED episode status detected! episode_id: " + str(cur_ep["episode_id"]) + " showid: " + str(
    ...
    def fix_invalid_airdates(self):
        ...
        for bad_airdate in sqlResults:
            logger.log(
                u"Bad episode airdate detected! episode_id: " + str(bad_airdate["episode_id"]) + " showid: " + str(

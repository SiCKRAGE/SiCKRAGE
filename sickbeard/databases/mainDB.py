...
class MainSanityCheck(db.DBSanityCheck):
    ...
    def fix_duplicate_shows(self, column='indexer_id'):
        ...
        sqlResults = self.connection.select(
            "SELECT show_id, " + column + ", COUNT(" + column + ") as count FROM tv_shows GROUP BY " + column +
            " HAVING count > 1")
    ...
    def fix_duplicate_episodes(self):
        ...
        sqlResults = self.connection.select(
            "SELECT showid, season, episode, COUNT(showid) as count FROM tv_episodes GROUP BY showid, season, "
            "episode HAVING count > 1")

        for cur_duplicate in sqlResults:

            logger.log(u"Duplicate episode detected! showid: {0} season: {1} episode: {2} count: {3}".format(
                str(cur_duplicate["showid"]), str(cur_duplicate["season"]), str(cur_duplicate["episode"]),
                str(cur_duplicate["count"])), logger.DEBUG)
    ...
    def fix_orphan_episodes(self):
        ...
        sqlResults = self.connection.select(
            "SELECT episode_id, showid, tv_shows.indexer_id FROM tv_episodes LEFT JOIN tv_shows ON "
            "tv_episodes.showid=tv_shows.indexer_id WHERE tv_shows.indexer_id is NULL")
    ...
    def execute(self):
        if not self.hasTable("tv_shows") and not self.hasTable("db_version"):
            queries = [
                ...
                "scene_season INTEGER, scene_episode INTEGER, PRIMARY KEY(indexer_id, season, episode))",
                
                "CREATE TABLE tv_shows (show_id INTEGER PRIMARY KEY, indexer_id NUMERIC, indexer TEXT, show_name TEXT,"
                " location TEXT, network TEXT, genre TEXT, classification TEXT, runtime NUMERIC, quality NUMERIC,"
                " airs TEXT, status TEXT, flatten_folders NUMERIC, paused NUMERIC, startyear NUMERIC,"
                ...
                " scene_season NUMERIC, scene_episode NUMERIC);",
            ...
            if cur_db_version < MIN_DB_VERSION:
                logger.log_error_and_exit(
                    u"Your database version ({0}) is too old to migrate from what this version of SickRage supports "
                    u"({1}).\nUpgrade using a previous version (tag) build 496 to build 501 of SickRage first or "
                    u"remove database file to begin fresh.".format(str(cur_db_version), str(MIN_DB_VERSION))
                )

            if cur_db_version > MAX_DB_VERSION:
                logger.log_error_and_exit(
                    u"Your database version ({0}) has been incremented past what this version of SickRage supports "
                    u"({1}).\nIf you have used other forks of SickRage, your database may be unusable due to their "
                    u"modifications.".format(str(cur_db_version), str(MAX_DB_VERSION))
                )
    ...
    def execute(self):
        ...
        for cur_result in history_results:
            ...
            download_results = self.connection.select(
                "SELECT resource FROM history WHERE provider = -1 AND showid = ? AND season = ? AND episode = ? "
                "AND date > ?",
            ...
            ep_results = self.connection.select(
                "SELECT episode_id, status FROM tv_episodes WHERE showid = ? AND season = ? AND episode = ? "
                "AND location != ''",
...
class RenameSeasonFolders(AddSizeAndSceneNameFields):
    ...
    def execute(self):
        ...
        sql = "INSERT INTO tv_shows(show_id, location, show_name, tvdb_id, network, genre, runtime, quality, airs, " \
              "status, flatten_folders, paused, startyear, tvr_id, tvr_name, air_by_date, lang) SELECT show_id, " \
              "location, show_name, tvdb_id, network, genre, runtime, quality, airs, status, seasonfolders, paused, " \
              "startyear, tvr_id, tvr_name, air_by_date, lang FROM tmp_tv_shows"
...
class Add1080pAndRawHDQualities(RenameSeasonFolders):
    ...
    def execute(self):
        ...
        new_hd = common.Quality.combineQualities(
            [common.Quality.HDTV, common.Quality.HDWEBDL, common.Quality.HDBLURAY],
            []
        )
        ...
        # update status that are are within the old hdwebdl (1<<3 which is 8) and better
        # -- exclude unknown (1<<15 which is 32768)
        ...
        # make two seperate passes through the history since snatched and downloaded
        # (action & quality) may not always coordinate together
...
class AddIMDbInfo(AddDBIncreaseTo15):
    ...
    def execute(self):
        self.connection.action(
            "CREATE TABLE imdb_info (tvdb_id INTEGER PRIMARY KEY, imdb_id TEXT, title TEXT, year NUMERIC, akas TEXT, "
...
class ConvertTVShowsToIndexerScheme(AddSubtitlesSupport):
    ...
    def execute(self):
        ...
        self.connection.action(
            ...
            "last_update_indexer, dvdorder) SELECT show_id, tvdb_id, show_name, location, network, genre, runtime, "
...
class ConvertInfoToIndexerScheme(ConvertIMDBInfoToIndexerScheme):
    ...
    def execute(self):
        ...
        self.connection.action(
            "CREATE TABLE info (last_backlog NUMERIC, last_indexer NUMERIC, last_proper_search NUMERIC)")
        self.connection.action(
            "INSERT INTO info(last_backlog, last_indexer, last_proper_search) SELECT last_backlog, last_tvdb, "
            "last_proper_search FROM tmp_info")
        self.connection.action("DROP TABLE tmp_info")
...
class AddSceneNumbering(AddArchiveFirstMatchOption):
    ...
    def execute(self):
        ...
        self.connection.action(
            "CREATE TABLE scene_numbering (indexer TEXT, indexer_id INTEGER, season INTEGER, episode INTEGER, "
            "scene_season INTEGER, scene_episode INTEGER, PRIMARY KEY (indexer_id, season, episode, scene_season, "
            "scene_episode))")
...
class AddIndexerMapping(AddSceneToTvShows):
    ...
    def execute(self):
        ...
        self.connection.action(
            "CREATE TABLE indexer_mapping (indexer_id INTEGER, indexer NUMERIC, mindexer_id INTEGER, "

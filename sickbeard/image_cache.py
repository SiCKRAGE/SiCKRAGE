...
class ImageCache:
    ...
    def _cache_image_from_file(self, image_path, img_type, indexer_id):
        ...
        if not ek(os.path.isdir, self._cache_dir()):
            logger.log(
                u"Image cache dir didn't exist, creating it at " + str(self._cache_dir()))
            ek(os.makedirs, self._cache_dir())

        if not ek(os.path.isdir, self._thumbnails_dir()):
            logger.log(
                u"Thumbnails cache dir didn't exist, creating it at " + str(self._thumbnails_dir()))
    ...
    def fill_cache(self, show_obj):
        ...
        logger.log(u"Checking if we need any cache images for show " +
        ...
        if not need_images[self.POSTER] and not need_images[self.BANNER] and not need_images[self.POSTER_THUMB] and not \
            ...
            logger.log(
                u"No new cache images needed, not retrieving new ones",
                logger.DEBUG)
            return

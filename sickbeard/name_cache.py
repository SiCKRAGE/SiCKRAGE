...
def addNameToCache(name, indexer_id=0):
    ...
    # standardize the name we're using to account for small differences in
    # providers
...
def clearCache(indexerid=0):
    ...
    toRemove = [key for key, value in nameCache.iteritems() if value ==
                0 or value == indexerid]
    for key in toRemove:
        del nameCache[key]
...
def buildNameCache(show=None):
    ...
    if not show:
        ...
        logger.log(
            u"Building internal name cache for " +
            show.name,
            logger.DEBUG)
        clearCache(show.indexerid)
        for curSeason in [-1] + \
                sickbeard.scene_exceptions.get_scene_seasons(show.indexerid):
            for name in list(set(sickbeard.scene_exceptions.get_scene_exceptions(
        ...
        logger.log(
            u"Internal name cache for " +
            show.name +
            " set to: [ " +
            u', '.join(
                [
                    key for key,
                    value in nameCache.iteritems() if value == show.indexerid]) +
            " ]",
            logger.DEBUG)

...
class NMJNotifier:
    ...
    def notify_settings(self, host):
        ...
        try:
            ...
            logger.log(
                u"Warning: unable to get a telnet session to %s" %
                (host), logger.WARNING)
            return False
        ...
        if match:
            ...
            logger.log(
                u"Found NMJ database %s on device %s" %
                (database, device), logger.DEBUG)
            sickbeard.NMJ_DATABASE = database
        else:
            logger.log(
                u"Could not get current NMJ database on %s, NMJ is probably not running!" %
                (host), logger.WARNING)
            return False
        ...
        if device.startswith("NETWORK_SHARE/"):
            ...
            if match:
                ...
                logger.log(
                    u"Found mounting url on the Popcorn Hour in configuration: %s" %
                    (mount), logger.DEBUG)
    ...
    def _sendNMJ(self, host, database, mount=None):
        ...
        if mount:
            try:
                ...
                logger.log(
                    u"Try to mount network drive via url: %s" %
                    (mount), logger.DEBUG)
        ...
        try:
            ...
            logger.log(
                u"Sending NMJ scan update command via url: %s" %
                (updateUrl), logger.DEBUG)
            handle = urllib2.urlopen(req)
        ...
        if int(result) > 0:
            logger.log(
                u"Popcorn Hour returned an errorcode: %s" %
                (result), logger.ERROR)
            return False
        else:

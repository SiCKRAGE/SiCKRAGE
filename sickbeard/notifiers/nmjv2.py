...
class NMJv2Notifier:
        ...
        try:
            ...
            for node in xml.getElementsByTagName('path'):
                ...
                xmlData = xmlTag.replace(
                    ...
                    '')
                url_db = "http://" + host + \
                    ":8008/metadata_database?arg0=check_database&arg1=" + xmlData
    ...
    def _sendNMJ(self, host):
        ...
        try:
            ...
            logger.log(
                u"NMJ scan update command sent to host: %s" %
                (host), logger.DEBUG)
        ...
        try:
            ...
            logger.log(
                u"Unable to parse XML returned from the Popcorn Hour: update_scandir, %s" %
                (e), logger.ERROR)
            return False
        try:
            ...
            logger.log(
                u"Unable to parse XML returned from the Popcorn Hour: scanner_start, %s" %
                (e), logger.ERROR)
            return False
        ...
        if int(result1) > 0:
            ...
            logger.log(
                u"Popcorn Hour returned an error: %s" %
                (error_messages[index]), logger.ERROR)
            return False
        else:
            if int(result2) > 0:

...
class PLEXNotifier:
    ...
    def _send_to_plex(self, command, host, username=None, password=None):
        ...
        if not host:
            logger.log(
                u'PLEX: No host specified, check your settings',
                logger.WARNING)
            return False
        ...
        try:
            ...
            if password:
                ...
                logger.log(
                    u'PLEX: Contacting (with auth header) via url: ' + url,
                    logger.DEBUG)
            else:
            ...
            logger.log(
                u'PLEX: HTTP response: ' +
                result.replace(
                    '\n',
                    ''),
                logger.DEBUG)
            # could return result response =
            # re.compile('<html><li>(.+\w)</html>').findall(result)
            return 'OK'

        except (urllib2.URLError, IOError) as e:
            logger.log(
                u'PLEX: Warning: Couldn\'t contact Plex at ' +
                url +
                ' ' +
                ex(e),
                logger.WARNING)
    ...
    def _notify_pmc(self, message, title='SickRage', host=None,
        ...
        for curHost in [x.strip() for x in host.split(',')]:
            logger.log(
                u'PLEX: Sending notification to \'%s\' - %s' %
                (curHost, message), logger.DEBUG)

            command = {'command': 'ExecBuiltIn', 'parameter': 'Notification(%s,%s)' % (
                title.encode('utf-8'), message.encode('utf-8'))}
            notify_result = self._send_to_plex(
...
##############################################################################
    ...
    def test_notify_pmc(self, host, username, password):
        return self._notify_pmc('This is a test notification from SickRage',
    ...
    def test_notify_pms(self, host, username, password, plex_server_token):
        return self.update_library(host=host, username=username, password=password,
                                   plex_server_token=plex_server_token, force=False)

    def update_library(self, ep_obj=None, host=None, username=None,
                       password=None, plex_server_token=None, force=True):
        ...
        if sickbeard.USE_PLEX and sickbeard.PLEX_UPDATE_LIBRARY:
            ...
            # if username and password were provided, fetch the auth token from
            # plex.tv
            token_arg = ''
            if plex_server_token:
                ...
                logger.log(
                    u'PLEX: fetching plex.tv credentials for user: ' +
                    username,
                    logger.DEBUG)
                req = urllib2.Request(
                    'https://plex.tv/users/sign_in.xml', data='')
                ...
                try:
                    ...
                    logger.log(
                        u'PLEX: Error parsing plex.tv response: ' +
                        ex(e),
                        logger.DEBUG)
            ...
            for cur_host in host_list:
                ...
                if not sections:
                    logger.log(
                        u'PLEX: Plex Media Server not running on: ' +
                        cur_host,
                        logger.DEBUG)
                    hosts_failed.append(cur_host)
                    continue
                ...
                for section in sections:
                    if 'show' == section.attrib['type']:
                        ...
                        for section_location in section.findall('.//Location'):
                            section_path = re.sub(
                                r'[/\\]+', '/', section_location.attrib['path'].lower())
                            section_path = re.sub(
                                r'^(.{,2})[/\\]', '', section_path)
                            location_path = re.sub(
                                r'[/\\]+', '/', file_location.lower())
                            location_path = re.sub(
                                r'^(.{,2})[/\\]', '', location_path)
            ...
            for section_key, cur_host in hosts_try.iteritems():
                ...
                url = 'http://%s/library/sections/%s/refresh%s' % (
                    cur_host, section_key, token_arg)
                try:
                    force and urllib.urlopen(url)
                    host_list.append(cur_host)
                except Exception as e:
                    logger.log(
                        u'PLEX: Error updating library section for Plex Media Server: ' +
                        ex(e),
                        logger.WARNING)
                    hosts_failed.append(cur_host)
            ...
            if hosts_match:
                logger.log(
                    u'PLEX: Updating hosts where TV section paths match the downloaded show: ' +
                    ', '.join(
                        set(host_list)),
                    logger.DEBUG)
            else:
                logger.log(
                    u'PLEX: Updating all hosts with TV sections: ' +
                    ', '.join(
                        set(host_list)),
                    logger.DEBUG)

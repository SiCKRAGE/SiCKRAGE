...
class PLEXNotifier:
    ...
    def _send_to_plex(self, command, host, username=None, password=None):
        ...
        try:
            ...
            logging.warning(
                'PLEX: Warning: Couldn\'t contact Plex at ' +
                url +
                ' ' +
                ex(e))
            return False
    ...
    def _notify_pmc(self, message, title='SiCKRAGE', host=None,
        ...
        for curHost in [x.strip() for x in host.split(',')]:
            logging.debug(
                'PLEX: Sending notification to \'%s\' - %s' %
                (curHost, message))
    ...
    def update_library(self, ep_obj=None, host=None, username=None,
        ...
        if sickbeard.USE_PLEX and sickbeard.PLEX_UPDATE_LIBRARY:
            ...
            if plex_server_token:
                ...
                logging.debug(
                    'PLEX: fetching plex.tv credentials for user: ' + username)
                req = urllib2.Request(
                ...
                try:
                    ...
                    logging.debug(
                        'PLEX: Error parsing plex.tv response: ' + ex(e))
            ...
            for cur_host in host_list:
                ...
                try:
                    ...
                    logging.warning(
                        'PLEX: Error while trying to contact Plex Media Server: ' + ex(e))
                    hosts_failed.append(cur_host)
                    continue
                except Exception as e:
                    if 'invalid token' in str(e):
                        logging.error(
                            'PLEX: Please set TOKEN in Plex settings: ')
                    else:
                        logging.error(
                            'PLEX: Error while trying to contact Plex Media Server: ' + ex(e))
                    continue
                ...
                if not sections:
                    logging.debug(
                        'PLEX: Plex Media Server not running on: ' + cur_host)
                    hosts_failed.append(cur_host)
                    continue
                ...
                for section in sections:
                    if 'show' == section.attrib[b'type']:
                        ...
                        for section_location in section.findall('.//Location'):
                            section_path = re.sub(
                                r'[/\\]+', '/', section_location.attrib[b'path'].lower())
                            section_path = re.sub(
                                r'^(.{,2})[/\\]', '', section_path)
            ...
            for section_key, cur_host in hosts_try.iteritems():
                ...
                try:
                    ...
                    logging.warning(
                        'PLEX: Error updating library section for Plex Media Server: ' + ex(e))
                    hosts_failed.append(cur_host)

...
class TwitterNotifier:
        ...
        if sickbeard.TWITTER_NOTIFY_ONSNATCH:
            self._notifyTwitter(
                common.notifyStrings[
                    common.NOTIFY_SNATCH] + ': ' + ep_name)

    def notify_download(self, ep_name):
        if sickbeard.TWITTER_NOTIFY_ONDOWNLOAD:
            self._notifyTwitter(
                common.notifyStrings[
                    common.NOTIFY_DOWNLOAD] + ': ' + ep_name)
    ...
    def _get_authorization(self):
        ...
        if resp[b'status'] != '200':
            ...
            return self.AUTHORIZATION_URL + "?oauth_token=" + \
    ...
    def _get_credentials(self, key):
        ...
        resp, content = oauth_client.request(
            self.ACCESS_TOKEN_URL, method='POST', body='oauth_verifier=%s' %
            key)
        ...
        if resp[b'status'] != '200':
            logging.error(
                'The request for a token with did not succeed: ' + str(resp[b'status']))
            return False
        else:
            logging.debug(
                'Your Twitter Access Token key: %s' %
                access_token[b'oauth_token'])
            logging.debug(
                'Access Token secret: %s' %
                access_token[b'oauth_token_secret'])

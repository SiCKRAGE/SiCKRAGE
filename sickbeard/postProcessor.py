...
class PostProcessor(object):
    ...
    def __init__(self, file_path, nzb_name=None,
                 process_method=None, is_priority=None):
        """
    ...
    def _checkForExistingFile(self, existing_file):
        ...
        if not existing_file:
            self._log(
                u"There is no existing file so there's no worries about replacing it",
                logger.DEBUG)
            return PostProcessor.DOESNT_EXIST

        # if the new file exists, return the appropriate code depending on the
        # size
        if ek(os.path.isfile, existing_file):

            # see if it's bigger than our old file
            if ek(os.path.getsize, existing_file) > ek(
    ...
    def list_associated_files(
            self, file_path, base_name_only=False, subtitles_only=False, subfolders=False):
        """
        ...
        if subfolders:
            base_name = ek(os.path.basename,
                           globbable_file_path).rpartition('.')[0]
        else:
        ...
        if subfolders:  # subfolders are only checked in show folder, so names will always be exactly alike
            ...
            filelist = recursive_glob(
                ek(os.path.dirname, globbable_file_path), base_name + '*')
        else:  # this is called when PP, so we need to do the filename check case-insensitive
            filelist = []

            # get a list of all the files in the folder
            checklist = glob.glob(
                ek(os.path.join, ek(os.path.dirname, globbable_file_path), '*'))
        ...
        for associated_file_path in filelist:
            ...
            if associated_file_path == file_path:
                continue
            # only list it if the only non-shared part is the extension or if
            # it is a subtitle
            if subtitles_only and not associated_file_path[
                    len(associated_file_path) - 3:] in common.subtitleExtensions:
                continue
        ...
        if file_path_list:
            self._log(
                u"Found the following associated files: " +
                str(file_path_list),
                logger.DEBUG)
        else:
            self._log(
                u"No associated files were during this pass",
                logger.DEBUG)
    ...
    def _delete(self, file_path, associated_files=False):
        ...
        if associated_files:
            file_list = file_list + \
                self.list_associated_files(
                    file_path, base_name_only=True, subfolders=True)

        if not file_list:
            self._log(
                u"There were no files associated with " +
                file_path +
        ...
        for cur_file in file_list:
            if ek(os.path.isfile, cur_file):
                ...
                if not file_attribute & stat.S_IWRITE:
                    ...
                    self._log(
                        'Read only mode on file ' +
                        cur_file +
                        ' Will try to make it writeable',
                        logger.DEBUG)
                    try:
                        ek(os.chmod, cur_file, stat.S_IWRITE)
                    except Exception:
                        self._log(
                            u'Cannot change permissions of ' +
                            cur_file,
                            logger.WARNING)
    ...
    def _combined_file_operation(self, file_path, new_path, new_base_name, associated_files=False, action=None,
        ...
        if associated_files:
            ...
            file_list = file_list + \
                self.list_associated_files(file_path, subtitles_only=True)
        ...
        for cur_file_path in file_list:
            ...
            if os.path.splitext(cur_extension)[1][
                    1:] in common.subtitleExtensions:
                cur_lang = os.path.splitext(cur_extension)[0]
                if cur_lang in sickbeard.subtitles.wantedLanguages():
                    cur_extension = cur_lang + \
                        os.path.splitext(cur_extension)[1]
            ...
            # if we're not renaming we still want to change extensions
            # sometimes
            else:
            ...
            if sickbeard.SUBTITLES_DIR and cur_extension in common.subtitleExtensions:
                subs_new_path = ek(
                    os.path.join, new_path, sickbeard.SUBTITLES_DIR)
    ...
    def _move(self, file_path, new_path, new_base_name,
              associated_files=False, subtitles=False):
        """
    ...
    def _copy(self, file_path, new_path, new_base_name,
              associated_files=False, subtitles=False):
        """
    ...
    def _hardlink(self, file_path, new_path, new_base_name,
                  associated_files=False, subtitles=False):
        """
        ...
        self._combined_file_operation(
            file_path,
            new_path,
            new_base_name,
            associated_files,
            action=_int_hard_link,
            subtitles=subtitles)

    def _moveAndSymlink(self, file_path, new_path, new_base_name,
                        associated_files=False, subtitles=False):
        """
        ...
        def _int_move_and_sym_link(cur_file_path, new_file_path):
            ...
            self._log(
                u"Moving then symbolic linking file from " +
                cur_file_path +
                " to " +
                new_file_path,
                logger.DEBUG)
    ...
    def _history_lookup(self):
        ...
        # if we don't have either of these then there's nothing to use to
        # search the history for anyway
        ...
        # search the database for a possible match and return immediately if we
        # find one
        ...
        if parse_result.series_name and ((parse_result.season_number is not None and parse_result.episode_numbers)
            ...
            if not self.release_name:
                self.release_name = helpers.remove_non_release_groups(
                    helpers.remove_extension(ek(os.path.basename, parse_result.original_name)))

        else:
            logger.log(u"Parse result not sufficient (all following have to be set). will not save release name",
                       logger.DEBUG)
            logger.log(u"Parse result(series_name): " +
                       str(parse_result.series_name), logger.DEBUG)
            logger.log(u"Parse result(season_number): " +
                       str(parse_result.season_number), logger.DEBUG)
            logger.log(u"Parse result(episode_numbers): " +
                       str(parse_result.episode_numbers), logger.DEBUG)
            logger.log(u" or Parse result(air_date): " +
                       str(parse_result.air_date), logger.DEBUG)
            logger.log(u"Parse result(release_group): " +
    ...
    def _add_to_anidb_mylist(self, filePath):
        ...
        if helpers.set_up_anidb_connection():
            if not self.anidbEpisode:  # seems like we could parse the name before, now lets build the anidb object
                self.anidbEpisode = self._build_anidb_episode(
                    sickbeard.ADBA_CONNECTION, filePath)

            self._log(u"Adding the file to the anidb mylist", logger.DEBUG)
            try:
                # status = 1 sets the status of the file to "internal HDD"
    ...
    def _find_info(self):
        ...
        attempt_list = [
            ...
            lambda: self._analyze_name(
                self.folder_name + u' ' + self.file_name)
        ]
        ...
        for cur_attempt in attempt_list:
            ...
            try:
                (cur_show, cur_season, cur_episodes,
                 cur_quality, cur_version) = cur_attempt()
            except (InvalidNameException, InvalidShowException) as e:
                logger.log(
                    u"Unable to parse, skipping: " +
                    ex(e),
                    logger.DEBUG)
                continue
            ...
            # we only get current version for animes from history to prevent
            # issues with old database entries
            ...
            # for air-by-date shows we need to look up the season/episode from
            # database
            if season == -1 and show and episodes:
                ...
                myDB = db.DBConnection()
                # Ignore season 0 when searching for episode(Conflict between
                # special and regular episode, same air date)
                ...
                if sql_result:
                    ...
                    if sql_result:
                        ...
                        # we don't want to leave dates in the episode list if
                        # we couldn't convert them to real episode numbers
                        episodes = []
                        continue

            # if there's no season then we can hopefully just use 1
            # automatically
    ...
    def _get_ep_obj(self, show, season, episodes):
        ...
        for cur_episode in episodes:
            self._log(
                u"Retrieving episode object for " +
                str(season) +
                "x" +
                str(cur_episode),
                logger.DEBUG)

            # now that we've figured out which episode this file is just load
            # it manually
            try:
    ...
    def _get_quality(self, ep_obj):
        ...
        # if there is a quality available in the status then we don't need to
        # bother guessing from the filename
        if ep_obj.status in common.Quality.SNATCHED + \
                common.Quality.SNATCHED_PROPER + common.Quality.SNATCHED_BEST:
            _, ep_quality = common.Quality.splitCompositeStatus(
                ep_obj.status)  # @UnusedVariable
            if ep_quality != common.Quality.UNKNOWN:
                self._log(
                    u"The old status had a quality in it, using that: " +
                    common.Quality.qualityStrings[ep_quality],
                    logger.DEBUG)
                return ep_quality

        # nzb name is the most reliable if it exists, followed by folder name
        # and lastly file name
        name_list = [self.nzb_name, self.folder_name, self.file_name]

        # search all possible names for our new quality, in case the file or
        # dir doesn't have it
        for cur_name in name_list:
            ...
            ep_quality = common.Quality.nameQuality(
                cur_name, ep_obj.show.is_anime)
            self._log(
                u"Looking up quality for name " + cur_name +
                u", got " + common.Quality.qualityStrings[ep_quality],
                logger.DEBUG)
        ...
        self._log(
            u"Guessing quality for name " + self.file_name +
            u", got " + common.Quality.qualityStrings[ep_quality],
            logger.DEBUG)
        if ep_quality != common.Quality.UNKNOWN:
    ...
    def _run_extra_scripts(self, ep_obj):
        ...
        for curScriptName in sickbeard.EXTRA_SCRIPTS:
            ...
            # generate a safe command line string to execute the script and
            # provide all the parameters
            script_cmd = [
                piece for piece in re.split(
                    "( |\\\".*?\\\"|'.*?')",
                    curScriptName) if piece.strip()]
            script_cmd[0] = ek(os.path.abspath, script_cmd[0])
            self._log(
                u"Absolute path to script: " +
                script_cmd[0],
                logger.DEBUG)
    ...
    def _is_priority(self, ep_obj, new_ep_quality):
        ...
        if self.in_history or ep_obj.status in common.Quality.SNATCHED + \
                common.Quality.SNATCHED_PROPER + common.Quality.SNATCHED_BEST:
            # if the episode is still in a snatched status, then we can assume
            # we want this
            if not self.in_history:
                self._log(
                    u"SR snatched this episode and it is not processed before",
                    logger.DEBUG)
                return True

            # if it's in history, we only want it if the new quality is higher
            # or if it's a proper of equal or higher quality
            if new_ep_quality > old_ep_quality and new_ep_quality != common.Quality.UNKNOWN:
                self._log(
                    u"SR snatched this episode and it is a higher quality so I'm marking it as priority",
                    logger.DEBUG)
                return True

            if self.is_proper and new_ep_quality >= old_ep_quality and new_ep_quality != common.Quality.UNKNOWN:
                self._log(
                    u"SR snatched this episode and it is a proper of equal or higher quality so I'm marking it as priority",
                    logger.DEBUG)
                return True

            return False

        # if the user downloaded it manually and it's higher quality than the
        # existing episode then it's priority
        if new_ep_quality > old_ep_quality and new_ep_quality != common.Quality.UNKNOWN:
            self._log(
        ...
        # if the user downloaded it manually and it appears to be a
        # PROPER/REPACK then it's priority
        if self.is_proper and new_ep_quality >= old_ep_quality and new_ep_quality != common.Quality.UNKNOWN:
            self._log(
    ...
    def process(self):
        ...
        self._log(u"Processing " + self.file_path +
                  " (" + str(self.nzb_name) + ")")
        ...
        if not ek(os.path.exists, self.file_path):
            self._log(
                u"File %s doesn't exist, did unrar fail?" %
                self.file_path)
            return False
        ...
        for ignore_file in self.IGNORED_FILESTRINGS:
            if ignore_file in self.file_path:
                self._log(
                    u"File %s is ignored type, skipping" %
                    self.file_path)
                return False
        ...
        if not show:
            self._log(
                u"This show isn't in your list, you need to add it to SR before post-processing an episode")
            raise EpisodePostProcessingFailedException()
        elif season is None or not episodes:
            self._log(
                u"Not enough information to determine what episode this is. Quitting post-processing")
            return False
        ...
        logger.log(
            u"Quality of the episode we're processing: %s" %
            new_ep_quality, logger.DEBUG)

        # see if this is a priority download (is it snatched, in history,
        # PROPER, or BEST)
        priority_download = self._is_priority(ep_obj, new_ep_quality)
        self._log(
            u"Is ep a priority download: " +
            str(priority_download),
            logger.DEBUG)
        ...
        # if it's not priority then we don't want to replace smaller files in
        # case it was a mistake
        if not priority_download:

            # Not a priority and the quality is lower than what we already have
            if (new_ep_quality < old_ep_quality and old_ep_quality !=
                    common.Quality.UNKNOWN) and not existing_file_status == PostProcessor.DOESNT_EXIST:
                self._log(
                    u"File exists and new file quality is lower than existing, marking it unsafe to replace")
                return False

            # if there's an existing file that we don't want to replace stop
            # here
        ...
        # if the file is priority then we're going to replace it even if it
        # exists
        else:
            self._log(
                u"This download is marked a priority download so I'm going to replace an existing file if I find one")

        # try to find out if we have enough space to perform the copy or move
        # action.
        if not helpers.isFileLocked(self.file_path, False):
            if not verify_freespace(self.file_path, ep_obj.show._location, [
                                    ep_obj] + ep_obj.relatedEps):
                self._log(
            ...
            self._log(
                "Unable to determine needed filespace as the source file is locked for access")
            ...
            try:
                ...
                if cur_ep.location:
                    helpers.delete_empty_folders(
                        ek(os.path.dirname, cur_ep.location), keep_dir=ep_obj.show._location)
        ...
        if not ek(os.path.isdir,
            ...
            self._log(
                u"Show directory doesn't exist, creating it",
                logger.DEBUG)
            try:
            ...
            # get metadata for the show (but not episode because it hasn't been
            # fully processed)
            ep_obj.show.writeMetadata(True)

        # update the ep info before we rename so the quality & release name go
        # into the name properly
        sql_l = []
        ...
        if releaseName is not None:
            ...
            self._log(
                u"Couldn't find release in snatch history",
                logger.WARNING)

        # find the destination folder
        try:
            proper_path = ep_obj.proper_path()
            proper_absolute_path = ek(
                os.path.join, ep_obj.show.location, proper_path)
        ...
        self._log(
            u"Destination folder for this episode: " +
            dest_path,
            logger.DEBUG)
        ...
        if sickbeard.RENAME_EPISODES:
            ...
            # if we're not renaming then there's no new base name, we'll just
        ...
        try:
            ...
            if self.process_method == "copy":
                ...
                if helpers.isFileLocked(self.file_path, True):
                    raise EpisodePostProcessingFailedException(
                        "File is locked for reading/writing")
                ...
                logger.log(u"Unknown process method: " +
                           str(self.process_method), logger.ERROR)
                raise EpisodePostProcessingFailedException(
        ...
        # now that processing has finished, we can put the info in the DB. If
        # we do it earlier, then when processing fails, it won't try again.
        if len(sql_l) > 0:
            myDB = db.DBConnection()
        ...
        try:
            ...
            # nmj_notifier kicks off its library update when the
            # notify_download is issued (inside notifiers)
            ...
            logger.log(
                u"Some notifications could not be sent. Continuing with postProcessing...")

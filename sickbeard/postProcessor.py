...
class PostProcessor(object):
    ...
    def list_associated_files(
        ...
        if subfolders:  # subfolders are only checked in show folder, so names will always be exactly alike
            filelist = recursive_glob(ek(os.path.dirname, globbable_file_path),
        ...
        if file_path_list:
            self._log(
                "Found the following associated files: " +
                str(file_path_list),
                logging.DEBUG)
        else:
            self._log(
                "No associated files were during this pass",
                logging.DEBUG)
    ...
    def _delete(self, file_path, associated_files=False):
        ...
        if not file_list:
            self._log(
                "There were no files associated with " +
                file_path +
        ...
        for cur_file in file_list:
            if ek(os.path.isfile, cur_file):
                self._log("Deleting file " + cur_file, logging.DEBUG)
    ...
    def _move(self, file_path, new_path, new_base_name,
        ...
        def _int_move(cur_file_path, new_file_path):
            ...
            try:
                ...
                self._log("Unable to move file " + cur_file_path + " to " + new_file_path + ": {}".format(ex(e)),
                          logging.ERROR)
                raise
    ...
    def _moveAndSymlink(self, file_path, new_path, new_base_name,
        ...
        def _int_move_and_sym_link(cur_file_path, new_file_path):
            ...
            try:
                ...
                self._log("Unable to link file " + cur_file_path + " to " + new_file_path + ": {}".format(ex(e)),
                          logging.ERROR)
                raise
    ...
    def _finalize(self, parse_result):
        ...
        if parse_result.series_name and ((parse_result.season_number is not None and parse_result.episode_numbers)
            ...
            logging.debug("Parse result(series_name): " +
                          str(parse_result.series_name))
            logging.debug("Parse result(season_number): " +
                          str(parse_result.season_number))
            logging.debug("Parse result(episode_numbers): " +
                          str(parse_result.episode_numbers))
            logging.debug("Parse result(air_date): " +
                          str(parse_result.air_date))
            logging.debug("Parse result(release_group): " +
                          str(parse_result.release_group))
    ...
    def _analyze_name(self, name):
        ...
        logging.debug("Analyzing name " + repr(name))
    ...
    def _find_info(self):
        ...
        for cur_attempt in attempt_list:
            ...
            try:
                ...
                logging.debug("Unable to parse, skipping: {}".format(ex(e)))
                continue
                ...
                if sql_result:
                    ...
                    if sql_result:
                        ...
                        self._log(
                            ...
                            str(episodes[0]) + " for show " + str(show.indexerid) + ", skipping", logging.DEBUG)
    ...
    def _get_ep_obj(self, show, season, episodes):
        ...
        for cur_episode in episodes:
            ...
            try:
                ...
                self._log(
                    "Unable to create episode: {}".format(
                        ex(e)), logging.DEBUG)
    ...
    def _get_quality(self, ep_obj):
        ...
        for cur_name in name_list:
            ...
            self._log(
                "Looking up quality for name " + cur_name + ", got " +
                common.Quality.qualityStrings[ep_quality],
                logging.DEBUG)

            # if we find a good one then use it
            if ep_quality != common.Quality.UNKNOWN:
                logging.debug(cur_name + " looks like it has quality " + common.Quality.qualityStrings[
        ...
        self._log(
            "Guessing quality for name " + self.file_name +
            ", got " + common.Quality.qualityStrings[ep_quality],
            logging.DEBUG)

        if ep_quality != common.Quality.UNKNOWN:
            logging.debug(self.file_name + " looks like it has quality " + common.Quality.qualityStrings[
                ep_quality] + ", using that")
    ...
    def _run_extra_scripts(self, ep_obj):
        ...
        for curScriptName in sickbeard.EXTRA_SCRIPTS:
            ...
            try:
                ...
                self._log("Unable to run extra_script: {}".format(ex(e)))
    ...
    def _is_priority(self, ep_obj, new_ep_quality):
        ...
        if self.in_history or ep_obj.status in common.Quality.SNATCHED + \
            ...
            if not self.in_history:
                self._log(
                    "SR snatched this episode and it is not processed before",
                    logging.DEBUG)
                return True
    ...
    def process(self):
        ...
        self._log("Processing " + self.file_path +
                  " (" + str(self.nzb_name) + ")")
        ...
        if not ek(os.path.exists, self.file_path):
            self._log(
                "File %s doesn't exist, did unrar fail?" %
                self.file_path)
            return False
        ...
        if not show:
            self._log(
                "This show isn't in your list, you need to add it to SR before post-processing an episode")
            raise EpisodePostProcessingFailedException()
        ...
        logging.debug(
            "Quality of the episode we're processing: %s" %
            new_ep_quality)
        ...
        if not priority_download:
            ...
            if (
                    new_ep_quality < old_ep_quality and old_ep_quality != common.Quality.UNKNOWN) and not existing_file_status == PostProcessor.DOESNT_EXIST:
                self._log(
        ...
        if not ek(os.path.isdir,
            ...
            self._log(
                "Show directory doesn't exist, creating it",
                logging.DEBUG)
            try:
        ...
        if releaseName is not None:
            ...
            self._log(
                "Couldn't find release in snatch history",
                logging.WARNING)
        ...
        self._log(
            "Destination folder for this episode: " +
            dest_path,
            logging.DEBUG)
        ...
        try:
            ...
            if self.process_method == "copy":
                ...
                logging.error("Unknown process method: " +
                              str(self.process_method))
                raise EpisodePostProcessingFailedException(
        ...
        try:
            ...
            logging.info(
                "Some notifications could not be sent. Continuing with postProcessing...")

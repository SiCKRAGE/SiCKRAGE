...
def delete_folder(folder, check_empty=True):
    ...
    if check_empty:
        ...
        try:
            ...
            logger.log(u"Warning: unable to delete folder: " +
                       folder + ": " + ex(e), logger.WARNING)
            return False
    else:
        try:
...
def delete_files(processPath, notwantedFiles, result, force=False):
    ...
    if not result.result and force:
        result.output += logHelper(
            u"Forcing deletion of files, even though last result was not success",
            logger.DEBUG)
    ...
    for cur_file in notwantedFiles:
        ...
        if not file_attribute & stat.S_IWRITE:
            ...
            result.output += logHelper(
                u"Changing ReadOnly Flag for file " + cur_file, logger.DEBUG)
            try:
        ...
        try:
            ...
            result.output += logHelper(u"Unable to delete file " +
                                       cur_file + ': ' + str(e.strerror), logger.DEBUG)
...
def processDir(dirName, nzbName=None, process_method=None, force=False,
               is_priority=None, delete_on=False, failed=False, type="auto"):
    """
    ...
    result.output += logHelper(u"TV_DOWNLOAD_DIR: " +
                               sickbeard.TV_DOWNLOAD_DIR, logger.DEBUG)
    postpone = False
    # if they passed us a real dir then assume it's the one we want
    if ek(os.path.isdir, dirName):
        dirName = ek(os.path.realpath, dirName)

    # if the client and SickRage are not on the same machine translate the Dir
    # in a network dir
        ...
        dirName = ek(os.path.join, sickbeard.TV_DOWNLOAD_DIR, ek(
            os.path.abspath, dirName).split(os.path.sep)[-1])
        result.output += logHelper(u"Trying to use folder " +
                                   dirName, logger.DEBUG)
    ...
    # Don't post process if files are still being synced and option is
    # activated
    ...
    if not postpone:
        result.output += logHelper(u"PostProcessing Path: " +
                                   path, logger.INFO)
        result.output += logHelper(u"PostProcessing Dirs: " +
                                   str(dirs), logger.DEBUG)
        ...
        result.output += logHelper(u"PostProcessing Files: " +
                                   str(files), logger.DEBUG)
        result.output += logHelper(u"PostProcessing VideoFiles: " +
                                   str(videoFiles), logger.DEBUG)
        result.output += logHelper(u"PostProcessing RarContent: " +
                                   str(rarContent), logger.DEBUG)
        result.output += logHelper(u"PostProcessing VideoInRar: " +
                                   str(videoInRar), logger.DEBUG)

        # If nzbName is set and there's more than one videofile in the folder,
        # files will be lost (overwritten).
        if len(videoFiles) >= 2:
            nzbName = None
        ...
        # Don't Link media when the media is extracted from a rar in the same
        # path
        ...
        result.output += logHelper(
            u"Found temporary sync files, skipping post processing for folder " +
            str(path))
        result.output += logHelper(u"Sync Files: " +
                                   str(SyncFiles) + " in path: " + path)
        result.missedfiles.append(path + " : Syncfiles found")

    # Process Video File in all TV Subdir
    for dir in [x for x in dirs if validateDir(
            path, x, nzbNameOriginal, failed, result)]:

        result.result = True

        for processPath, processDir, fileList in ek(
                os.walk, ek(os.path.join, path, dir), topdown=False):
            ...
            if not postpone:
                ...
                if notwantedFiles:
                    result.output += logHelper(
                        u"Found unwanted files: " + str(notwantedFiles), logger.DEBUG)

                # Don't Link media when the media is extracted from a rar in
                # the same path
                if process_method in ('hardlink', 'symlink') and videoInRar:
                    process_media(
                        processPath,
                    ...
                    if (not sickbeard.NO_DELETE or type == "manual") and process_method == "move" and \
                        ...
                        if delete_folder(processPath, check_empty=True):
                            result.output += logHelper(
                                u"Deleted folder: " + processPath, logger.DEBUG)
            else:
                result.output += logHelper(
                    u"Found temporary sync files, skipping post processing for folder: " +
                    str(processPath))
                result.output += logHelper(u"Sync Files: " +
                                           str(SyncFiles) + " in path: " + processPath)
...
def validateDir(path, dirName, nzbNameOriginal, failed, result):
    ...
    if helpers.is_hidden_folder(os.path.join(path, dirName)):
        result.output += logHelper(u"Ignoring hidden folder: " +
                                   dirName, logger.DEBUG)
    ...
    allDirs = []
    for processPath, processDir, fileList in ek(
            os.walk, ek(os.path.join, path, dirName), topdown=False):
        allDirs += processDir
        allFiles += fileList
    ...
    result.output += logHelper(dirName +
                               " : No processable items found in folder", logger.DEBUG)
    return False
...
def unRAR(path, rarFiles, force, result):
    ...
    if sickbeard.UNPACK and rarFiles:
        ...
        result.output += logHelper(u"Packed Releases detected: " +
                                   str(rarFiles), logger.DEBUG)

        for archive in rarFiles:

            result.output += logHelper(u"Unpacking archive: " +
                                       archive, logger.DEBUG)

            try:
                rar_handle = RarFile(os.path.join(path, archive))

                # Skip extraction if any file in archive has previously been
                # extracted
                skip_file = False
                for file_in_archive in [os.path.basename(
                        x.filename) for x in rar_handle.infolist() if not x.isdir]:
                    if already_postprocessed(
                            path, file_in_archive, force, result):
                        result.output += logHelper(
                ...
                rar_handle.extract(
                    path=path, withSubpath=False, overwrite=False)
                ...
                result.output += logHelper(
                    u"Failed Unrar archive {0}: Unrar: Archive Header Broken".format(
                        archive),
                    logger.ERROR)
                result.result = False
                result.missedfiles.append(
                    archive + " : Unpacking failed because the Archive Header is Broken")
                continue
            except IncorrectRARPassword:
                result.output += logHelper(
                    u"Failed Unrar archive {0}: Unrar: Incorrect Rar Password".format(
                        archive),
                    logger.ERROR)
                result.result = False
                result.missedfiles.append(
                    archive + " : Unpacking failed because of an Incorrect Rar Password")
                continue
            except FileOpenError:
                result.output += logHelper(
                    u"Failed Unrar archive {0}: Unrar: File Open Error, check the parent folder and destination file permissions.".format(
                        archive),
                    logger.ERROR)
                result.result = False
                result.missedfiles.append(
                    archive + " : Unpacking failed with a File Open Error (file permissions?)")
                continue
            except InvalidRARArchiveUsage:
                result.output += logHelper(
                    u"Failed Unrar archive {0}: Unrar: Invalid Rar Archive Usage".format(
                        archive),
                    logger.ERROR)
                result.result = False
                result.missedfiles.append(
                    archive + " : Unpacking Failed with Invalid Rar Archive Usage")
                continue
            except InvalidRARArchive:
                result.output += logHelper(
                    u"Failed Unrar archive {0}: Unrar: Invalid Rar Archive".format(
                        archive),
                    logger.ERROR)
                result.result = False
                result.missedfiles.append(
                    archive + " : Unpacking Failed with an Invalid Rar Archive Error")
                continue
            except Exception as e:
                result.output += logHelper(u"Failed Unrar archive " +
                                           archive + ': ' + ex(e), logger.ERROR)
                result.result = False
                result.missedfiles.append(
                    archive + " : Unpacking failed for an unknown reason")
                continue

        result.output += logHelper(u"UnRar content: " +
                                   str(unpacked_files), logger.DEBUG)
...
def already_postprocessed(dirName, videofile, force, result):
    ...
    if sqlResult:
        ...
        # But we need to make sure we check the history of the episode we're
        # going to PP, and not others
        ...
        # If we find a showid, a season number, and one or more episode numbers
        # then we need to use those in the query
        if parse_result and (
                parse_result.show.indexerid and parse_result.episode_numbers and parse_result.season_number):
            search_sql += " and tv_episodes.showid = '" + str(parse_result.show.indexerid) + "' and tv_episodes.season = '" + str(
                parse_result.season_number) + "' and tv_episodes.episode = '" + str(parse_result.episode_numbers[0]) + "'"
...
def process_media(processPath, videoFiles, nzbName,
    ...
    for cur_video_file in videoFiles:
        ...
        if already_postprocessed(processPath, cur_video_file, force, result):
            result.output += logHelper(u"Already Processed " +
                                       cur_video_file_path +
                                       " : Skipping", logger.DEBUG)
            continue

        try:
            processor = postProcessor.PostProcessor(
                cur_video_file_path, nzbName, process_method, is_priority)
    ...
    if sickbeard.USE_FAILED_DOWNLOADS:
        ...
        if sickbeard.DELETE_FAILED and result.result:
            if delete_folder(dirName, check_empty=False):
                result.output += logHelper(u"Deleted folder: " +
                                           dirName, logger.DEBUG)

        if result.result:
            result.output += logHelper(
                u"Failed Download Processing succeeded: (" + str(nzbName) + ", " + dirName + ")")
        else:
            result.output += logHelper(
                u"Failed Download Processing failed: (" + str(

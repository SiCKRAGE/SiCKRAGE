...
def validateDir(path, dirName, nzbNameOriginal, failed, result):
    ...
    for sqlShow in db.DBConnection().select("SELECT * FROM tv_shows"):
...
def already_postprocessed(dirName, videofile, force, result):
    ...
    if db.DBConnection().select("SELECT * FROM tv_episodes WHERE release_name = ?", [dirName]):
        # result.output += logHelper(u"You're trying to post process a dir that's already been processed, skipping", logging.DEBUG)
        return True

    else:
        if db.DBConnection().select("SELECT * FROM tv_episodes WHERE release_name = ?", [videofile.rpartition('.')[0]]):

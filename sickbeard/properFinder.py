...
class ProperFinder:
    ...
    def run(self, force=False):
        ...
        if None is sickbeard.properFinderScheduler.start_time:
            run_in = sickbeard.properFinderScheduler.lastRun + \
                sickbeard.properFinderScheduler.cycleTime - datetime.datetime.now()
    ...
    def _getProperList(self):
        ...
        providers = [x for x in sickbeard.providers.sortedProviderList(
            sickbeard.RANDOMIZE_PROVIDERS) if x.isActive()]
        for curProvider in providers:
            threading.currentThread().name = origThreadName + \
                " :: [" + curProvider.name + "]"

            logger.log(
                u"Searching for any new PROPER releases from " +
                curProvider.name)
            ...
            # if they haven't been added by a different provider than add the
            # proper to the list
            for x in curPropers:
                if not re.search(
                        r'(^|[\. _-])(proper|repack)([\. _-]|$)', x.name, re.I):
                    logger.log(
                        u'findPropers returned a non-proper, we have caught and skipped it.',
                        logger.DEBUG)
                    continue
        ...
        for curProper in sortedPropers:
            ...
            curProper.quality = Quality.nameQuality(
                curProper.name, parse_result.is_anime)
            ...
            if oldStatus not in (
                    DOWNLOADED, SNATCHED) or oldQuality != bestResult.quality:
                continue

            # check if we actually want this proper (if it's the right release
            # group and a higher version)
            if bestResult.show.is_anime:
                myDB = db.DBConnection()
            ...
            # if the show is in our list and there hasn't been a proper already
            # added for that particular episode then add it to our list of
            # propers
    ...
    def _downloadPropers(self, properList):
        ...
        for curProper in properList:
            ...
            # if we didn't download this episode in the first place we don't
            # know what quality to use for the proper so we can't do it
            if len(historyResults) == 0:
                ...
                # make sure that none of the existing history downloads are the
                # same proper we're trying to download
                clean_proper_name = self._genericName(
                    helpers.remove_non_release_groups(curProper.name))
                isSame = False
                for curResult in historyResults:
                    # if the result exists in history already we need to skip
                    # it
                    if self._genericName(helpers.remove_non_release_groups(
                ...
                epObj = curProper.show.getEpisode(
                    curProper.season, curProper.episode)
    ...
    def _set_lastProperSearch(self, when):
        ...
        logger.log(
            u"Setting the last Proper search in the DB to " +
            str(when),
            logger.DEBUG)

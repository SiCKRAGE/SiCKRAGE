...
class GenericProvider(object):
    ...
    def _makeURL(self, result):
        ...
        if result.url.startswith('magnet'):
            try:
                ...
                if not torrent_hash:
                    logging.error("Unable to extract torrent hash from magnet: " + result.url)
                    return urls, filename

                urls = random.shuffle(
                        [x.format(torrent_hash=torrent_hash, torrent_name=torrent_name) for x in self.btCacheURLS])
            except Exception:
                logging.error("Unable to extract torrent hash or name from magnet: " + result.url)
                return urls, filename
        else:
            urls = [result.url]

        if self.type == self.TORRENT:
            filename = os.path.join(sickbeard.TORRENT_DIR,
                                    helpers.sanitizeFileName(result.name) + '.' + self.type)

        elif self.type == self.NZB:
            filename = os.path.join(sickbeard.NZB_DIR,
                                    helpers.sanitizeFileName(result.name) + '.' + self.type)
    ...
    def _verify_download(self, file_name=None):
        ...
        if file_name.endswith(GenericProvider.TORRENT):
            try:
                ...
                logging.debug("Failed to validate torrent file: {}".format(e))
    ...
    def loadProvider(cls, name, *args, **kwargs):
        ...
        members = dict(
                inspect.getmembers(
                        importlib.import_module('.{}.{}'.format(cls.type, name), 'sickbeard.providers'),
                        inspect.isclass)
        )
        instance = [v for v in members.values() if hasattr(v, 'type') and v.type == cls.type][0](*args, **kwargs)
        return instance
...
class NewznabProvider(NZBProvider):
    ...
    def imageName(self):
        if os.path.isfile(
                os.path.join(sickbeard.PROG_DIR, 'gui', sickbeard.GUI_NAME, 'images', 'providers', self.id + '.png')):
            return self.id + '.png'
        return 'newznab.png'
    ...
    def _doSearch(self, search_params, search_mode='eponly', epcount=0, age=0, epObj=None):
        ...
        if search_params:
            ...
            logging.debug('Search parameters: %s' % repr(search_params))
        ...
        params[b'maxage'] = min(params[b'maxage'], sickbeard.USENET_RETENTION)

        results = []
        offset = total = 0

        if 'lolo.sickbeard.com' in self.url and params[b'maxage'] < 33:
            params[b'maxage'] = 33
    ...
    def findPropers(self, search_date=datetime.datetime.today()):
        ...
        sqlResults = myDB.select(
                ...
                ' INNER JOIN tv_shows AS s ON (e.showid = s.indexer_id)' +
                ' WHERE e.airdate >= ' + str(search_date.toordinal()) +
                ' AND (e.status IN (' + ','.join([str(x) for x in common.Quality.DOWNLOADED]) + ')' +
                ' OR (e.status IN (' + ','.join([str(x) for x in common.Quality.SNATCHED]) + ')))'
        )
...
class TorrentRssCache(tvcache.TVCache):
    ...
    def _getRSSData(self):
        logging.debug("Cache update URL: %s" % self.provider.url)
...
class NewznabCache(tvcache.TVCache):
    ...
    def _parseItem(self, item):
        ...
        logging.debug("Attempting to add item from RSS to cache: %s" % title)
...
def sortedProviderDict(randomize=False):
    ...
    providerOrder = sickbeard.PROVIDER_ORDER

    if randomize:
        random.shuffle(providerOrder)

    for providerType in [GenericProvider.NZB, GenericProvider.TORRENT]:
        sortedProviders.update((key, sickbeard.providersDict[providerType][key]) for key in providerOrder
                               if sickbeard.providersDict[providerType].has_key(key))

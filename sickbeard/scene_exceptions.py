    ...
    if indexer_id not in exceptionsSeasonCache:
        ...
        if sqlResults:
            exceptionsSeasonList = list(
                set([int(x["season"]) for x in sqlResults]))
...
def get_scene_exception_by_name_multiple(show_name):
    ...
    for cur_exception in all_exception_results:
        ...
        if show_name.lower() in (
            ...
            logger.log(
                u"Scene exception lookup got indexer id " +
                str(cur_indexer_id) +
                u", using that",
                logger.DEBUG)
...
def retrieve_exceptions():
    ...
    for indexer in sickbeard.indexerApi().indexers:
        if shouldRefresh(sickbeard.indexerApi(indexer).name):
            logger.log(
                u"Checking for scene exception updates for " +
            ...
            try:
                data = helpers.getURL(
                    loc, session=sickbeard.indexerApi(indexer).session)
            except Exception:
                continue

            if data is None:
                # When data is None, trouble connecting to github, or reading
                # file failed
                logger.log(
            ...
            # each exception is on one line with the format indexer_id: 'show
            # name 1', 'show name 2', etc
    ...
    for xem_ex in xem_exception_dict:
        if xem_ex in exception_dict:
            exception_dict[xem_ex] = exception_dict[
                xem_ex] + xem_exception_dict[xem_ex]
        else:
...
def update_scene_exceptions(indexer_id, scene_exceptions, season=-1):
    ...
    # A change has been made to the scene exception list. Let's clear the
    # cache, to make this visible
...
def _xem_exceptions_fetcher():
    ...
    if shouldRefresh('xem'):
        for indexer in sickbeard.indexerApi().indexers:
            logger.log(
                u"Checking for XEM scene exception updates for " +
                sickbeard.indexerApi(indexer).name)

            url = "http://thexem.de/map/allNames?origin=%s&seasonNumbers=1" % sickbeard.indexerApi(indexer).config[
                'xem_origin']

            parsedJSON = helpers.getURL(
                url, session=xem_session, timeout=90, json=True)
            if not parsedJSON:
            ...
            for indexerid, names in parsedJSON['data'].iteritems():
                try:
                    ...
                    logger.log(
                        u"XEM: Rejected entry: indexerid:{0}; names:{1}".format(
                            indexerid, names), logger.WARNING)
                    logger.log(
                        u"XEM: Rejected entry error message:{0}".format(
                            str(e)), logger.DEBUG)

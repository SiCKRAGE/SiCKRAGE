...
class Scheduler(threading.Thread):
    ...
    def __init__(self, action, cycleTime=datetime.timedelta(minutes=10), run_delay=datetime.timedelta(minutes=0),
        ...
        if start_time is None:
            ...
            self.lastRun = datetime.datetime(
                temp_now.year,
                temp_now.month,
                temp_now.day,
                temp_now.hour,
    ...
    def timeLeft(self):
        ...
        if self.isAlive():
            if self.start_time is None:
                ...
                start_time_tomorrow = datetime.datetime.combine(
                    time_now.date(), self.start_time) + datetime.timedelta(days=1)
    ...
    def run(self):
        ...
        try:
            while not self.stop.is_set():
                if self.enable:
                    ...
                    if should_run:
                        ...
                        if not self.silent:
                            logger.log(
                                u"Starting new thread: " + self.name, logger.DEBUG)
            ...
            logger.log(u"Exception generated in thread " +

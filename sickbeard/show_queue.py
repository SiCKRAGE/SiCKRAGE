...
class ShowQueue(generic_queue.GenericQueue):
    ...
    def refreshShow(self, show, force=False):
        ...
        if (self.isBeingUpdated(show) or self.isInUpdateQueue(show)) and not force:
            logger.log(
                u"A refresh was attempted but there is already an update queued or in progress. Since updates do a "
                u"refresh at the end anyway I'm skipping this request.",
                logger.DEBUG)
            return
...
class QueueItemAdd(ShowQueueItem):
    ...
    def run(self):
        ...
        try:
            ...
            # this usually only happens if they have an NFO in their show dir which gave us a Indexer ID
            # that has no proper english version of the show
            if getattr(s, 'seriesname', None) is None:
                logger.log(
                    u"Show in " + self.showDir + " has no name on " +
                    str(sickbeard.indexerApi(self.indexer).name) +
                    ", probably the wrong language used to search with.", logger.ERROR
                )
                ui.notifications.error(
                    "Unable to add show", "Show in " + self.showDir + " has no name on " +
                    str(sickbeard.indexerApi(self.indexer).name) +
            ...
            if not s:
                logger.log(
                    u"Show " + str(s['seriesname']) + " is on " + str(sickbeard.indexerApi(self.indexer).name) +
                    " but contains no season/episode data.", logger.ERROR
                )
                ui.notifications.error(
                    "Unable to add show",
                    "Show " + str(s['seriesname']) + " is on " + str(sickbeard.indexerApi(self.indexer).name) +
            ...
            logger.log(
                u"Unable to find show ID:" + str(self.indexer_id) + " on Indexer: " +
                str(sickbeard.indexerApi(self.indexer).name), logger.ERROR
            )
            ui.notifications.error(
                "Unable to add show", "Unable to look up the show in " +
                self.showDir + " on " + str(sickbeard.indexerApi(self.indexer).name) + " using ID " +
                str(self.indexer_id) + ", not using the NFO. Delete .nfo and try adding manually again."
            )
            self._finishEarly()
            return
        ...
        # check if show has XEM mapping so we can determin if searches should go by
        # scene numbering or indexer numbering.

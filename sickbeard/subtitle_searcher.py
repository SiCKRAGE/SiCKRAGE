...
def subtitlesLanguages(video_path):
    ...
    if os.path.isabs(sickbeard.SUBTITLES_DIR):
        video_path = os.path.join(sickbeard.SUBTITLES_DIR, os.path.basename(video_path))
    # Search subtitles with the relative path
    elif sickbeard.SUBTITLES_DIR:
        check_subtitles_path = os.path.join(os.path.dirname(video_path), sickbeard.SUBTITLES_DIR)
        if not os.path.exists(check_subtitles_path):
            getSubtitlesPath(video_path)
        video_path = os.path.join(os.path.dirname(video_path), sickbeard.SUBTITLES_DIR,
            ...
            if (
                        (epToSub[b'airdate_daydiff'] > 7 and epToSub[
                            b'searchcount'] < 2 and now - datetime.datetime.strptime(
                                epToSub[b'lastsearch'], common.dateTimeFormat) > datetime.timedelta(
                ...
                showObj = helpers.findCertainShow(sickbeard.showList, int(epToSub[b'showid']))
                if not showObj:

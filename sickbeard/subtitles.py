...
def downloadSubtitles(subtitles_info):
    ...
    subtitles_path = getSubtitlesPath(
        subtitles_info[b'location']).encode(
...
def save_subtitles(video, subtitles, single=False, directory=None):
    ...
    for subtitle in subtitles:
        ...
        if directory is not None:
            subtitle_path = ek(
                os.path.join, directory, ek(
                    os.path.split, subtitle_path)[1])

        # save content as is or in the specified encoding
        logging.debug(
            "Saving subtitle for %s to %s" %
...
def scan_subtitle_languages(path):
    language_extensions = tuple(
        '.' + c for c in language_converters[b'opensubtitles'].codes)
        ...
        for epToSub in sqlResults:
            ...
            if (
                ...
                showObj = sickbeard.helpers.findCertainShow(
                    sickbeard.showList, int(epToSub[b'showid']))
                if not showObj:
                    logging.debug('Show not found')
                    return

                epObj = showObj.getEpisode(
                    int(epToSub[b"season"]), int(epToSub[b"episode"]))
    ...
    for curScriptName in sickbeard.SUBTITLES_EXTRA_SCRIPTS:
        ...
        for subtitle in found_subtitles:
            ...
            inner_cmd = script_cmd + [video.name, subtitle_path, subtitle.language.opensubtitles, epObj[b'show.name'],
                                      str(epObj[b'season']), str(
                                          epObj[b'episode']), epObj[b'name'],
            ...
            try:
                ...
                logging.info(
                    "Unable to run subs_extra_script: {}".format(
                        ex(e)))

...
def sortedServiceList():
    ...
    curIndex = 0
    for curService in SUBTITLES_SERVICES_LIST:
        if curService in subliminal.provider_manager.names():
            newList.append({'name': curService,
                            ...
                            'enabled': SUBTITLES_SERVICES_ENABLED[curIndex] == 1
                            })
        curIndex += 1
...
def downloadSubtitles(subtitles_info):
    ...
    subtitles_path = getSubtitlesPath(subtitles_info[b'location']).encode(SYS_ENCODING)
    video_path = subtitles_info[b'location'].encode(SYS_ENCODING)
    ...
    provider_configs = {'addic7ed': {'username': ADDIC7ED_USER, 'password': ADDIC7ED_PASS},
                        'legendastv': {'username': LEGENDASTV_USER, 'password': LEGENDASTV_PASS},
                        'opensubtitles': {'username': OPENSUBTITLES_USER,
    ...
    try:
        ...
        found_subtitles = pool.download_best_subtitles(subtitles_list, video, languages=languages,
                                                       ...
                                                       only_one=not SUBTITLES_MULTI)

        save_subtitles(video, found_subtitles, directory=subtitles_path, single=not SUBTITLES_MULTI)

        if not EMBEDDED_SUBTITLES_ALL and SUBTITLES_EXTRA_SCRIPTS and video_path.endswith(
                ('.mkv', '.mp4')):
            run_subs_extra_scripts(subtitles_info, found_subtitles, video, single=not SUBTITLES_MULTI)
...
def wantedLanguages(sqlLike=False):
    wanted = [x for x in sorted(SUBTITLES_LANGUAGES) if x in subtitleCodeFilter()]
    if sqlLike:
        return '%' + ','.join(wanted) + '%'
...
def getSubtitlesPath(video_path):
    if os.path.isabs(SUBTITLES_DIR):
        new_subtitles_path = SUBTITLES_DIR
    elif SUBTITLES_DIR:
        new_subtitles_path = ek(os.path.join, ek(os.path.dirname, video_path), SUBTITLES_DIR)
        dir_exists = helpers.makeDir(new_subtitles_path)
        if not dir_exists:
...
def subtitlesLanguages(video_path):
    ...
    if not EMBEDDED_SUBTITLES_ALL and video_path.endswith('.mkv'):
        embedded_subtitle_languages = getEmbeddedLanguages(video_path.encode(SYS_ENCODING))

    # Search subtitles with the absolute path
    if os.path.isabs(SUBTITLES_DIR):
        video_path = ek(os.path.join, SUBTITLES_DIR, ek(os.path.basename, video_path))
    # Search subtitles with the relative path
    elif SUBTITLES_DIR:
        check_subtitles_path = ek(os.path.join, ek(os.path.dirname, video_path), SUBTITLES_DIR)
        if not ek(os.path.exists, check_subtitles_path):
            getSubtitlesPath(video_path)
        video_path = ek(os.path.join, ek(os.path.dirname, video_path), SUBTITLES_DIR,
    ...
    def run(self, force=False):
        ...
        for epToSub in sqlResults:
            ...
            if (
                ...
                showObj = helpers.findCertainShow(showList, int(epToSub[b'showid']))
                if not showObj:
...
def run_subs_extra_scripts(epObj, found_subtitles, video, single=False):
    for curScriptName in SUBTITLES_EXTRA_SCRIPTS:

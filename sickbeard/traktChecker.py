...
def setEpisodeToWanted(show, s, e):
    ...
    if epObj:
        ...
        with epObj.lock:
            ...
            logger.log(
                u"Setting episode %s S%02dE%02d to wanted" %
                (show.name, s, e))
            # figure out what segment the episode is in and remember it so we
            # can backlog it
...
class TraktChecker():
    ...
    def findShow(self, indexer, indexerid):
        ...
        try:
            ...
            if not library:
                logger.log(
                    u"Could not connect to trakt service, aborting library check",
                    logger.WARNING)
                return

            if not len(library):
                logger.log(
                    u"No shows found in your library, aborting library update",
                    logger.DEBUG)
                return

            traktShow = filter(lambda x: int(indexerid) in [int(
                x['show']['ids']['tvdb'] or 0), int(x['show']['ids']['tvrage'] or 0)], library)
        except traktException as e:
            logger.log(
                u"Could not connect to Trakt service. Aborting library check. Error: %s" %
                repr(e), logger.WARNING)
    ...
    def removeShowFromTraktLibrary(self, show_obj):
        if self.findShow(show_obj.indexer, show_obj.indexerid):
            ...
            logger.log(
                u"Removing %s from trakt.tv library" %
                show_obj.name, logger.DEBUG)

            try:
                self.trakt_api.traktRequest(
                    "sync/collection/remove", data, method='POST')
            except traktException as e:
                logger.log(
                    u"Could not connect to Trakt service. Aborting removing show %s from Trakt library. Error: %s" %
                    (show_obj.name, repr(e)), logger.WARNING)
    ...
    def addShowToTraktLibrary(self, show_obj):
        ...
        if len(data):
            logger.log(
                u"Adding %s to trakt.tv library" %
                show_obj.name, logger.DEBUG)

            try:
                self.trakt_api.traktRequest(
                    "sync/collection", data, method='POST')
            except traktException as e:
                logger.log(
                    u"Could not connect to Trakt service. Aborting adding show %s to Trakt library. Error: %s" %
                    (show_obj.name, repr(e)), logger.WARNING)
                return
    ...
    def removeEpisodeFromTraktCollection(self):
        if sickbeard.TRAKT_SYNC_REMOVE and sickbeard.TRAKT_SYNC and sickbeard.USE_TRAKT:
            logger.log(
                u"COLLECTION::REMOVE::START - Look for Episodes to Remove From Trakt Collection",
                logger.DEBUG)
            ...
            if episodes is not None:
                ...
                for cur_episode in episodes:
                    ...
                    if self._checkInList(trakt_id, str(cur_episode["showid"]), str(
                            cur_episode["season"]), str(cur_episode["episode"]), List='Collection'):
            ...
            if len(trakt_data):
                ...
                try:
                    ...
                    logger.log(
                        u"Could not connect to Trakt service. Aborting removing episode %s S%02dE%02d from Trakt collection. Error: %s" %
                        (cur_episode["show_name"], cur_episode["season"], cur_episode["episode"], repr(e)), logger.WARNING)

            logger.log(
                u"COLLECTION::REMOVE::FINISH - Look for Episodes to Remove From Trakt Collection",
                logger.DEBUG)

    def addEpisodeToTraktCollection(self):
        if sickbeard.TRAKT_SYNC and sickbeard.USE_TRAKT:
            logger.log(
                u"COLLECTION::ADD::START - Look for Episodes to Add to Trakt Collection",
                logger.DEBUG)
            ...
            if episodes is not None:
                ...
                for cur_episode in episodes:
                    trakt_id = sickbeard.indexerApi(
                        cur_episode["indexer"]).config['trakt_id']
                    if not self._checkInList(trakt_id, str(cur_episode["showid"]), str(
                            cur_episode["season"]), str(cur_episode["episode"]), List='Collection'):
                ...
                if len(trakt_data):
                    ...
                    try:
                        ...
                        logger.log(
                            u"Could not connect to Trakt service. Aborting adding episode to Trakt collection. Error: %s" %
                            repr(e), logger.WARNING)

            logger.log(
                u"COLLECTION::ADD::FINISH - Look for Episodes to Add to Trakt Collection",
                logger.DEBUG)
    ...
    def removeEpisodeFromTraktWatchList(self):
        if sickbeard.TRAKT_SYNC_WATCHLIST and sickbeard.USE_TRAKT:
            logger.log(
                u"WATCHLIST::REMOVE::START - Look for Episodes to Remove from Trakt Watchlist",
                logger.DEBUG)
            ...
            if episodes is not None:
                ...
                for cur_episode in episodes:
                    trakt_id = sickbeard.indexerApi(
                        cur_episode["indexer"]).config['trakt_id']
                    if self._checkInList(trakt_id, str(cur_episode["showid"]), str(
                            cur_episode["season"]), str(cur_episode["episode"])):
                        if cur_episode["status"] not in Quality.SNATCHED + \
                                Quality.SNATCHED_PROPER + [UNKNOWN] + [WANTED]:
            ...
            if len(trakt_data):
                ...
                try:
                    ...
                    logger.log(
                        u"Could not connect to Trakt service. Aborting removing episode %s S%02dE%02d from Trakt watchlist. Error: %s" %
                        (cur_episode["show_name"], cur_episode["season"], cur_episode["episode"], repr(e)), logger.WARNING)

            logger.log(
                u"WATCHLIST::REMOVE::FINISH - Look for Episodes to Remove from Trakt Watchlist",
                logger.DEBUG)

    def addEpisodeToTraktWatchList(self):
        if sickbeard.TRAKT_SYNC_WATCHLIST and sickbeard.USE_TRAKT:
            logger.log(
                u"WATCHLIST::ADD::START - Look for Episodes to Add to Trakt Watchlist",
                logger.DEBUG)
            ...
            if episodes is not None:
                ...
                if len(trakt_data):
                    ...
                    try:
                        data = self.trakt_bulk_data_generate(trakt_data)
                        self.trakt_api.traktRequest(
                            "sync/watchlist", data, method='POST')
                        self._getEpisodeWatchlist()
                    except traktException as e:
                        logger.log(
                            u"Could not connect to Trakt service. Aborting adding episode %s S%02dE%02d to Trakt watchlist. Error: %s" %
                            (cur_episode["show_name"], cur_episode["season"], cur_episode["episode"], repr(e)), logger.WARNING)

            logger.log(
                u"WATCHLIST::ADD::FINISH - Look for Episodes to Add to Trakt Watchlist",
                logger.DEBUG)

    def addShowToTraktWatchList(self):
        if sickbeard.TRAKT_SYNC_WATCHLIST and sickbeard.USE_TRAKT:
            logger.log(
                u"SHOW_WATCHLIST::ADD::START - Look for Shows to Add to Trakt Watchlist",
                logger.DEBUG)
            ...
            if sickbeard.showList is not None:
                ...
                for show in sickbeard.showList:
                    ...
                    if not self._checkInList(trakt_id, str(
                        ...
                        logger.log(
                            u"Adding Show: Indexer %s %s - %s to Watchlist" %
                            (trakt_id, str(
                                show.indexerid), show.name), logger.DEBUG)
                        show_el = {
                            'title': show.name,
                            'year': show.startyear,
                            'ids': {}}
                ...
                if len(trakt_data):
                    ...
                    try:
                        ...
                        logger.log(
                            u"Could not connect to Trakt service. Aborting adding show %s to Trakt watchlist. Error: %s" %
                            (show.name, repr(e)), logger.WARNING)

            logger.log(
                u"SHOW_WATCHLIST::ADD::FINISH - Look for Shows to Add to Trakt Watchlist",
                logger.DEBUG)
    ...
    def removeShowFromSickRage(self):
        if sickbeard.TRAKT_SYNC_WATCHLIST and sickbeard.USE_TRAKT and sickbeard.TRAKT_REMOVE_SHOW_FROM_SICKRAGE:
            logger.log(
                u"SHOW_SICKRAGE::REMOVE::START - Look for Shows to remove from SickRage",
                logger.DEBUG)
                    ...
                    if show.status == "Ended":
                        try:
                            progress = self.trakt_api.traktRequest(
                                "shows/" + show.imdbid + "/progress/watched") or []
                        except traktException as e:
                            logger.log(
                                u"Could not connect to Trakt service. Aborting removing show %s from SickRage. Error: %s" %
                                (show.name, repr(e)), logger.WARNING)
                            return
                        ...
                        if 'aired' in progress and 'completed' in progress and progress[
                            ...
                            sickbeard.showQueueScheduler.action.removeShow(
                                show, full=True)
                            logger.log(
                                u"Show: %s has been removed from SickRage" %
                                show.name, logger.DEBUG)

            logger.log(
                u"SHOW_SICKRAGE::REMOVE::FINISH - Trakt Show Watchlist",
                logger.DEBUG)

    def updateShows(self):
        logger.log(
            u"SHOW_WATCHLIST::CHECK::START - Trakt Show Watchlist",
            logger.DEBUG)

        if not len(self.ShowWatchlist):
            logger.log(
                u"No shows found in your watchlist, aborting watchlist update",
                logger.DEBUG)
            return
        ...
        logger.log(
            u"SHOW_WATCHLIST::CHECK::FINISH - Trakt Show Watchlist",
            logger.DEBUG)
    ...
    def updateEpisodes(self):
        ...
        """
        logger.log(
            u"SHOW_WATCHLIST::CHECK::START - Trakt Episode Watchlist",
            logger.DEBUG)

        if not len(self.EpisodeWatchlist):
            logger.log(
                u"No episode found in your watchlist, aborting episode update",
                logger.DEBUG)
            return
        ...
        for show_el in self.EpisodeWatchlist[trakt_id]:
            ...
            try:
                ...
                logger.log(
                    u"Could not parse the output from trakt for %s " %
                    show["title"], logger.DEBUG)
        logger.log(
            u"SHOW_WATCHLIST::CHECK::FINISH - Trakt Episode Watchlist",
            logger.DEBUG)
    ...
    def addDefaultShow(self, indexer, indexer_id, name, status):
        ...
        if not helpers.findCertainShow(sickbeard.showList, int(indexer_id)):
            ...
            if location:
                ...
                if not dir_exists:
                    logger.log(
                        u"Unable to create the folder %s , can't add the show" %
                        showPath, logger.WARNING)
                    return
                else:
    ...
    def manageNewShow(self, show):
        logger.log(
            u"Checking if trakt watch list wants to search for episodes from new show " +
            show.name,
            logger.DEBUG)
    ...
    def _checkInList(self, trakt_id, showid, season, episode, List=None):
        ...
        if "Collection" == List:
            try:
                if self.Collectionlist[trakt_id][showid]['seasons'][
            ...
            try:
                if self.EpisodeWatchlist[trakt_id][showid]['seasons'][
                        season]['episodes'][episode] == episode:
                    return True
            except:
                return False
    ...
    def _getShowWatchlist(self):
        ...
        try:
            ...
            logger.log(
                u"Could not connect to trakt service, cannot download Show Watchlist: %s" %
                repr(e), logger.WARNING)
            return False
    ...
    def _getEpisodeWatchlist(self):
        ...
        try:
            ...
            for watchlist_el in TraktEpisodeWatchlist:
                ...
                if tvdb:
                    showid = str(watchlist_el['show']['ids'][tvdb_id])
                    if not showid in self.EpisodeWatchlist[
                            tvdb_id + '_id'].keys():
                        self.EpisodeWatchlist[
                            tvdb_id +
                            '_id'][showid] = {
                    ...
                    if not season in self.EpisodeWatchlist[
                    ...
                    if not episode in self.EpisodeWatchlist[
                            tvdb_id + '_id'][showid]['seasons'][season]['episodes'].keys():
                        self.EpisodeWatchlist[
                            tvdb_id + '_id'][showid]['seasons'][season]['episodes'][episode] = episode
                ...
                if tvrage:
                    ...
                    if not episode in self.EpisodeWatchlist[
                            tvrage_id + '_id'][showid]['seasons'][season]['episodes'].keys():
                        self.EpisodeWatchlist[
                            tvrage_id + '_id'][showid]['seasons'][season]['episodes'][episode] = episode

        except traktException as e:
            logger.log(
                u"Could not connect to trakt service, cannot download Episode Watchlist: %s" %
                repr(e), logger.WARNING)
            return False
    ...
    def _getShowCollection(self):
        ...
        try:
                    ...
                    for season_el in watchlist_el['seasons']:
                        for episode_el in season_el['episodes']:
                            ...
                            if tvdb:
                                ...
                                if not showid in self.Collectionlist[
                                        tvdb_id + '_id'].keys():
                                    self.Collectionlist[
                                        tvdb_id +
                                        '_id'][showid] = {
                                ...
                                if not season in self.Collectionlist[
                                        tvdb_id + '_id'][showid]['seasons'].keys():
                                    self.Collectionlist[
                                        tvdb_id +
                                        '_id'][showid]['seasons'][season] = {
                                ...
                                if not episode in self.Collectionlist[
                                        tvdb_id + '_id'][showid]['seasons'][season]['episodes'].keys():
                                    self.Collectionlist[
                                        tvdb_id + '_id'][showid]['seasons'][season]['episodes'][episode] = episode
            ...
            logger.log(
                u"Could not connect to trakt service, cannot download Show Collection: %s" %
                repr(e), logger.WARNING)
            return False

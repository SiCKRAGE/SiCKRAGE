...
def setEpisodeToWanted(show, s, e):
    ...
    if epObj:
        ...
        with epObj.lock:
            if epObj.status != common.SKIPPED or epObj.airdate == datetime.date.fromordinal(1):
                return
...
class TraktSearcher(object):
    def __init__(self, *args, **kwargs):
        ...
        self.trakt_api = trakt.TraktAPI(sickbeard.SSL_VERIFY, sickbeard.TRAKT_TIMEOUT)
        self.todoBacklog = []
        self.todoWanted = []
        self.ShowWatchlist = {}
    ...
    def findShow(self, indexer, indexerid):
        ...
        except trakt.traktException as e:
            ...
            if episodes is not None:
                ...
                if len(trakt_data):
                    try:
                        ...
                        logging.warning("Could not connect to Trakt service. Error: %s" % e)
    ...
    def addEpisodeToTraktCollection(self):
        if sickbeard.TRAKT_SYNC and sickbeard.USE_TRAKT:
            ...
            sql_selection = 'SELECT tv_shows.indexer, tv_shows.startyear, showid, show_name, season, episode FROM tv_episodes,tv_shows WHERE tv_shows.indexer_id = tv_episodes.showid AND tv_episodes.status IN (' + ','.join(
                    [str(x) for x in common.Quality.DOWNLOADED + [common.ARCHIVED]]) + ')'
            episodes = myDB.select(sql_selection)
            ...
            if episodes is not None:
                ...
                for cur_episode in episodes:
                    ...
                    if self._checkInList(trakt_id, str(cur_episode[b"showid"]), str(cur_episode[b"season"]),
                        ...
                        b"status"] not in common.Quality.SNATCHED + common.Quality.SNATCHED_PROPER + [
                        common.UNKNOWN] + [common.WANTED]:
    ...
    def addEpisodeToTraktWatchList(self):
        if sickbeard.TRAKT_SYNC_WATCHLIST and sickbeard.USE_TRAKT:
            ...
            myDB = db.DBConnection()
            sql_selection = 'SELECT tv_shows.indexer, tv_shows.startyear, showid, show_name, season, episode FROM tv_episodes,tv_shows WHERE tv_shows.indexer_id = tv_episodes.showid AND tv_episodes.status IN (' + ','.join(
                    [str(x) for x in common.Quality.SNATCHED + common.Quality.SNATCHED_PROPER + [common.WANTED]]) + ')'
            episodes = myDB.select(sql_selection)
                ...
                for show in sickbeard.showList:
                    if show.status == "Ended":
                        ...
                        if 'aired' in progress and 'completed' in progress and progress[b'aired'] == progress[
                            b'completed']:
                            sickbeard.showQueue.removeShow(show, full=True)
    ...
    def updateShows(self):
        ...
        for show_el in self.ShowWatchlist[trakt_id]:
            ...
            if int(sickbeard.TRAKT_METHOD_ADD) != 2:
                self.addDefaultShow(indexer, indexer_id, show[b'title'], common.SKIPPED)
            else:
                self.addDefaultShow(indexer, indexer_id, show[b'title'], common.WANTED)
    ...
    def addDefaultShow(indexer, indexer_id, name, status):
        ...
        if not helpers.findCertainShow(sickbeard.showList, int(indexer_id)):
            ...
            if location:
                showPath = os.path.join(location, helpers.sanitizeFileName(name))
                ...
                sickbeard.showQueue.addShow(int(indexer), int(indexer_id), showPath,

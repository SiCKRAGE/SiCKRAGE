...
class TVShow(object):
    ...
    def getAllEpisodes(self, season=None, has_location=False):
        ...
        sql_selection = sql_selection + \
            " (SELECT COUNT (*) FROM tv_episodes WHERE showid = tve.showid AND " \
            "season = tve.season AND location != '' AND location = tve.location AND episode != tve.episode) " \
            "AS share_location "
        ...
        for cur_result in results:
            ...
            if cur_ep:
                ...
                if cur_ep.location:
                    ...
                    # and put them in relatedEps
                    if cur_result["share_location"] > 0:
                        related_eps_result = myDB.select(
                            "SELECT * FROM tv_episodes WHERE showid = ? AND season = ? AND location = ? "
                            "AND episode != ? ORDER BY episode ASC",
    ...
    def should_update(self, update_date=datetime.date.today()):
        ...
        # run logic against the current show latest aired and next unaired data to see
        # if we should bypass 'Ended' status
        ...
        sql_result = myDB.select(
            "SELECT * FROM tv_episodes WHERE showid = ? AND season > '0' AND airdate > '1' AND status > '1' "
            "ORDER BY airdate DESC LIMIT 1", [cur_indexerid])
        ...
        if (update_date - last_airdate) < datetime.timedelta(days=450) and (update_date - last_update_indexer) > \
    ...
    def loadEpisodesFromIndexer(self, cache=True):
        ...
        try:
            ...
            logger.log(
                u"" + sickbeard.indexerApi(self.indexer).name + " timed out, unable to update episodes from " +
    ...
    def getImages(self, fanart=None, poster=None):
        ...
        return fanart_result or poster_result or banner_result or season_posters_result \
            or season_banners_result or season_all_poster_result or season_all_banner_result
        ...
        for curEpNum in episodes:
            ...
            if curEp is None:
                ...
                if curEp.location and ek.ek(os.path.normpath, curEp.location) != ek.ek(os.path.normpath, file):
                    logger.log(
                        u"The old episode had a different file associated with it, I will re-check the quality "
                        u"based on the new filename " + file, logger.DEBUG
                    )
            ...
            # if they replace a file on me I'll make some attempt at re-checking the quality unless
            # I know it's the same file
            ...
            # check for status/quality changes as long as it's a new file
                ...
                if oldStatus == SNATCHED and oldQuality <= newQuality:
                    logger.log(
                        u"STATUS: this ep used to be snatched with quality " +
                        Quality.qualityStrings[oldQuality] + u" but a file exists with quality " +
                        Quality.qualityStrings[newQuality] + u" so I'm setting the status to DOWNLOADED", logger.DEBUG
                    )
                ...
                # if it was snatched proper and we found a higher quality one then allow the status change
                    ...
                    logger.log(
                        u"STATUS: this ep used to be snatched proper with quality " +
                        Quality.qualityStrings[oldQuality] + u" but a file exists with quality " +
                        Quality.qualityStrings[newQuality] + u" so I'm setting the status to DOWNLOADED", logger.DEBUG
                    )
                    newStatus = DOWNLOADED
                ...
                if newStatus is not None:
                    with curEp.lock:
                        logger.log(
                            u"STATUS: we have an associated file, so setting the status from " +
                            str(curEp.status) + u" to DOWNLOADED/" + str(Quality.statusFromName(file)), logger.DEBUG
                        )
    ...
    def loadIMDbInfo(self, imdbapi=None):
        ...
        if self.imdbid:
            ...
            for key in filter(lambda x: x.replace('_', ' ') in imdbTv.keys(), imdb_info.keys()):
                ...
                if isinstance(imdb_info[key], str) and isinstance(imdbTv.get(key), list):
    ...
    def nextEpisode(self):
        ...
        if not self.nextaired or self.nextaired and curDate > self.nextaired:
            ...
            sqlResults = myDB.select(
                "SELECT airdate, season, episode FROM tv_episodes WHERE showid = ? AND airdate >= ? "
                "AND status in (?,?) ORDER BY airdate ASC LIMIT 1",
                ...
                if not sickbeard.SKIP_REMOVED_FILES:
                    with curEp.lock:
                        ...
                        if curEp.location and curEp.status in Quality.DOWNLOADED:
                            logger.log(
                                str(self.indexerid) + u": Location for " + str(season) + "x" +
                                str(episode) + " doesn't exist, removing it and changing our status to IGNORED",
                                logger.DEBUG
                            )
    ...
    def downloadSubtitles(self, force=False):
        ...
        try:
            ...
            episodes = myDB.select(
                "SELECT location FROM tv_episodes WHERE showid = ? AND location NOT LIKE '' "
    ...
    def wantEpisode(self, season, episode, quality, manualSearch=False):
        ...
        logger.log(
            u"Checking if found episode " + str(season) + "x" + str(episode) + " is wanted at quality " +
        ...
        logger.log(
            u"any,best = " + str(anyQualities) + " " + str(bestQualities) + " and found " + str(quality),
            logger.DEBUG
        )
        ...
        if curStatus in Quality.DOWNLOADED + Quality.SNATCHED + Quality.SNATCHED_PROPER + \
                Quality.SNATCHED_BEST and quality in bestQualities and quality > curQuality:
            logger.log(
                u"Episode already exists but the found episode has better quality, getting found episode",
                logger.DEBUG
            )
            return True
        else:
            logger.log(
                u"Episode already exists and the found episode has same/lower quality, ignoring found episode",
                logger.DEBUG
            )
...
class TVEpisode(object):
    ...
    def downloadSubtitles(self, force=False):
        ...
        # added the if because sometime it raise an error
        ...
        if newsubtitles:
            ...
            logger.log(
                str(self.show.indexerid) + u": Downloaded " + subtitleList +
                " subtitles for episode " + str(self.season) + "x" + str(self.episode), logger.DEBUG
            )
            notifiers.notify_subtitle_download(self.prettyName(), subtitleList)

        else:
            logger.log(
                str(self.show.indexerid) + u": No subtitles downloaded for episode " + str(self.season) + "x" +
                str(self.episode), logger.DEBUG
            )
    ...
    def loadFromDB(self, season, episode):
        ...
        if len(sqlResults) > 1:
            ...
            # self.name + " to " + str(sqlResults[0]["name"]))
            ...
            # logger.log(u"1 Status changes from " + str(self.status) + " to " +
            # str(sqlResults[0]["status"]), logger.DEBUG)
    ...
    def loadFromIndexer(self, season=None, episode=None, cache=True, tvapi=None, cachedSeason=None):
        ...
        try:
            ...
            if self.name:
                logger.log(
                    u"" + sickbeard.indexerApi(self.indexer).name +
                    " timed out but we have enough info from other sources, allowing the error", logger.DEBUG
                )
                return
            else:
                logger.log(
                    u"" + sickbeard.indexerApi(self.indexer).name + " timed out, unable to create the episode",
                    logger.ERROR
                )
                return False
        except (sickbeard.indexer_episodenotfound, sickbeard.indexer_seasonnotfound):
            logger.log(
                u"Unable to find the episode on " + sickbeard.indexerApi(self.indexer).name +
                "... has it been removed? Should I delete from db?", logger.DEBUG
            )
        ...
        if getattr(myEp, 'absolute_number', None) is None:
            logger.log(
                u"This episode (" + self.show.name + " - " + str(season) + "x" +
                str(episode) + ") has no absolute number on " +
        ...
        try:
            ...
            logger.log(
                u"Malformed air date retrieved from " + sickbeard.indexerApi(self.indexer).name +
                " (" + self.show.name + " - " + str(season) + "x" + str(episode) + ")", logger.ERROR
            )
            # if I'm incomplete on TVDB but I once was complete then just delete myself from the DB for now
            if self.indexerid != -1:
                self.deleteEpisode()
        ...
        if not ek.ek(os.path.isfile, self.location):
            ...
            # if we don't have the file and the airdate is in the past
                ...
                # if we somehow are still UNKNOWN then just skip it
                    ...
                    logger.log(
                        u"Not touching status because we have no ep file, the airdate is in the past, "
                        u"and the status is " + str(self.status), logger.DEBUG
                    )
                ...
                for epDetails in showXML.getiterator('episodedetails'):
                    if epDetails.findtext('season') is None or int(epDetails.findtext('season')) != self.season or \
                        ...
                        logger.log(
                            ...
                            u": NFO has an <episodedetails> block for a different episode - wanted " +
                            str(self.season) + "x" + str(self.episode) + " but got " +
                            str(epDetails.findtext('season')) + "x" + str(epDetails.findtext('episode')), logger.DEBUG
                        )
                        continue
        ...
        for cur_name_group in name_groups:
            ...
            for other_ep in self.relatedEps:
                ...
                if multi in (NAMING_LIMITED_EXTEND, NAMING_LIMITED_EXTEND_E_PREFIXED) \
                        and other_ep != self.relatedEps[-1]:
                    continue

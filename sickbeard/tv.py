...
class TVShow(object):
    ...
    def __init__(self, indexer, indexerid, lang=""):
        ...
        if otherShow is not None:
            raise MultipleShowObjectsException(
                "Can't create a show if it already exists")

        self.loadFromDB()

    name = property(lambda self: self._name, dirty_setter("_name"))
    indexerid = property(
        lambda self: self._indexerid,
        dirty_setter("_indexerid"))
    ...
    classification = property(
        lambda self: self._classification,
        dirty_setter("_classification"))
    runtime = property(lambda self: self._runtime, dirty_setter("_runtime"))
    imdb_info = property(
        lambda self: self._imdb_info,
        dirty_setter("_imdb_info"))
    quality = property(lambda self: self._quality, dirty_setter("_quality"))
    flatten_folders = property(
        lambda self: self._flatten_folders,
        dirty_setter("_flatten_folders"))
    status = property(lambda self: self._status, dirty_setter("_status"))
    airs = property(lambda self: self._airs, dirty_setter("_airs"))
    startyear = property(
        lambda self: self._startyear,
        dirty_setter("_startyear"))
    paused = property(lambda self: self._paused, dirty_setter("_paused"))
    air_by_date = property(
        lambda self: self._air_by_date,
        dirty_setter("_air_by_date"))
    subtitles = property(
        lambda self: self._subtitles,
        dirty_setter("_subtitles"))
    dvdorder = property(lambda self: self._dvdorder, dirty_setter("_dvdorder"))
    archive_firstmatch = property(
        lambda self: self._archive_firstmatch,
        dirty_setter("_archive_firstmatch"))
    lang = property(lambda self: self._lang, dirty_setter("_lang"))
    last_update_indexer = property(
        lambda self: self._last_update_indexer,
        dirty_setter("_last_update_indexer"))
    ...
    rls_ignore_words = property(
        lambda self: self._rls_ignore_words,
        dirty_setter("_rls_ignore_words"))
    rls_require_words = property(
        lambda self: self._rls_require_words,
        dirty_setter("_rls_require_words"))
    default_ep_status = property(
        lambda self: self._default_ep_status,
        dirty_setter("_default_ep_status"))
    ...
    def network_logo_name(self):
        return self.network.replace(
            u'\u00C9', 'e').replace(u'\u00E9', 'e').lower()

    def _getLocation(self):
        # no dir check needed if missing show dirs are created during
        # post-processing
        ...
        if ek(os.path.isdir, self._location):
            ...
            raise ShowDirectoryNotFoundException(
                "Show folder doesn't exist, you shouldn't be using it")
    ...
    def getAllEpisodes(self, season=None, has_location=False):
        ...
        sql_selection = sql_selection + \
            " (SELECT COUNT (*) FROM tv_episodes WHERE showid = tve.showid AND season = tve.season AND location != '' AND location = tve.location AND episode != tve.episode) AS share_location "

        sql_selection = sql_selection + \
            " FROM tv_episodes tve WHERE showid = " + str(self.indexerid)
        ...
        for cur_result in results:
            ...
            cur_ep.relatedEps = []
            if cur_ep.location:
                # if there is a location, check if it's a multi-episode
                # (share_location > 0) and put them in relatedEps
                if cur_result["share_location"] > 0:
                    ...
                    for cur_related_ep in related_eps_result:
                        related_ep = self.getEpisode(
                            int(cur_related_ep["season"]), int(cur_related_ep["episode"]))
    ...
    def getEpisode(self, season=None, episode=None, file=None,
                   noCreate=False, absolute_number=None, forceUpdate=False):
        ...
        if self.is_anime and absolute_number and not season and not episode:
            ...
            if len(sqlResults) == 1:
                ...
                logger.log(
                    "No entries for absolute number: " +
                    str(absolute_number) + " in show: " +
                    self.name + " found.",
                    logger.DEBUG)
                return None
        ...
        if not episode in self.episodes[
                season] or self.episodes[season][episode] is None:
            if noCreate:
                return None
    ...
    def should_update(self, update_date=datetime.date.today()):
        ...
        # run logic against the current show latest aired and next unaired data
        # to see if we should bypass 'Ended' status

        graceperiod = datetime.timedelta(days=30)

        last_airdate = datetime.date.fromordinal(1)

        # get latest aired episode to compare against today - graceperiod and
        # today + graceperiod
        myDB = db.DBConnection()
        ...
        if sql_result:
            ...
            if last_airdate >= (
                    update_date - graceperiod) and last_airdate <= (update_date + graceperiod):
                return True

        # get next upcoming UNAIRED episode to compare against today +
        # graceperiod
        sql_result = myDB.select(
    ...
    def writeShowNFO(self):
        ...
        if not ek(os.path.isdir, self._location):
            logger.log(str(self.indexerid) +
                       u": Show dir doesn't exist, skipping NFO generation")
            return False

        logger.log(str(self.indexerid) +
                   u": Writing NFOs for show", logger.DEBUG)
    ...
    def writeEpisodeNFOs(self):
        ...
        logger.log(str(self.indexerid) +
                   u": Writing NFOs for all episodes", logger.DEBUG)
        ...
        for epResult in sqlResults:
            logger.log(str(self.indexerid) +
                       u": Retrieving/creating episode S%02dE%02d" %
                       (epResult["season"] or 0, epResult["episode"] or 0), logger.DEBUG)
    ...
    def updateShowNFO(self):
        ...
        logger.log(str(self.indexerid) +
                   u": Updating NFOs for show with new indexer info")
        for cur_provider in sickbeard.metadata_provider_dict.values():
            result = cur_provider.update_show_indexer_metadata(self) or result

        return result

    # find all media files in the show folder and create episodes for as many
    # as possible
    def loadEpisodesFromDir(self):

        if not ek(os.path.isdir, self._location):
            logger.log(str(self.indexerid) +
                       u": Show dir doesn't exist, not loading episodes from disk", logger.DEBUG)
            return

        logger.log(str(self.indexerid) +
                   u": Loading all episodes from the show directory " +
                   self._location, logger.DEBUG)
        ...
        for mediaFile in mediaFiles:
            ...
            logger.log(str(self.indexerid) +
                       u": Creating episode from " +
                       mediaFile, logger.DEBUG)
            try:
                curEpisode = self.makeEpFromFile(
                    ek(os.path.join, self._location, mediaFile))
                ...
                logger.log(
                    u"The episode deleted itself when I tried making an object for it",
                    logger.DEBUG)
            ...
            if not ' ' in ep_file_name and parse_result and parse_result.release_group:
                logger.log(
                    u"Name " + ep_file_name + u" gave release group of " +
                    parse_result.release_group + ", seems valid",
                    logger.DEBUG)
            ...
            if curEpisode is not None:
                if self.subtitles:
                    try:
                        ...
                        logger.log(
                            "%s: Could not refresh subtitles" %
                            self.indexerid, logger.ERROR)
    ...
    def loadEpisodesFromDB(self):
        ...
        for curResult in sqlResults:
            ...
            logger.log(
                u"%s: loading Episodes from DB" %
                curShowid, logger.DEBUG)
            deleteEp = False
            ...
            logger.log(
                u"%s: Loading episode S%02dE%02d from the DB" %
                (curShowid, curSeason or 0, curEpisode or 0), logger.DEBUG)
            ...
            try:
                ...
                # if we found out that the ep is no longer on TVDB then delete
                # it from our database too
                if deleteEp:
                    curEp.deleteEpisode()

                curEp.loadFromDB(curSeason, curEpisode)
                curEp.loadFromIndexer(
                    tvapi=t, cachedSeason=cachedSeasons[curSeason])
    ...
    def loadEpisodesFromIndexer(self, cache=True):
        ...
        for season in showObj:
            ...
            for episode in showObj[season]:
                # need some examples of wtf episode 0 means to decide if we
                # want it or not
                if episode == 0:
                    continue
                try:
                    ...
                    logger.log(
                        ...
                        (self.indexerid, sickbeard.indexerApi(
                            self.indexer).name, season or 0, episode or 0))
                    continue
                else:
                    try:
                        ...
                        logger.log(
                            u"The episode was deleted, skipping the rest of the load")
                        continue
    ...
    def getImages(self, fanart=None, poster=None):
        ...
        for cur_provider in sickbeard.metadata_provider_dict.values():
            ...
            season_posters_result = cur_provider.create_season_posters(
                self) or season_posters_result
            season_banners_result = cur_provider.create_season_banners(
                self) or season_banners_result
            season_all_poster_result = cur_provider.create_season_all_poster(
                self) or season_all_poster_result
            season_all_banner_result = cur_provider.create_season_all_banner(
                self) or season_all_banner_result
    ...
    def makeEpFromFile(self, file):
        ...
        if not ek(os.path.isfile, file):
            logger.log(str(self.indexerid) +
                       u": That isn't even a real file dude... " + file)
            return None

        logger.log(str(self.indexerid) +
                   u": Creating episode object from " +
                   file, logger.DEBUG)
        ...
        if not len(parse_result.episode_numbers):
            ...
            logger.log(
                u"No episode number found in " +
                file +
                ", ignoring it",
                logger.WARNING)
            return None

        # for now lets assume that any episode in the show dir belongs to that
        # show
        ...
        for curEpNum in episodes:
            ...
            if not curEp:
                ...
                # if there is a new file associated with this ep then re-check
                # the quality
                if curEp.location and ek(os.path.normpath, curEp.location) != ek(
                ...
                with curEp.lock:
                    ...
                    curEp.location = file
                    # if the sizes are the same then it's probably the same
                    # file
            ...
            # if they replace a file on me I'll make some attempt at
            # re-checking the quality unless I know it's the same file
            ...
            # check for status/quality changes as long as it's a new file
                ...
                # if it was snatched and now exists then set the status
                # correctly
                ...
                # if it was snatched proper and we found a higher quality one
                # then allow the status change
    ...
    def loadFromDB(self, skipNFO=False):
        ...
        logger.log(str(self.indexerid) +
                   u": Loading show info from database", logger.DEBUG)
        ...
        if len(sqlResults) > 1:
            ...
            logger.log(str(self.indexerid) +
                       ": Unable to find the show in the database")
            return
        else:
            ...
            self.archive_firstmatch = int(
                sqlResults[0]["archive_firstmatch"] or 0)
            ...
            self.default_ep_status = int(
                sqlResults[0]["default_ep_status"] or SKIPPED)
        ...
        if len(sqlResults) == 0:
            logger.log(str(self.indexerid) +
                       ": Unable to find IMDb show info in the database")
            return
        else:
    ...
    def loadFromIndexer(self, cache=True, tvapi=None, cachedSeason=None):
        ...
        if self.indexer is not INDEXER_TVRAGE:
            ...
            if getattr(myEp, 'airs_dayofweek', None) is not None and getattr(
                    myEp, 'airs_time', None) is not None:
            ...
            logger.log(str(self.indexerid) +
                       u": NOT loading info from " +
                       sickbeard.indexerApi(self.indexer).name +
                       " as it is temporarily disabled.", logger.WARNING)
    ...
    def loadIMDbInfo(self, imdbapi=None):
        ...
        if self.imdbid:
            logger.log(str(self.indexerid) +
                       u": Loading show info from IMDb", logger.DEBUG)

            imdbTv = i.get_movie(str(re.sub(r"[^0-9]", "", self.imdbid)))

            for key in [x for x in imdb_info.keys() if x.replace(
                    '_', ' ') in imdbTv.keys()]:
                # Store only the first value for string type
                if isinstance(imdb_info[key], basestring) and isinstance(
                        imdbTv.get(key.replace('_', ' ')), list):
            ...
            if imdb_info['runtimes']:
                imdb_info['runtimes'] = re.search(
                    r'\d+', imdb_info['runtimes']).group(0)
            else:
            ...
            logger.log(str(self.indexerid) +
                       u": Obtained info from IMDb ->" +
                       str(self.imdb_info), logger.DEBUG)

    def nextEpisode(self):
        logger.log(str(self.indexerid) +
                   ": Finding the episode which airs next", logger.DEBUG)
        ...
        if not self.nextaired or self.nextaired and curDate > self.nextaired:
            ...
            if sqlResults is None or len(sqlResults) == 0:
                logger.log(str(self.indexerid) +
                           u": No episode found... need to implement a show status", logger.DEBUG)
                self.nextaired = ""
            else:
                logger.log(
                    u"%s: Found episode S%02dE%02d" %
                    (self.indexerid,
                     sqlResults[0]["season"] or 0,
                        sqlResults[0]["episode"] or 0),
                    logger.DEBUG)
    ...
    def deleteShow(self, full=False):
        ...
        sickbeard.showList = [
            x for x in sickbeard.showList if int(
                x.indexerid) != self.indexerid]

        # clear the cache
        image_cache_dir = ek(os.path.join, sickbeard.CACHE_DIR, 'images')
        for cache_file in ek(glob.glob, ek(
                os.path.join, image_cache_dir, str(self.indexerid) + '.*')):
            ...
            try:
                ...
                logger.log(
                    u'Unable to %s %s: %s / %s' %
                    (action, cache_file, repr(e), str(e)), logger.WARNING)

        # remove entire show folder
        if full:
            try:
                logger.log(
                    u'Attempt to %s show folder %s' %
                ...
                if not file_attribute & stat.S_IWRITE:
                    ...
                    logger.log(
                        'Attempting to make writeable the read only folder %s' %
                        self._location, logger.DEBUG)
                    try:
                        ek(os.chmod, self.location, stat.S_IWRITE)
                    except Exception:
                        logger.log(
                            u'Unable to change permissions of %s' %
                            self._location, logger.WARNING)
                ...
                logger.log(
                    u"Show folder does not exist, no need to %s %s" %
                    (action, self._location), logger.WARNING)
            except OSError as e:
                logger.log(
                    u'Unable to %s %s: %s / %s' %
                    (action, self._location, repr(e), str(e)), logger.WARNING)

        if sickbeard.USE_TRAKT and sickbeard.TRAKT_SYNC_WATCHLIST:
            logger.log(u"Removing show: indexerid " +
                       str(self.indexerid) +
                       ", Title " +
                       str(self.name) +
                       " from Watchlist", logger.DEBUG)
    ...
    def populateCache(self):
        ...
        logger.log(
            u"Checking & filling cache for show " +
            self.name,
            logger.DEBUG)
    ...
    def refreshDir(self):
        ...
        # make sure the show dir is where we think it is unless dirs are
        # created on the fly
        if not ek(os.path.isdir,
                  self._location) and not sickbeard.CREATE_MISSING_SHOW_DIRS:
            return False
        ...
        logger.log(str(self.indexerid) +
                   u": Loading all episodes with a location from the database", logger.DEBUG)
        ...
        for ep in sqlResults:
            ...
            if not ek(os.path.isfile, curLoc) or not os.path.normpath(curLoc).startswith(
                ...
                # check if downloaded files still exist, update our data if
                # this has changed
                if not sickbeard.SKIP_REMOVED_FILES:
                    with curEp.lock:
                        # if it used to have a file associated with it and it
                        # doesn't anymore then set it to
                        # sickbeard.EP_DEFAULT_DELETED_STATUS
                        if curEp.location and curEp.status in Quality.DOWNLOADED:

                            if sickbeard.EP_DEFAULT_DELETED_STATUS == ARCHIVED:
                                _, oldQuality = Quality.splitCompositeStatus(
                                    curEp.status)
                                new_status = Quality.compositeStatus(
                                    ARCHIVED, oldQuality)
                            else:
                            ...
                            curEp.subtitles_lastsearch = str(
    ...
    def downloadSubtitles(self, force=False):
        ...
        if not ek(os.path.isdir, self._location):
            logger.log(str(self.indexerid) +
                       ": Show dir doesn't exist, can't download subtitles", logger.DEBUG)
            return
        ...
        try:
            ...
            if not episodes:
                logger.log(
                    "%s: No episodes to download subtitles for %s" %
    ...
    def saveToDB(self, forceSave=False):
        ...
        if not self.dirty and not forceSave:
            logger.log(str(self.indexerid) +
                       ": Not saving show to db - record is not dirty", logger.DEBUG)
            return

        logger.log(str(self.indexerid) +
                   u": Saving show info to database", logger.DEBUG)
    ...
    def wantEpisode(self, season, episode, quality,
                    manualSearch=False, downCurQuality=False):

        logger.log(
            u"Checking if found episode %s S%02dE%02d is wanted at quality %s" %
            (self.name,
             season or 0,
             episode or 0,
        ...
        # if the quality isn't one we want under any circumstances then just
        # say no
        ...
        if quality not in anyQualities + bestQualities or quality is UNKNOWN:
            logger.log(
                u"Don't want this quality, ignoring found episode",
                logger.DEBUG)
            return False
        ...
        if not sqlResults or not len(sqlResults):
            logger.log(
                u"Unable to find a matching episode in database, ignoring found episode",
                logger.DEBUG)
            return False
        ...
        logger.log(u"Existing episode status: " + str(epStatus) +
                   " (" + epStatus_text + ")", logger.DEBUG)

        # if we know we don't want it then just say no
        if epStatus in Quality.ARCHIVED + \
                [UNAIRED, SKIPPED, IGNORED] and not manualSearch:
            logger.log(
                u"Existing episode status is unaired/skipped/ignored/archived, ignoring found episode",
                logger.DEBUG)
            return False

        curStatus, curQuality = Quality.splitCompositeStatus(epStatus)

        # if it's one of these then we want it as long as it's in our allowed
        # initial qualities
        if epStatus in (WANTED, SKIPPED, UNKNOWN):
            logger.log(
                u"Existing episode status is wanted/skipped/unknown, getting found episode",
                logger.DEBUG)
            return True
        elif manualSearch:
            if (downCurQuality and quality >= curQuality) or (
                    not downCurQuality and quality > curQuality):
                logger.log(
        ...
        # bestQualities list and better than what we have, or we only have one
        # bestQuality and we do not have that quality yet
        if epStatus in Quality.DOWNLOADED + Quality.SNATCHED + \
                Quality.SNATCHED_PROPER and quality in bestQualities and (quality > curQuality or curQuality not in bestQualities):
        ...
        logger.log(
            u"None of the conditions were met, ignoring found episode",
            logger.DEBUG)
        return False
    ...
    def getOverview(self, epStatus):
        ...
        if epStatus == WANTED:
            ...
            _, bestQualities = Quality.splitQuality(
                self.quality)  # @UnusedVariable
            if bestQualities:
            ...
            # if they don't want re-downloads then we call it good if they have
            # anything
            ...
            # if the want only first match and already have one call it good
                ...
                return Overview.GOOD
            # if they want only first match and current quality is higher than
            # minimal best quality call it good
            elif self.archive_firstmatch and minBestQuality is not None and curQuality > minBestQuality:
                return Overview.GOOD
            # if they have one but it's not the best they want then mark it as
            # qual
...
class TVEpisode(object):
    ...
    absolute_number = property(
        lambda self: self._absolute_number,
        dirty_setter("_absolute_number"))
    description = property(
        lambda self: self._description,
        dirty_setter("_description"))
    subtitles = property(
        lambda self: self._subtitles,
        dirty_setter("_subtitles"))
    subtitles_searchcount = property(
        lambda self: self._subtitles_searchcount,
        dirty_setter("_subtitles_searchcount"))
    subtitles_lastsearch = property(
        lambda self: self._subtitles_lastsearch,
        dirty_setter("_subtitles_lastsearch"))
    ...
    # location = property(lambda self: self._location, dirty_setter("_location"))
    file_size = property(
        lambda self: self._file_size,
        dirty_setter("_file_size"))
    release_name = property(
        lambda self: self._release_name,
        dirty_setter("_release_name"))
    is_proper = property(
        lambda self: self._is_proper,
        dirty_setter("_is_proper"))
    version = property(lambda self: self._version, dirty_setter("_version"))
    release_group = property(
        lambda self: self._release_group,
        dirty_setter("_release_group"))
    ...
    def refreshSubtitles(self):
        ...
        self.subtitles, save_subtitles = subtitles.subtitlesLanguages(
            self.location)
        if save_subtitles:
            self.saveToDB()
    ...
    def downloadSubtitles(self, force=False):
        ...
        logger.log(
            u"%s: Downloading subtitles for S%02dE%02d" %
            (self.show.indexerid,
             self.season or 0,
             self.episode or 0),
            logger.DEBUG)
        ...
        self.subtitles, newSubtitles = subtitles.downloadSubtitles(
            subtitles_info)
        ...
        if newSubtitles:
            subtitleList = ", ".join(
                [subtitles.fromietf(newSub).name for newSub in newSubtitles])
        ...
        if not sqlResult:
            ...
            if ek(os.path.isfile, self.location):
                try:
                    ...
                    logger.log(
                        u"%s: There was an error loading the NFO for episode S%02dE%02d" %
                        (self.show.indexerid, season or 0, episode or 0), logger.ERROR)

                # if we tried loading it from NFO and didn't find the NFO, try
                # the Indexers
                if not self.hasnfo:
                    try:
                    ...
                    if not result:
                        raise EpisodeNotFoundException(
                            "Couldn't find episode S%02dE%02d" %
                            (season or 0, episode or 0))

    def loadFromDB(self, season, episode):
        logger.log(u"%s: Loading episode details from DB for episode %s S%02dE%02d" % (
            self.show.indexerid, self.show.name, season or 0, episode or 0), logger.DEBUG)
        ...
        if len(sqlResults) > 1:
            raise MultipleEpisodesInDatabaseException(
                "Your DB has two records for the same show somehow.")
        elif len(sqlResults) == 0:
            logger.log(
                u"%s: Episode S%02dE%02d not found in the database" %
                (self.show.indexerid, self.season or 0, self.episode or 0), logger.DEBUG)
            return False
        else:
            # NAMEIT logger.log(u"AAAAA from" +
            # str(self.season)+"x"+str(self.episode) + " -" + self.name + " to
            # " + str(sqlResults[0]["name"]))
            ...
            self.airdate = datetime.date.fromordinal(
            ...
            self.scene_season = helpers.tryInt(
                sqlResults[0]["scene_season"], 0)
            self.scene_episode = helpers.tryInt(
                sqlResults[0]["scene_episode"], 0)
            self.scene_absolute_number = helpers.tryInt(
                sqlResults[0]["scene_absolute_number"], 0)
    ...
    def loadFromIndexer(self, season=None, episode=None,
                        cache=True, tvapi=None, cachedSeason=None):
        ...
        try:
            ...
            # if I'm no longer on the Indexers but I once was then delete
            # myself from the DB
            if self.indexerid != -1:
                self.deleteEpisode()
            return
        ...
        if getattr(myEp, 'episodename', None) is None:
            logger.log(
                u"This episode %s - S%02dE%02d has no name on %s. Setting to an empty string" %
                (self.show.name,
                 season or 0,
                 episode or 0,
                 sickbeard.indexerApi(
        ...
        if getattr(myEp, 'absolute_number', None) is None:
            logger.log(
                u"This episode %s - S%02dE%02d has no absolute number on %s" %
                (self.show.name,
                 season or 0,
                 episode or 0,
                 sickbeard.indexerApi(
            ...
            logger.log(
                u"%s: The absolute_number for S%02dE%02d is: %s " %
                (self.show.indexerid,
                 season or 0,
                 episode or 0,
                 myEp["absolute_number"]),
                logger.DEBUG)
        ...
        try:
            self.airdate = datetime.date(
                rawAirdate[0], rawAirdate[1], rawAirdate[2])
        except (ValueError, IndexError):
            logger.log(
                u"Malformed air date of %s retrieved from %s for (%s - S%02dE%02d)" %
                (firstaired,
                 sickbeard.indexerApi(
                     self.indexer).name,
                    self.show.name,
                    ...
                    episode or 0),
                logger.WARNING)
            # if I'm incomplete on the indexer but I once was complete then
            # just delete myself from the DB for now
            if self.indexerid != -1:
                self.deleteEpisode()
            return False
        ...
        # don't update show status if show dir is missing, unless it's missing
        # on purpose
        if not ek(os.path.isdir,
                  self.show._location) and not sickbeard.CREATE_MISSING_SHOW_DIRS and not sickbeard.ADD_SHOWS_WO_DIR:
            logger.log(
                u"The show dir %s is missing, not bothering to change the episode statuses since it'd probably be invalid" %
                self.show._location)
            return
        ...
        if not ek(os.path.isfile, self.location):
            ...
            ) or self.airdate == datetime.date.fromordinal(1):
                logger.log(
                    u"Episode airs in the future or has no airdate, marking it %s" %
                    statusStrings[UNAIRED], logger.DEBUG)
                self.status = UNAIRED
            elif self.status in [UNAIRED, UNKNOWN]:
                # Only do UNAIRED/UNKNOWN, it could already be
                # snatched/ignored/skipped, or downloaded/archived to
                # disconnected media
                logger.log(
                    u"Episode has already aired, marking it %s" %
                    statusStrings[
                        self.show.default_ep_status],
                    logger.DEBUG)
                # auto-skip specials
                self.status = self.show.default_ep_status if self.season > 0 else SKIPPED
            else:
                logger.log(
                    u"Not touching status [ %s ] It could be skipped/ignored/snatched/archived" %
                    statusStrings[
                        self.status],
                    logger.DEBUG)
        ...
        # if we have a media file then it's downloaded
            ...
            # leave propers alone, you have to either post-process them or
            # manually change them back
            if self.status not in Quality.SNATCHED_PROPER + \
                    Quality.DOWNLOADED + Quality.SNATCHED + Quality.ARCHIVED:
                logger.log(
                    u"5 Status changes from " +
                    str(self.status) + " to " +
                    str(Quality.statusFromName(self.location)),
                    logger.DEBUG)
                self.status = Quality.statusFromName(
                    self.location, anime=self.show.is_anime)

        # shouldn't get here probably
        else:
            logger.log(u"6 Status changes from " + str(self.status) +
                       " to " + str(UNKNOWN), logger.DEBUG)
            self.status = UNKNOWN
    ...
    def loadFromNFO(self, location):
        ...
        logger.log(
            str(self.show.indexerid) +
            u": Loading episode details from the NFO file associated with " + location,
            logger.DEBUG)
        ...
        if self.location != "":
            ...
            logger.log(str(self.show.indexerid) +
                       u": Using NFO name " + nfoFile, logger.DEBUG)
            ...
            if ek(os.path.isfile, nfoFile):
                try:
                    ...
                    logger.log(
                        u"Error loading the NFO, backing up the NFO and skipping for now: " +
                        ex(e),
                        logger.ERROR)
                    try:
                ...
                for epDetails in showXML.getiterator('episodedetails'):
                    ...
                    if epDetails.findtext(
                            'title') is None or epDetails.findtext('aired') is None:
                        raise NoNFOException(
                            "Error in NFO format (missing episode title or airdate)")
                    ...
                    if epDetails.findtext('aired'):
                        rawAirdate = [
                            int(x) for x in epDetails.findtext('aired').split("-")]
                        self.airdate = datetime.date(
                            rawAirdate[0], rawAirdate[1], rawAirdate[2])
                    else:
    ...
    def __str__(self):
        ...
        toReturn = ""
        toReturn += "%s - S%02dE%02d - %s " % (
            self.show.name, self.season or 0, self.episode or 0, self.name) + "\n"
        toReturn += "location: " + str(self.location) + "\n"
        toReturn += "description: " + str(self.description) + "\n"
        toReturn += "subtitles: " + str(",".join(self.subtitles)) + "\n"
        toReturn += "subtitles_searchcount: " + \
            str(self.subtitles_searchcount) + "\n"
        toReturn += "subtitles_lastsearch: " + \
            str(self.subtitles_lastsearch) + "\n"
        toReturn += "airdate: " + \
            str(self.airdate.toordinal()) + " (" + str(self.airdate) + ")\n"
    ...
    def createMetaFiles(self):
        ...
        if not ek(os.path.isdir, self.show._location):
            logger.log(str(self.show.indexerid) +
                       u": The show dir is missing, not bothering to try to create metadata")
            return
    ...
    def deleteEpisode(self):
        ...
        logger.log(
            u"Deleting %s S%02dE%02d from the DB" %
            (self.show.name,
             self.season or 0,
             self.episode or 0),
            logger.DEBUG)
        ...
        if self.show.getEpisode(self.season, self.episode,
                                noCreate=True) == self:
    ...
    def get_sql(self, forceSave=False):
        ...
        try:
            if not self.dirty and not forceSave:
                logger.log(str(self.show.indexerid) +
                           u": Not creating SQL queue - record is not dirty", logger.DEBUG)
                return
            ...
            if epID:
                ...
                if sickbeard.SUBTITLES_MULTI or not rows[
                        0]['subtitles'] or not self.subtitles:
                    return [
                    ...
                    # Don't update the subtitle language when the srt file
                    # doesn't contain the alpha2 code, keep value from
                    # subliminal
                    return [
                ...
                return [
                    ...
                    [self.show.indexerid, self.season, self.episode, self.indexerid, self.indexer, self.name,
                     self.description, ",".join(
                         self.subtitles), self.subtitles_searchcount, self.subtitles_lastsearch,
    ...
    def saveToDB(self, forceSave=False):
        ...
        if not self.dirty and not forceSave:
            logger.log(str(self.show.indexerid) +
                       u": Not saving episode to db - record is not dirty", logger.DEBUG)
            return

        logger.log(str(self.show.indexerid) +
                   u": Saving episode details to database", logger.DEBUG)
    ...
    def _replace_map(self):
        ...
        def release_group(show, name):
            ...
            try:
                ...
                logger.log(
                    u"Unable to get parse release_group: " +
                    ex(e),
                    logger.DEBUG)
                return ''
        ...
        _, epQual = Quality.splitCompositeStatus(
            self.status)  # @UnusedVariable
        ...
        if hasattr(self, 'release_name'):  # from the release name field in db
            rel_grp['release_name'] = release_group(
                self.show, self.release_name)
    ...
    def _format_string(self, pattern, replace_map):
        ...
        for cur_replacement in sorted(replace_map.keys(), reverse=True):
            result_name = result_name.replace(
                cur_replacement, helpers.sanitizeFileName(
    ...
    def _format_pattern(self, pattern=None, multi=None, anime_type=None):
        ...
        # if there's no release group in the db, let the user know we replaced
        # it
        if replace_map['%RG'] and replace_map['%RG'] != 'SiCKRAGE':
            if not hasattr(self, '_release_group'):
                logger.log(
                    u"Episode has no release group, replacing it with '" +
                    replace_map['%RG'] +
                    "'",
                    logger.DEBUG)
                # if release_group is not in the db, put it there
                self._release_group = replace_map['%RG']
        ...
        # if there's no release name then replace it with a reasonable
        # facsimile
        if not replace_map['%RN']:

            if self.show.air_by_date or self.show.sports:
                result_name = result_name.replace(
                    '%RN', '%S.N.%A.D.%E.N-' + replace_map['%RG'])
                result_name = result_name.replace(
                    '%rn', '%s.n.%A.D.%e.n-' + replace_map['%RG'].lower())

            elif anime_type != 3:
                result_name = result_name.replace(
                    '%RN', '%S.N.%AB.%E.N-' + replace_map['%RG'])
                result_name = result_name.replace(
                    '%rn', '%s.n.%ab.%e.n-' + replace_map['%RG'].lower())

            else:
                result_name = result_name.replace(
                    '%RN', '%S.N.S%0SE%0E.%E.N-' + replace_map['%RG'])
                result_name = result_name.replace(
                    '%rn', '%s.n.s%0se%0e.%e.n-' + replace_map['%RG'].lower())
        ...
        # figure out the double-ep numbering style for each group, if
        # applicable
        for cur_name_group in name_groups:
            ...
            season_ep_match = re.search(
                season_ep_regex, cur_name_group, re.I | re.X)
            ep_only_match = re.search(
                ep_only_regex, cur_name_group, re.I | re.X)
            ...
            # if there's no season then there's not much choice so we'll just
            # force them to use 03-04-05 style
            elif ep_only_match:
                season_format = ''
                ep_sep = '-'
            ...
            for other_ep in self.relatedEps:
                ...
                if multi in (NAMING_LIMITED_EXTEND,
                             NAMING_LIMITED_EXTEND_E_PREFIXED) and other_ep != self.relatedEps[-1]:
                    continue
                ...
                ep_string += other_ep._format_string(
                    ep_format.upper(), other_ep._replace_map())
            ...
            if anime_type != 3:
                ...
                if self.season != 0:  # dont set absolute numbers if we are on specials !
                    # this crazy person wants both ! (note: +=)
                    if anime_type == 1:
                        ep_string += sep + "%(#)03d" % {
                            "#": curAbsolute_number}
                    # total anime freak only need the absolute number ! (note:
                    # =)
                    elif anime_type == 2:
            ...
            if regex_replacement:
                # fill out the template for this piece and then insert this
                # piece into the actual pattern
                cur_name_group_result = re.sub(
                    '(?i)(?x)' + regex_used, regex_replacement, cur_name_group)
                ...
                result_name = result_name.replace(
                    cur_name_group, cur_name_group_result)

        result_name = self._format_string(result_name, replace_map)

        logger.log(
            u"formatting pattern: " +
            pattern +
            " -> " +
            result_name,
            logger.DEBUG)
    ...
    def proper_path(self):
        ...
        # if they want us to flatten it and we're allowed to flatten it then we
        # will
    ...
    def formatted_dir(self, pattern=None, multi=None):
        ...
        if pattern is None:
            # we only use ABD if it's enabled, this is an ABD show, AND this is
            # not a multi-ep
    ...
    def formatted_filename(self, pattern=None, multi=None, anime_type=None):
        ...
        return helpers.sanitizeFileName(
            self._format_pattern(name_groups[-1], multi, anime_type))
    ...
    def rename(self):
        ...
        if not ek(os.path.isfile, self.location):
            logger.log(
                u"Can't perform rename on " +
                self.location +
                " when it doesn't exist, skipping",
                logger.WARNING)
            return

        proper_path = self.proper_path()
        absolute_proper_path = ek(
            os.path.join,
            self.show.location,
            proper_path)
        absolute_current_path_no_ext, file_ext = ek(
            os.path.splitext, self.location)
        ...
        if self.show.subtitles and sickbeard.SUBTITLES_DIR != '':
            ...
            absolute_proper_subs_path = ek(
                os.path.join,
                sickbeard.SUBTITLES_DIR,
                self.formatted_filename())

        logger.log(
            u"Files associated to " +
            self.location +
            ": " +
            str(related_files),
            logger.DEBUG)
        ...
        # move the ep file
        result = helpers.rename_ep_file(
            self.location,
            absolute_proper_path,
            absolute_current_path_no_ext_length)

        # move related files
        for cur_related_file in related_files:
            # We need to fix something here because related files can be in
            # subfolders and the original code doesn't handle this (at all)
            cur_related_dir = ek(
                os.path.dirname, ek(
                    os.path.abspath, cur_related_file))
            subfolder = cur_related_dir.replace(
                ek(os.path.dirname, ek(os.path.abspath, self.location)), '')
            # We now have a subfolder. We need to add that to the absolute_proper_path.
            # First get the absolute proper-path dir
            proper_related_dir = ek(
                os.path.dirname, ek(
                    os.path.abspath, absolute_proper_path + file_ext))
            proper_related_path = absolute_proper_path.replace(
                proper_related_dir, proper_related_dir + subfolder)

            cur_result = helpers.rename_ep_file(cur_related_file, proper_related_path,
                                                absolute_current_path_no_ext_length + len(subfolder))
            if not cur_result:
                logger.log(str(self.indexerid) +
                           u": Unable to rename file " +
                           cur_related_file, logger.ERROR)
    ...
    def airdateModifyStamp(self):
        ...
        airdatetime = network_timezones.parse_date_time(
            airdate_ordinal, self.show.airs, self.show.network)

...
class TVShow(object):
    ...
    def getEpisode(self, season=None, episode=None, file=None, noCreate=False, absolute_number=None, forceUpdate=False):
        ...
        if self.is_anime and absolute_number and not season and not episode:
            myDB = db.DBConnection()
            sql = "SELECT episode, season FROM tv_episodes WHERE showid = ? AND absolute_number = ? AND season != 0"
    ...
    def should_update(self, update_date=datetime.date.today()):
        ...
        sql_result = myDB.select(
            "SELECT airdate FROM tv_episodes WHERE showid = ? AND season > '0' AND airdate > '1' AND status > '1' ORDER BY airdate DESC LIMIT 1",
            [self.indexerid])
        ...
        sql_result = myDB.select(
            "SELECT airdate FROM tv_episodes WHERE showid = ? AND season > '0' AND airdate > '1' AND status = '1' ORDER BY airdate ASC LIMIT 1",
            [self.indexerid])
    ...
    def loadEpisodesFromDB(self):
        ...
        myDB = db.DBConnection()
        sql = "SELECT season, episode FROM tv_episodes WHERE showid = ?"
    ...
    def refreshDir(self):
        ...
        myDB = db.DBConnection()
        sqlResults = myDB.select("SELECT location, season, episode FROM tv_episodes WHERE showid = ? AND location != ''", [self.indexerid])

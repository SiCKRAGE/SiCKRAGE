...
class TVShow(object):
    ...
    def getAllEpisodes(self, season=None, has_location=False):
        ...
        for cur_result in results:
            cur_ep = self.getEpisode(
                int(cur_result[b"season"]), int(cur_result[b"episode"]))
            if not cur_ep:
                continue
    ...
    def writeShowNFO(self):
        ...
        if not ek(os.path.isdir, self._location):
            logging.info(str(self.indexerid) +
                         ": Show dir doesn't exist, skipping NFO generation")
            return False
    ...
    def updateShowNFO(self):
        ...
        logging.info(str(self.indexerid) +
                     ": Updating NFOs for show with new indexer info")
        for cur_provider in sickbeard.metadata_provider_dict.values():
    ...
    def loadEpisodesFromDir(self):
        ...
        if not ek(os.path.isdir, self._location):
            logging.debug(str(self.indexerid) +
                          ": Show dir doesn't exist, not loading episodes from disk")
            return

        logging.debug(str(self.indexerid) +
                      ": Loading all episodes from the show directory " +
                      self._location)
        ...
        for mediaFile in mediaFiles:
            ...
            logging.debug(str(self.indexerid) +
                          ": Creating episode from " + mediaFile)
            try:
                ...
                logging.error(
                    "Episode " +
                    mediaFile +
                    " returned an exception: {}".format(
                        ex(e)))
                continue
            except EpisodeDeletedException:
                logging.debug(
                    "The episode deleted itself when I tried making an object for it")
    ...
    def loadEpisodesFromDB(self):
        ...
        for curResult in sqlResults:
            ...
            logging.debug(
                "%s: Loading episode S%02dE%02d from the DB" %
                (curShowid, curSeason or 0, curEpisode or 0))
            ...
            try:
                ...
                logging.debug(
                    "Tried loading an episode from the DB that should have been deleted, skipping it")
                continue
            ...
            for episode in showObj[season]:
                ...
                try:
                    ...
                    try:
                        ...
                        logging.info(
                            "The episode was deleted, skipping the rest of the load")
                        continue
    ...
    def makeEpFromFile(self, file):
        ...
        if not ek(os.path.isfile, file):
            logging.info(str(self.indexerid) +
                         ": That isn't even a real file dude... " + file)
            return None

        logging.debug(str(self.indexerid) +
                      ": Creating episode object from " + file)
        ...
        for curEpNum in episodes:
            ...
            logging.debug(
                ...
                (self.indexerid, file, self.name, season or 0, episode or 0))
            ...
            if not curEp:
                try:
                    ...
                    logging.error(str(self.indexerid) +
    ...
    def loadFromDB(self, skipNFO=False):
        ...
        logging.debug(str(self.indexerid) +
                      ": Loading show info from database")
        ...
        if len(sqlResults) > 1:
            ...
            self.archive_firstmatch = int(
                sqlResults[0][b"archive_firstmatch"] or 0)
            ...
            self.default_ep_status = int(
                sqlResults[0][b"default_ep_status"] or SKIPPED)
    ...
    def loadIMDbInfo(self, imdbapi=None):
        ...
        if self.imdbid:
            ...
            if imdb_info[b'runtimes']:
                imdb_info[b'runtimes'] = re.search(
                    r'\d+', imdb_info[b'runtimes']).group(0)
            else:
            ...
            if imdb_info[b'country_codes']:
                imdb_info[b'country_codes'] = '|'.join(
            ...
            logging.debug(str(self.indexerid) +
                          ": Obtained info from IMDb ->" +
                          str(self.imdb_info))

    def nextEpisode(self):
        logging.debug(str(self.indexerid) +
                      ": Finding the episode which airs next")
        ...
        if not self.nextaired or self.nextaired and curDate > self.nextaired:
            ...
            if sqlResults is None or len(sqlResults) == 0:
                logging.debug(str(self.indexerid) +
                              ": No episode found... need to implement a show status")
                self.nextaired = ""
            else:
    ...
    def deleteShow(self, full=False):
        ...
        for cache_file in ek(glob.glob, ek(
            ...
            try:
                ...
                logging.warning(
                    'Unable to %s %s: %s / %s' %
                    (action, cache_file, repr(e), str(e)))

        # remove entire show folder
        if full:
            try:
                logging.info(
                    'Attempt to %s show folder %s' %
                ...
                logging.warning(
                    "Show folder does not exist, no need to %s %s" %
                    (action, self._location))
            except OSError as e:
                logging.warning(
                    'Unable to %s %s: %s / %s' %
                    (action, self._location, repr(e), str(e)))
    ...
    def downloadSubtitles(self, force=False):
        ...
        if not ek(os.path.isdir, self._location):
            logging.debug(str(self.indexerid) +
                          ": Show dir doesn't exist, can't download subtitles")
            return
    ...
    def wantEpisode(self, season, episode, quality,
        ...
        logging.debug("Existing episode status: " +
                      str(epStatus) + " (" + epStatus_text + ")")
        ...
        if epStatus in (WANTED, SKIPPED, UNKNOWN):
            logging.debug(
                "Existing episode status is wanted/skipped/unknown, getting found episode")
            return True
        elif manualSearch:
...
class TVEpisode(object):
    ...
    def loadFromDB(self, season, episode):
        ...
        if len(sqlResults) > 1:
            ...
            # NAMEIT logging.info(u"AAAAA from" +
            # str(self.season)+"x"+str(self.episode) + " -" + self.name + " to
            # " + str(sqlResults[0][b"name"]))
            ...
            self.scene_season = helpers.tryInt(
                sqlResults[0][b"scene_season"], 0)
            self.scene_episode = helpers.tryInt(
                sqlResults[0][b"scene_episode"], 0)
            self.scene_absolute_number = helpers.tryInt(
                sqlResults[0][b"scene_absolute_number"], 0)
    ...
    def loadFromIndexer(self, season=None, episode=None,
        ...
        if not ek(os.path.isfile, self.location):
            if self.airdate >= datetime.date.today(
                ...
                logging.debug(
                    "Episode airs in the future or has no airdate, marking it %s" %
                    statusStrings[UNAIRED])
                self.status = UNAIRED
                ...
                # disconnected media
                logging.debug(
                    "Episode has already aired, marking it %s" %
                    statusStrings[
        ...
        # shouldn't get here probably
            ...
            logging.debug("6 Status changes from " +
                          str(self.status) + " to " + str(UNKNOWN))
            self.status = UNKNOWN
    ...
    def loadFromNFO(self, location):
        ...
        if self.location != "":
            ...
            logging.debug(str(self.show.indexerid) +
                          ": Using NFO name " + nfoFile)
            ...
            if ek(os.path.isfile, nfoFile):
                try:
                    ...
                    logging.error(
                        "Error loading the NFO, backing up the NFO and skipping for now: {}".format(
                            ex(e)))
                    try:
                ...
                for epDetails in showXML.getiterator('episodedetails'):
                    if epDetails.findtext('season') is None or int(epDetails.findtext('season')) != self.season or \
                        ...
                        logging.debug(
                            ...
                            (
                                self.show.indexerid, self.season or 0, self.episode or 0, epDetails.findtext(
                                    'season') or 0,
    ...
    def createMetaFiles(self):
        ...
        if not ek(os.path.isdir, self.show._location):
            logging.info(str(self.show.indexerid) +
                         ": The show dir is missing, not bothering to try to create metadata")
            return
    ...
    def deleteEpisode(self, full=False):
        ...
        logging.debug("Deleting %s S%02dE%02d from the DB" %
                      (self.show.name, self.season or 0, self.episode or 0))
        ...
        if sickbeard.USE_TRAKT and sickbeard.TRAKT_SYNC_WATCHLIST and data:
            ...
            notifiers.trakt_notifier.update_watchlist(
                self.show, data_episode=data, update="remove")
        ...
        try:
            ...
            if epID:
                ...
                if sickbeard.SUBTITLES_MULTI or not rows[
                        0][b'subtitles'] or not self.subtitles:
                    return [
    ...
    def _replace_map(self):
        ...
        def release_group(show, name):
            ...
            try:
                ...
                logging.debug(
                    "Unable to get parse release_group: {}".format(
                        ex(e)))
                return ''
        ...
        if hasattr(self, 'release_name'):  # from the release name field in db
            rel_grp[b'release_name'] = release_group(
        ...
        encoder = Quality.sceneQualityFromName(
            self.release_name.replace(
                rel_grp[relgrp], ""), epQual)
        if encoder:
            logging.debug(
                "Found codec for '" +
                show_name +
    ...
    def _format_pattern(self, pattern=None, multi=None, anime_type=None):
        ...
        if replace_map['%RG'] and replace_map['%RG'] != 'SiCKRAGE':
            if not hasattr(self, '_release_group'):
                logging.debug(
                    "Episode has no release group, replacing it with '" +
                    replace_map['%RG'] +
                    "'")
    ...
    def formatted_dir(self, pattern=None, multi=None):
        ...
        if len(name_groups) == 1:
            ...
            return self._format_pattern(
                ek(os.sep.join, name_groups[:-1]), multi)
    ...
    def rename(self):
        ...
        logging.debug(
            "Renaming/moving episode from the base path " +
            self.location +
            " to " +
        ...
        if proper_path == current_path:
            logging.debug(str(self.indexerid) +
                          ": File " +
                          self.location +
                          " is already named correctly, skipping")
            return
        ...
        for cur_related_file in related_files:
            ...
            if not cur_result:
                logging.error(str(self.indexerid) +
                              ": Unable to rename file " + cur_related_file)
        ...
        for cur_related_sub in related_subs:
            ...
            if not cur_result:
                logging.error(str(self.indexerid) +
                              ": Unable to rename file " + cur_related_sub)
        ...
        if filemtime != airdatetime:
            ...
            try:
                if helpers.touchFile(self.location, time.mktime(airdatetime)):
                    logging.info(
                        str(self.show.indexerid) + ": Changed modify date of " +
                        ek(os.path.basename, self.location)
                        + " to show air date " + time.strftime("%b %d,%Y (%H:%M)", airdatetime))
                else:
                    logging.error(
                        str(self.show.indexerid) + ": Unable to modify date of " +
                        ek(os.path.basename, self.location)
                        + " to show air date " + time.strftime("%b %d,%Y (%H:%M)", airdatetime))
            except Exception:
                logging.error(
                    str(self.show.indexerid) + ": Failed to modify date of '" +
                    ek(os.path.basename, self.location)
                    + "' to show air date " + time.strftime("%b %d,%Y (%H:%M)", airdatetime))

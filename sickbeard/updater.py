...
class Updater(object):
    ...
    def run(self, force=False):
        ...
        if not sickbeard.GITHUB:
            self.init_github()

        if self.updater:
            # set current branch version
            sickbeard.GIT_BRANCH = self.get_branch
    ...
    def init_github(self):
        try:
            sickbeard.GITHUB = github.Github(
                    login_or_token=sickbeard.GIT_USERNAME,
                    password=sickbeard.GIT_PASSWORD,
                    user_agent="SiCKRAGE"
            ).get_organization(sickbeard.GIT_ORG).get_repo(sickbeard.GIT_REPO)
        except:
            sickbeard.GITHUB = github.Github(
                    user_agent="SiCKRAGE"
            ).get_organization(sickbeard.GIT_ORG).get_repo(sickbeard.GIT_REPO)
    ...
    def _runbackup(self):
        ...
        try:
            ...
            if self._keeplatestbackup(backupDir) and helpers.backupAll(backupDir):
            ...
            logging.error('Update: Config backup failed. Error: %s' % e)
            ui.notifications.message('Backup', 'Config backup failed, aborting update')
            return False
    ...
    def safe_to_update(self):
        ...
        def postprocessor_safe():
            if not sickbeard.SCHEDULER.get_job('POSTPROCESSOR').func.im_self.amActive:
        ...
        def showupdate_safe():
            if not sickbeard.SCHEDULER.get_job('SHOWUPDATER').func.im_self.amActive:
    ...
    def find_install_type():
        ...
        install_type = 'source'
        if sickbeard.GIT_BRANCH.startswith('build '):
            install_type = 'win'
    ...
    def update(self):
        if self.updater:
            ...
            self.updater.branch = sickbeard.GIT_BRANCH
...
class GitUpdateManager(UpdateManager):
    def __init__(self):
        ...
        self.branch = sickbeard.GIT_BRANCH = self._find_installed_branch()
    ...
    def list_remote_branches(self):
        # update remote origin url
        self.update_remote_origin()
        sickbeard.GIT_BRANCH = self._find_installed_branch()
...
class SourceUpdateManager(UpdateManager):
    def __init__(self):
        ...
        self.branch = sickbeard.GIT_BRANCH
        if sickbeard.GIT_BRANCH == '':
    ...
    def _check_github_for_update(self):
        ...
        if self._cur_commit_hash:
            branch_compared = sickbeard.GITHUB.compare(base=self.branch, head=self._cur_commit_hash)
            self._newest_commit_hash = branch_compared.base_commit.sha
            self._num_commits_behind = branch_compared.behind_by

        # fall back and iterate over last 100 (items per page in gh_api) commits
        if not self._newest_commit_hash:
            for curCommit in sickbeard.GITHUB.get_commits():
                if not self._newest_commit_hash:
        ...
        logging.debug(
                "cur_commit = " + str(self._cur_commit_hash) + ", newest_commit = " + str(self._newest_commit_hash)
    ...
    def update(self):
        ...
        try:
            ...
            if os.path.isdir(sr_update_dir):
                ...
                helpers.removetree(sr_update_dir)
                ...
                for curfile in filenames:
                    ...
                    if curfile in ('unrar.dll', 'unrar64.dll'):
                        try:
                            ...
                            logging.debug("Unable to update " + new_path + ': ' + e.message)
            ...
            logging.error("Error while trying to update: {}".format(e))
    ...
    @property
    def list_remote_branches(self):
        return [x.name for x in sickbeard.GITHUB.get_branches() if x]

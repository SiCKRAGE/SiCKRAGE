...
from browser import foldersAtPath
from common import Quality, Overview, statusStrings, cpu_presets
from common import SNATCHED, UNAIRED, IGNORED, WANTED, FAILED, SKIPPED
from imdb_popular import imdb_popular
from indexers import adba
from logger import SRLogger
from scene_exceptions import get_scene_exceptions, update_scene_exceptions
from scene_numbering import get_scene_numbering, set_scene_numbering, get_scene_numbering_for_show, \
...
from sickbeard.exceptions import MultipleShowObjectsException, NoNFOException, ShowDirectoryNotFoundException
from sickbeard.providers import TorrentRssProvider, NewznabProvider, sortedProviderDict, TorrentProvider, NZBProvider, \
    GenericProvider
...
class Home(WebRoot):
    ...
    def is_alive(self, *args, **kwargs):
        ...
        if sickbeard.STARTED:
            return callback + '(' + json_encode({"msg": str(sickbeard.PID)}) + ');'
        else:
    ...
    def shutdown(self, pid=None):
        if sickbeard.WEB_SERVER:
            sickbeard.WEB_SERVER.server_shutdown()
            title = "Shutting down"
    ...
    def restart(self, pid=None):
        if sickbeard.WEB_SERVER:
            sickbeard.WEB_SERVER.server_restart()
    ...
    def branchCheckout(self, branch):
        if sickbeard.GIT_BRANCH != branch:
            sickbeard.GIT_BRANCH = branch
    ...
    @staticmethod
    def getDBcompare():
        db_status = Updater().getDBcompare()
    ...
    def displayShow(self, show=None):
        ...
        if sickbeard.showQueue.isBeingAdded(showObj):
            show_message = 'This show is in the process of being downloaded - the info below is incomplete.'

        elif sickbeard.showQueue.isBeingUpdated(showObj):
            show_message = 'The information on this page is in the process of being updated.'

        elif sickbeard.showQueue.isBeingRefreshed(showObj):
            show_message = 'The episodes below are currently being refreshed from disk'

        elif sickbeard.showQueue.isBeingSubtitled(showObj):
            show_message = 'Currently downloading subtitles for this show'

        elif sickbeard.showQueue.isInRefreshQueue(showObj):
            show_message = 'This show is queued to be refreshed.'

        elif sickbeard.showQueue.isInUpdateQueue(showObj):
            show_message = 'This show is queued and awaiting an update.'

        elif sickbeard.showQueue.isInSubtitleQueue(showObj):
            show_message = 'This show is queued and awaiting subtitles download.'

        if not sickbeard.showQueue.isBeingAdded(showObj):
            if not sickbeard.showQueue.isBeingUpdated(showObj):
                if showObj.paused:
                ...
                if sickbeard.USE_SUBTITLES and not sickbeard.showQueue.isBeingSubtitled(
    ...
    def editShow(self, show=None, location=None, anyQualities=[], bestQualities=[], exceptions_list=[],
        ...
        with showObj.lock:
            ...
            if bool(showObj.flatten_folders) != bool(flatten_folders):
                ...
                try:
                    sickbeard.showQueue.refreshShow(showObj)
    ...
    def updateShow(self, show=None, force=0):
        ...
        # force the update
        try:
            sickbeard.showQueue.updateShow(showObj, bool(force))
        except CantUpdateShowException as e:
    ...
    def setStatus(self, show=None, eps=None, status=None, direct=False):
        ...
        if int(status) == WANTED and not showObj.paused:
            ...
            for season, segment in segments.iteritems():
                ...
                sickbeard.searchQueue.add_item(cur_backlog_queue_item)
        ...
        if int(status) == FAILED:
            ...
            for season, segment in segments.iteritems():
                ...
                sickbeard.searchQueue.add_item(cur_failed_queue_item)
    ...
    def doRename(self, show=None, eps=None):
        ...
        show_obj = helpers.findCertainShow(sickbeard.showList, int(show))
    ...
    def searchEpisode(self, show=None, season=None, episode=None, downCurQuality=0):
        ...
        if isinstance(ep_obj, TVEpisode):
            ...
            sickbeard.searchQueue.add_item(ep_queue_item)
    ...
    def getManualSearchStatus(self, show=None):
        def getEpisodes(searchThread, searchstatus):
            results = []
            showObj = helpers.findCertainShow(sickbeard.showList, int(searchThread.show.indexerid))
        ...
        # Queued Searches
        searchstatus = 'queued'
        for searchThread in sickbeard.searchQueue.get_all_ep_from_queue(show):
            episodes += getEpisodes(searchThread, searchstatus)

        # Running Searches
        searchstatus = 'searching'
        if sickbeard.searchQueue.is_manualsearch_in_progress():
            searchThread = sickbeard.searchQueue.currentItem
...
class HomeAddShows(Home):
    ...
    def massAddTable(self, rootDir=None):
        ...
        for root_dir in root_dirs:
            ...
            for cur_file in file_list:
                ...
                for cur_provider in sickbeard.metadataProvideDict.values():
    ...
    def addTraktShow(self, indexer_id, showName):
        ...
        if location:
            show_dir = os.path.join(location, helpers.sanitizeFileName(showName))
            ...
            sickbeard.showQueue.addShow(1, int(indexer_id), show_dir,
    ...
    def addNewShow(self, whichSeries=None, indexerLang=None, rootDir=None, defaultStatus=None,
        ...
        if fullShowPath:
            ...
            show_dir = os.path.join(rootDir, helpers.sanitizeFileName(show_name))
        ...
        # add the show
        sickbeard.showQueue.addShow(indexer, indexer_id, show_dir, int(defaultStatus), newQuality,
    ...
    def addExistingShows(self, shows_to_add=None, promptForSettings=None):
        ...
        for cur_show in indexer_id_given:
            ...
            if indexer is not None and indexer_id is not None:
                # add the show
                sickbeard.showQueue.addShow(indexer, indexer_id, show_dir,
                                            default_status=sickbeard.STATUS_DEFAULT,
                                            quality=sickbeard.QUALITY_DEFAULT,
                                            flatten_folders=sickbeard.FLATTEN_FOLDERS_DEFAULT,
                                            subtitles=sickbeard.SUBTITLES_DEFAULT,
...
class Manage(Home, WebRoot):
    ...
    def downloadSubtitleMissed(self, *args, **kwargs):
        ...
        for cur_indexer_id in to_download:
            ...
            for epResult in to_download[cur_indexer_id]:
                ...
                show = helpers.findCertainShow(sickbeard.showList, int(cur_indexer_id))
    ...
    def backlogShow(self, indexer_id):
        ...
        if show_obj:
            sickbeard.backlogSearcher.searchBacklog([show_obj])
    ...
    def massUpdate(self, toUpdate=None, toRefresh=None, toRename=None, toDelete=None, toRemove=None, toMetadata=None,
        ...
        for curShowID in set(toUpdate + toRefresh + toRename + toSubtitle + toDelete + toRemove + toMetadata):
            ...
            showObj = helpers.findCertainShow(sickbeard.showList, int(curShowID))

            if showObj is None:
                continue

            if curShowID in toDelete:
                sickbeard.showQueue.removeShow(showObj, True)
                # don't do anything else if it's being deleted
                continue

            if curShowID in toRemove:
                sickbeard.showQueue.removeShow(showObj)
            ...
            if curShowID in toRename:
                sickbeard.showQueue.renameShowEpisodes(showObj)
                renames.append(showObj.name)

            if curShowID in toSubtitle:
                sickbeard.showQueue.downloadSubtitles(showObj)
...
class ManageSearches(Manage):
    ...
    def index(self):
        return self.render("manage_manageSearches.mako",
                           backlogPaused=sickbeard.searchQueue.is_backlog_paused(),
                           backlogRunning=sickbeard.searchQueue.is_backlog_in_progress(),
                           dailySearchStatus=sickbeard.dailySearcher.amActive,
                           findPropersStatus=sickbeard.properSearcher.amActive,
                           queueLength=sickbeard.searchQueue.queue_length(),
    ...
    def forceBacklog(self):
        ...
        result = sickbeard.SCHEDULER.get_job('BACKLOG').func()
        if result:
    ...
    def forceSearch(self):
        ...
        # force it to run the next time it looks
        result = sickbeard.SCHEDULER.get_job('DAILYSEARCHER').func()
        if result:
    ...
    def forceFindPropers(self):
        # force it to run the next time it looks
        result = sickbeard.SCHEDULER.get_job('PROPERSEARCHER').func()
        if result:
    ...
    def pauseBacklog(self, paused=None):
        if paused == "1":
            sickbeard.searchQueue.pause_backlog()
        else:
            sickbeard.searchQueue.unpause_backlog()
...
class ConfigGeneral(Config):
    ...
    def saveGeneral(self, log_dir=None, log_nr=5, log_size=1048576, web_port=None, web_log=None,
        ...
        if len(results) > 0:
            ...
            ui.notifications.message('Configuration Saved', os.path.join(sickbeard.CONFIG_FILE))
...
class ConfigBackupRestore(Config):
    ...
    def backup(backupDir=None):
        ...
        if backupDir:
            if helpers.backupAll(backupDir):
...
class ConfigSearch(Config):
    ...
    def saveSearch(self, use_nzbs=None, use_torrents=None, nzb_dir=None, sab_username=None, sab_password=None,
        ...
        sickbeard.PROPER_SEARCHER_INTERVAL = check_propers_interval
...
class ConfigPostProcessing(Config):
    ...
    def savePostProcessing(self, naming_pattern=None, naming_multi_ep=None,
        ...
        sickbeard.metadataProvideDict[b'KODI'].set_config(sickbeard.METADATA_KODI)
        sickbeard.metadataProvideDict[b'KODI 12+'].set_config(sickbeard.METADATA_KODI_12PLUS)
        sickbeard.metadataProvideDict[b'MediaBrowser'].set_config(sickbeard.METADATA_MEDIABROWSER)
        sickbeard.metadataProvideDict[b'Sony PS3'].set_config(sickbeard.METADATA_PS3)
        sickbeard.metadataProvideDict[b'WDTV'].set_config(sickbeard.METADATA_WDTV)
        sickbeard.metadataProvideDict[b'TIVO'].set_config(sickbeard.METADATA_TIVO)
        sickbeard.metadataProvideDict[b'Mede8er'].set_config(sickbeard.METADATA_MEDE8ER)
...
class ConfigProviders(Config):
    ...
    def canAddNewznabProvider(name):
        ...
        providerDict = dict(zip([x.id for x in sickbeard.newznabProviderList], sickbeard.newznabProviderList))
    ...
    def canAddTorrentRssProvider(name, url, cookies, titleTAG):
        ...
        providerDict = dict(
                zip([x.id for x in sickbeard.torrentRssProviderList], sickbeard.torrentRssProviderList))
    ...
    def saveProviders(self, newznab_string='', torrentrss_string='', provider_order=None, **kwargs):
        ...
        sickbeard.PROVIDER_ORDER = providers_reordered
        ...
        try:
            for logFile in [x for x in logFiles if os.path.isfile(x)]:
                data += "\n".join(logRegex.findall(
                        "\n".join(re.findall(
                                "((?:^.+?{}.+?$))".format(logSearch),
                                "\n".join(next(helpers.readFileBuffered(

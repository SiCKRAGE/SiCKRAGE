...
class ImageCache(object):
    ...
    def _cache_dir(self):
        ...
        """
        return os.path.abspath(os.path.join(sickrage.srCore.CONFIG.CACHE_DIR, 'images'))
    ...
    def has_poster(self, indexer_id):
        ...
        sickrage.srCore.LOGGER.debug("Checking if file " + str(poster_path) + " exists")
    ...
    def has_banner(self, indexer_id):
        ...
        sickrage.srCore.LOGGER.debug("Checking if file " + str(banner_path) + " exists")
    ...
    def has_fanart(self, indexer_id):
        ...
        sickrage.srCore.LOGGER.debug("Checking if file " + str(fanart_path) + " exists")
    ...
    def has_poster_thumbnail(self, indexer_id):
        ...
        sickrage.srCore.LOGGER.debug("Checking if file " + str(poster_thumb_path) + " exists")
    ...
    def has_banner_thumbnail(self, indexer_id):
        ...
        sickrage.srCore.LOGGER.debug("Checking if file " + str(banner_thumb_path) + " exists")
    ...
    def which_type(self, path):
        ...
        if not os.path.isfile(path):
            sickrage.srCore.LOGGER.warning("Couldn't check the type of " + str(path) + " cause it doesn't exist")
            return None

        with io.open(path, 'rb') as fh:
            img_metadata = extractMetadata(guessParser(StringInputStream(fh.read())))
            if not img_metadata:
                sickrage.srCore.LOGGER.debug("Unable to get metadata from " + str(path) + ", not using your existing image")
                return None
            ...
            # most fanart are around 1.77777 width/height ratio (eg. 1280/720 and 1920/1080)
                ...
                sickrage.srCore.LOGGER.warning("Image has size ratio of " + str(img_ratio) + ", unknown type")
    ...
    def _cache_image_from_file(self, image_path, img_type, indexer_id):
        ...
        if img_type == self.POSTER:
            ...
            sickrage.srCore.LOGGER.error("Invalid cache image type: " + str(img_type))
            return False

        # make sure the cache folder exists before we try copying to it
        if not os.path.isdir(self._cache_dir()):
            sickrage.srCore.LOGGER.info("Image cache dir didn't exist, creating it at " + str(self._cache_dir()))
            os.makedirs(self._cache_dir())

        if not os.path.isdir(self._thumbnails_dir()):
            sickrage.srCore.LOGGER.info("Thumbnails cache dir didn't exist, creating it at " + str(self._thumbnails_dir()))
            os.makedirs(self._thumbnails_dir())

        sickrage.srCore.LOGGER.info("Copying from " + image_path + " to " + dest_path)
    ...
    def fill_cache(self, show_obj):
        ...
        sickrage.srCore.LOGGER.debug("Checking if we need any cache images for show " + str(show_obj.indexerid))
        ...
        if not need_images[self.POSTER] and not need_images[self.BANNER] and not need_images[self.POSTER_THUMB] and not \
            ...
            sickrage.srCore.LOGGER.debug("No new cache images needed, not retrieving new ones")
            return
        ...
        if need_images[self.POSTER] or need_images[self.BANNER] or need_images[self.FANART]:
            try:
                for cur_provider in sickrage.srCore.metadataProviderDict.values():
                    sickrage.srCore.LOGGER.debug("Checking if we can use the show image from the " + cur_provider.name + " metadata")
                    if os.path.isfile(cur_provider.get_poster_path(show_obj)):
                        ...
                        sickrage.srCore.LOGGER.debug("Checking if image " + cur_file_name + " (type " + str(
                ...
                sickrage.srCore.LOGGER.warning("Unable to search for images in show dir because it doesn't exist")
        ...
        for cur_image_type in [self.POSTER, self.BANNER, self.POSTER_THUMB, self.BANNER_THUMB, self.FANART]:
            sickrage.srCore.LOGGER.debug("Seeing if we still need an image of type " + str(cur_image_type) + ": " + str(
        ...
        sickrage.srCore.LOGGER.info("Done cache check")

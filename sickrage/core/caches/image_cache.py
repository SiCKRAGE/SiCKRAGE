...
class ImageCache(object):
    ...
    def has_poster(self, indexer_id):
        ...
        sickrage.app.srLogger.debug("Checking if file " + str(poster_path) + " exists")
    ...
    def has_banner(self, indexer_id):
        ...
        sickrage.app.srLogger.debug("Checking if file " + str(banner_path) + " exists")
    ...
    def has_fanart(self, indexer_id):
        ...
        sickrage.app.srLogger.debug("Checking if file " + str(fanart_path) + " exists")
    ...
    def has_poster_thumbnail(self, indexer_id):
        ...
        sickrage.app.srLogger.debug("Checking if file " + str(poster_thumb_path) + " exists")
    ...
    def has_banner_thumbnail(self, indexer_id):
        ...
        sickrage.app.srLogger.debug("Checking if file " + str(banner_thumb_path) + " exists")
    ...
    def which_type(self, path):
        ...
        if not os.path.isfile(path):
            sickrage.app.srLogger.warning("Couldn't check the type of " + str(path) + " cause it doesn't exist")
            return None
        ...
        with io.open(path, 'rb') as fh:
            ...
            # most fanart are around 1.77777 width/height ratio (eg. 1280/720 and 1920/1080)
                ...
                sickrage.app.srLogger.warning("Image has size ratio of " + str(img_ratio) + ", unknown type")
    ...
    def _cache_image_from_file(self, image_path, img_type, indexer_id):
        ...
        if img_type == self.POSTER:
            ...
            sickrage.app.srLogger.error("Invalid cache image type: " + str(img_type))
            return False

        # make sure the cache folder exists before we try copying to it
        if not os.path.isdir(self._cache_dir()):
            sickrage.app.srLogger.info("Image cache dir didn't exist, creating it at " + str(self._cache_dir()))
        ...
        sickrage.app.srLogger.info("Copying from " + image_path + " to " + dest_path)
    ...
    def fill_cache(self, show_obj, force=False):
        ...
        sickrage.app.srLogger.debug("Checking if we need any cache images for show " + str(show_obj.indexerid))
        ...
        if all([not need_images[self.POSTER],
            ...
            sickrage.app.srLogger.debug("No new cache images needed, not retrieving new ones")
            return
            ...
            if os.path.isdir(show_obj.location):
                for cur_provider in sickrage.app.metadataProvidersDict.values():
                    ...
                    if os.path.isfile(cur_provider.get_poster_path(show_obj)):
                        ...
                        sickrage.app.srLogger.debug("Checking if image " + cur_file_name + " (type " + str(
        ...
        sickrage.app.srLogger.info("Done cache check")

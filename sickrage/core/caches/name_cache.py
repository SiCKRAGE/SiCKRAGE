...
class srNameCache(object):
    ...
    def put(self, name, indexer_id=0):
        ...
        if name not in self.cache:
            ...
            try:
                dbData = [x['doc'] for x in sickrage.app.cacheDB.db.get_many('scene_names', name, with_doc=True) if
    ...
    def clear(self, indexerid=0):
        ...
        [sickrage.app.cacheDB.db.delete(x['doc']) for x in
         sickrage.app.cacheDB.db.all('scene_names', with_doc=True)
    ...
    def load(self):
        self.cache = dict(
            [(key, d['doc'][key]) for d in sickrage.app.cacheDB.db.all('scene_names', with_doc=True) for key in
             d['doc']])

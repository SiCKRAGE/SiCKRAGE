...
def getSeasonNZBs(name, urlData, season):
    ...
    try:
        ...
        sickrage.srCore.LOGGER.error("Unable to parse the XML of " + name + ", not splitting it")
        return {}, ''
    ...
    if sceneNameMatch:
        ...
        sickrage.srCore.LOGGER.error("Unable to parse " + name + " into a scene name. If it's a valid one log a bug.")
        return {}, ''
...
def splitNZBResult(result):
    ...
    if urlData is None:
        sickrage.srCore.LOGGER.error("Unable to load url " + result.url + ", can't download season NZB")
        return False
    ...
    try:
        ...
        sickrage.srCore.LOGGER.debug("Unable to parse the filename " + result.name + " into a valid episode")
        return False
    except InvalidShowException:
        sickrage.srCore.LOGGER.debug("Unable to parse the filename " + result.name + " into a valid show")
        return False
    ...
    for newNZB in separateNZBs:
        ...
        sickrage.srCore.LOGGER.debug("Split out " + newNZB + " from " + result.name)
        ...
        try:
            ...
            sickrage.srCore.LOGGER.debug("Unable to parse the filename " + newNZB + " into a valid episode")
            return False
        except InvalidShowException:
            sickrage.srCore.LOGGER.debug("Unable to parse the filename " + newNZB + " into a valid show")
            return False
        ...
        for epNo in parse_result.episode_numbers:
            if not result.extraInfo[0].wantEpisode(season, epNo, result.quality):
                sickrage.srCore.LOGGER.info("Ignoring result " + newNZB + " because we don't want an episode that is " +

...
def delete_folder(folder, check_empty=True):
    ...
    if sickrage.srCore.CONFIG.TV_DOWNLOAD_DIR:
        if real_path(folder) == real_path(sickrage.srCore.CONFIG.TV_DOWNLOAD_DIR):
            return False
    ...
    try:
        if check_empty:
            ...
            if check_files:
                sickrage.srCore.LOGGER.info("Not deleting folder {} found the following files: {}".format(folder, check_files))
                return False

            sickrage.srCore.LOGGER.info("Deleting folder (if it's empty): " + folder)
            os.rmdir(folder)
        else:
            sickrage.srCore.LOGGER.info("Deleting folder: " + folder)
            removetree(folder)
    except (OSError, IOError) as e:
        sickrage.srCore.LOGGER.warning("Warning: unable to delete folder: {}: {}".format(folder, e))
        return False
...
def delete_files(processPath, notwantedFiles, result, force=False):
    ...
    if not result.result and force:
        result.output += logHelper("Forcing deletion of files, even though last result was not success", sickrage.srCore.LOGGER.DEBUG)
    ...
    for cur_file in notwantedFiles:
        ...
        result.output += logHelper("Deleting file %s" % cur_file, sickrage.srCore.LOGGER.DEBUG)

        # check first the read-only attribute
        file_attribute = os.stat(cur_file_path)[0]
        if not file_attribute & stat.S_IWRITE:
            # File is read-only, so make it writeable
            result.output += logHelper("Changing ReadOnly Flag for file %s" % cur_file, sickrage.srCore.LOGGER.DEBUG)
            try:
                os.chmod(cur_file_path, stat.S_IWRITE)
            except OSError as e:
                result.output += logHelper(
                        "Cannot change permissions of %s: %s" % (
                            cur_file, str(e.strerror).decode(sickrage.srCore.CONFIG.SYS_ENCODING)),
                        sickrage.srCore.LOGGER.DEBUG)
        try:
            os.remove(cur_file_path)
        except OSError as e:
            result.output += logHelper(
                    "Unable to delete file %s: %s" % (cur_file, str(e.strerror).decode(sickrage.srCore.CONFIG.SYS_ENCODING)),
                    sickrage.srCore.LOGGER.DEBUG)


def logHelper(logMessage, logLevel=None):
    sickrage.srCore.LOGGER.log(logLevel or sickrage.srCore.LOGGER.INFO, logMessage)
    return logMessage + "\n"
...
def processDir(dirName, nzbName=None, process_method=None, force=False, is_priority=None, delete_on=False, failed=False,
    ...
    result.output += logHelper("Processing folder %s" % dirName, sickrage.srCore.LOGGER.DEBUG)

    result.output += logHelper("TV_DOWNLOAD_DIR: %s" % sickrage.srCore.CONFIG.TV_DOWNLOAD_DIR, sickrage.srCore.LOGGER.DEBUG)
    postpone = False
    ...
    elif sickrage.srCore.CONFIG.TV_DOWNLOAD_DIR and os.path.isdir(sickrage.srCore.CONFIG.TV_DOWNLOAD_DIR) \
            and os.path.normpath(dirName) != os.path.normpath(sickrage.srCore.CONFIG.TV_DOWNLOAD_DIR):
        dirName = os.path.join(sickrage.srCore.CONFIG.TV_DOWNLOAD_DIR, os.path.abspath(dirName).split(os.path.sep)[-1])
        result.output += logHelper("Trying to use folder %s" % dirName, sickrage.srCore.LOGGER.DEBUG)
    ...
    if SyncFiles and sickrage.srCore.CONFIG.POSTPONE_IF_SYNC_FILES:
        postpone = True

    nzbNameOriginal = nzbName

    if not postpone:
        result.output += logHelper("PostProcessing Path: %s" % path, sickrage.srCore.LOGGER.INFO)
        result.output += logHelper("PostProcessing Dirs: [%s]" % dirs, sickrage.srCore.LOGGER.DEBUG)
        ...
        result.output += logHelper("PostProcessing Files: [%s]" % ", ".join(files), sickrage.srCore.LOGGER.DEBUG)
        result.output += logHelper("PostProcessing VideoFiles: [%s]" % ", ".join(videoFiles), sickrage.srCore.LOGGER.DEBUG)
        result.output += logHelper("PostProcessing RarContent: [%s]" % ", ".join(rarContent), sickrage.srCore.LOGGER.DEBUG)
        result.output += logHelper("PostProcessing VideoInRar: [%s]" % ", ".join(videoInRar), sickrage.srCore.LOGGER.DEBUG)
        ...
        if not process_method:
            process_method = sickrage.srCore.CONFIG.PROCESS_METHOD
        ...
        elif sickrage.srCore.CONFIG.DELRARCONTENTS and videoInRar:
    ...
    for curDir in [x for x in dirs if validateDir(path, x, nzbNameOriginal, failed, result)]:
        ...
        for processPath, _, fileList in os.walk(os.path.join(path, curDir), topdown=False):
            ...
            if not postpone:
                ...
                if notwantedFiles:
                    result.output += logHelper("Found unwanted files: [%s]" % ", ".join(notwantedFiles), sickrage.srCore.LOGGER.DEBUG)
                ...
                if process_method in ('hardlink', 'symlink') and videoInRar:
                    ...
                    if (not sickrage.srCore.CONFIG.NO_DELETE or proc_type == "manual") and process_method == "move" and \
                                    os.path.normpath(processPath) != os.path.normpath(sickrage.srCore.CONFIG.TV_DOWNLOAD_DIR):
                        if delete_folder(processPath, check_empty=True):
                            result.output += logHelper("Deleted folder: %s" % processPath, sickrage.srCore.LOGGER.DEBUG)
            else:
...
def validateDir(path, dirName, nzbNameOriginal, failed, result):
    ...
    result.output += logHelper("Processing folder " + dirName, sickrage.srCore.LOGGER.DEBUG)

    if folder_name.startswith('_FAILED_'):
        result.output += logHelper("The directory name indicates it failed to extract.", sickrage.srCore.LOGGER.DEBUG)
        failed = True
        ...
        result.output += logHelper(
                "The directory name indicates that this release is in the process of being unpacked.", sickrage.srCore.LOGGER.DEBUG)
    ...
    if is_hidden_folder(os.path.join(path, dirName)):
        result.output += logHelper("Ignoring hidden folder: " + dirName, sickrage.srCore.LOGGER.DEBUG)
    ...
    if sickrage.srCore.CONFIG.UNPACK:
    ...
    result.output += logHelper(dirName + " : No processable items found in folder", sickrage.srCore.LOGGER.DEBUG)
    return False
...
def unRAR(path, rarFiles, force, result):
    ...
    if sickrage.srCore.CONFIG.UNPACK and rarFiles:

        result.output += logHelper("Packed Releases detected: " + str(rarFiles), sickrage.srCore.LOGGER.DEBUG)

        for archive in rarFiles:

            result.output += logHelper("Unpacking archive: " + archive, sickrage.srCore.LOGGER.DEBUG)
            ...
            try:
                ...
                result.output += logHelper(
                        "Failed Unrar archive {0}: Unrar: File Open Error, check the parent folder and destination file permissions.".format(
                                archive), sickrage.srCore.LOGGER.ERROR)
                result.result = False
                ...
                result.output += logHelper("Failed Unrar archive {}: {}".format(archive, e), sickrage.srCore.LOGGER.ERROR)
                result.result = False
                result.missedfiles.append(archive + " : Unpacking failed for an unknown reason")
                continue

        result.output += logHelper("UnRar content: " + str(unpacked_files), sickrage.srCore.LOGGER.DEBUG)
...
def process_media(processPath, videoFiles, nzbName, process_method, force, is_priority, result):
    ...
    for cur_video_file in videoFiles:
        ...
        if already_postprocessed(processPath, cur_video_file, force, result):
            result.output += logHelper("Already Processed " + cur_video_file_path + " : Skipping", sickrage.srCore.LOGGER.DEBUG)
            continue
...
def get_path_dir_files(dirName, nzbName, proc_type):
    ...
    if dirName == sickrage.srCore.CONFIG.TV_DOWNLOAD_DIR and not nzbName or proc_type == "manual":  # Scheduled Post Processing Active
...
def process_failed(dirName, nzbName, result):
    ...
    if sickrage.srCore.CONFIG.USE_FAILED_DOWNLOADS:
        ...
        if sickrage.srCore.CONFIG.DELETE_FAILED and result.result:
            if delete_folder(dirName, check_empty=False):
                result.output += logHelper("Deleted folder: " + dirName, sickrage.srCore.LOGGER.DEBUG)
        ...
        if result.result:
            ...
            result.output += logHelper("Failed Download Processing failed: ({}, {}): {}"
                                       .format(nzbName, dirName, process_fail_message), sickrage.srCore.LOGGER.WARNING)

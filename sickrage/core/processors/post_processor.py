...
class PostProcessor(object):
    ...
    def __init__(self, file_path, nzb_name=None, process_method=None, is_priority=None):
        ...
        self.process_method = process_method if process_method else sickrage.srCore.CONFIG.PROCESS_METHOD
    ...
    def _log(self, message, level=None):
        ...
        """
        sickrage.srCore.LOGGER.log(level or sickrage.srCore.LOGGER.INFO, message)
        self.log += message + '\n'
    ...
    def _checkForExistingFile(self, existing_file):
        ...
        if not existing_file:
            self._log("There is no existing file so there's no worries about replacing it", sickrage.srCore.LOGGER.DEBUG)
        ...
        if os.path.isfile(existing_file):
            ...
            if os.path.getsize(existing_file) > os.path.getsize(self.file_path):
                self._log("File " + existing_file + " is larger than " + self.file_path, sickrage.srCore.LOGGER.DEBUG)
                return PostProcessor.EXISTS_LARGER

            elif os.path.getsize(existing_file) == os.path.getsize(self.file_path):
                self._log("File " + existing_file + " is the same size as " + self.file_path, sickrage.srCore.LOGGER.DEBUG)
                return PostProcessor.EXISTS_SAME

            else:
                self._log("File " + existing_file + " is smaller than " + self.file_path, sickrage.srCore.LOGGER.DEBUG)
    ...
    def list_associated_files(self, file_path, base_name_only=False, subtitles_only=False, subfolders=False):
        ...
        if file_path_list:
            self._log("Found the following associated files: " + str(file_path_list), sickrage.srCore.LOGGER.DEBUG)
        else:
            self._log("No associated files were during this pass", sickrage.srCore.LOGGER.DEBUG)
    ...
    def _delete(self, file_path, associated_files=False):
        ...
        if not file_list:
            self._log("There were no files associated with " + file_path + ", not deleting anything", sickrage.srCore.LOGGER.DEBUG)
            return

        # delete the file and any other files which we want to delete
        for cur_file in file_list:
            if os.path.isfile(cur_file):
                self._log("Deleting file " + cur_file, sickrage.srCore.LOGGER.DEBUG)
                # check first the read-only attribute
                file_attribute = os.stat(cur_file)[0]
                if not file_attribute & stat.S_IWRITE:
                    # File is read-only, so make it writeable
                    self._log('Read only mode on file ' + cur_file + ' Will try to make it writeable', sickrage.srCore.LOGGER.DEBUG)
                    try:
                        os.chmod(cur_file, stat.S_IWRITE)
                    except:
                        self._log('Cannot change permissions of ' + cur_file, sickrage.srCore.LOGGER.WARNING)
    ...
    def _combined_file_operation(self, file_path, new_path, new_base_name, associated_files=False, action=None,
        ...
        if not action:
            self._log("Must provide an action for the combined file operation", sickrage.srCore.LOGGER.ERROR)
            return
        ...
        if not file_list:
            self._log("There were no files associated with " + file_path + ", not moving anything", sickrage.srCore.LOGGER.DEBUG)
            return
        ...
        for cur_file_path in file_list:
            ...
            if cur_extension == 'nfo' and sickrage.srCore.CONFIG.NFO_RENAME == True:
            ...
            if sickrage.srCore.CONFIG.SUBTITLES_DIR and cur_extension in subtitleExtensions:
                subs_new_path = os.path.join(new_path, sickrage.srCore.CONFIG.SUBTITLES_DIR)
                dir_exists = makeDir(subs_new_path)
                if not dir_exists:
                    sickrage.srCore.LOGGER.error("Unable to create subtitles folder " + subs_new_path)
                else:
    ...
    def _move(self, file_path, new_path, new_base_name, associated_files=False, subs=False):
        ...
        def _int_move(cur_file_path, new_file_path):
            ...
            self._log("Moving file from " + cur_file_path + " to " + new_file_path, sickrage.srCore.LOGGER.DEBUG)
            try:
    ...
    def _copy(self, file_path, new_path, new_base_name, associated_files=False, subs=False):
        ...
        def _int_copy(cur_file_path, new_file_path):
            ...
            self._log("Copying file from " + cur_file_path + " to " + new_file_path, sickrage.srCore.LOGGER.DEBUG)
            try:
    ...
    def _hardlink(self, file_path, new_path, new_base_name, associated_files=False, subs=False):
        ...
        def _int_hard_link(cur_file_path, new_file_path):
            ...
            self._log("Hard linking file from " + cur_file_path + " to " + new_file_path, sickrage.srCore.LOGGER.DEBUG)
            try:
                hardlinkFile(cur_file_path, new_file_path)
                chmodAsParent(new_file_path)
            except (IOError, OSError) as e:
                self._log("Unable to link file {} to {}: {}".format(cur_file_path, new_file_path, e)), sickrage.srCore.LOGGER.ERROR
                raise
    ...
    def _moveAndSymlink(self, file_path, new_path, new_base_name, associated_files=False, subs=False):
        ...
        def _int_move_and_sym_link(cur_file_path, new_file_path):
            ...
            self._log("Moving then symbolic linking file from " + cur_file_path + " to " + new_file_path, sickrage.srCore.LOGGER.DEBUG)
            try:
    ...
    def _history_lookup(self):
        ...
        for curName in names:
            ...
            self._log("Found result in history: " + str(to_return), sickrage.srCore.LOGGER.DEBUG)
    ...
    def _finalize(self, parse_result):
        ...
        if parse_result.series_name and (not (not (
            ...
            sickrage.srCore.LOGGER.debug("Parse result not sufficient (all following have to be set). will not save release name")
            sickrage.srCore.LOGGER.debug("Parse result(series_name): " + str(parse_result.series_name))
            sickrage.srCore.LOGGER.debug("Parse result(season_number): " + str(parse_result.season_number))
            sickrage.srCore.LOGGER.debug("Parse result(episode_numbers): " + str(parse_result.episode_numbers))
            sickrage.srCore.LOGGER.debug("Parse result(air_date): " + str(parse_result.air_date))
            sickrage.srCore.LOGGER.debug("Parse result(release_group): " + str(parse_result.release_group))
    ...
    def _analyze_name(self, name):
        ...
        sickrage.srCore.LOGGER.debug("Analyzing name " + repr(name))
    ...
    def _add_to_anidb_mylist(self, filePath):
        ...
        if set_up_anidb_connection():
            if not self.anidbEpisode:  # seems like we could parse the name before, now lets build the anidb object
                self.anidbEpisode = self._build_anidb_episode(sickrage.srCore.CONFIG.ADBA_CONNECTION, filePath)

            self._log("Adding the file to the anidb mylist", sickrage.srCore.LOGGER.DEBUG)
            try:
                ...
                if sql_result:
                    ...
                    if sql_result:
                        ...
                        self._log(
                                ...
                                str(episodes[0]) + " for show " + str(show.indexerid) + ", skipping", sickrage.srCore.LOGGER.DEBUG)
    ...
    def _get_ep_obj(self, show, season, episodes):
        ...
        for cur_episode in episodes:
            self._log("Retrieving episode object for " + str(season) + "x" + str(cur_episode), sickrage.srCore.LOGGER.DEBUG)
    ...
    def _get_quality(self, ep_obj):
        ...
        for cur_name in name_list:
            ...
            if ep_quality != Quality.UNKNOWN:
                sickrage.srCore.LOGGER.debug(cur_name + " looks like it has quality " + Quality.qualityStrings[
        ...
        if ep_quality != Quality.UNKNOWN:
            sickrage.srCore.LOGGER.debug(self.file_name + " looks like it has quality " + Quality.qualityStrings[
                ep_quality] + ", using that")
    ...
    def _run_extra_scripts(self, ep_obj):
        ...
        """
        for curScriptName in sickrage.srCore.CONFIG.EXTRA_SCRIPTS:

            # generate a safe command line string to execute the script and provide all the parameters
            script_cmd = [piece for piece in re.split("( |\\\".*?\\\"|'.*?')", curScriptName) if piece.strip()]
            script_cmd[0] = os.path.abspath(script_cmd[0])
            self._log("Absolute path to script: " + script_cmd[0], sickrage.srCore.LOGGER.DEBUG)
    ...
    def _is_priority(self, ep_obj, new_ep_quality):
        ...
        if self.in_history or ep_obj.status in Quality.SNATCHED + Quality.SNATCHED_PROPER + Quality.SNATCHED_BEST:
            ...
            if not self.in_history:
                self._log("SR snatched this episode and it is not processed before", sickrage.srCore.LOGGER.DEBUG)
                return True
    ...
    def process(self):
        ...
        sickrage.srCore.LOGGER.debug("Quality of the episode we're processing: %s" % new_ep_quality)

        # see if this is a priority download (is it snatched, in history, PROPER, or BEST)
        priority_download = self._is_priority(ep_obj, new_ep_quality)
        self._log("Is ep a priority download: " + str(priority_download), sickrage.srCore.LOGGER.DEBUG)
        ...
        for cur_ep in [ep_obj] + ep_obj.relatedEps:
            with cur_ep.lock:
                ...
                if self.release_name:
                    self._log("Found release name " + self.release_name, sickrage.srCore.LOGGER.DEBUG)
        ...
        if releaseName is not None:
            ...
            self._log("Couldn't find release in snatch history", sickrage.srCore.LOGGER.WARNING)
        ...
        self._log("Destination folder for this episode: " + dest_path, sickrage.srCore.LOGGER.DEBUG)

        # create any folders we need
        make_dirs(dest_path)

        # figure out the base name of the resulting episode file
        if sickrage.srCore.CONFIG.RENAME_EPISODES:
        ...
        # add to anidb
        if ep_obj.show.is_anime and sickrage.srCore.CONFIG.ANIDB_USE_MYLIST:
        ...
        try:
            ...
            if self.process_method == "copy":
                ...
                self._copy(self.file_path, dest_path, new_base_name, sickrage.srCore.CONFIG.MOVE_ASSOCIATED_FILES,
                           sickrage.srCore.CONFIG.USE_SUBTITLES and ep_obj.show.subtitles)
                ...
                self._move(self.file_path, dest_path, new_base_name, sickrage.srCore.CONFIG.MOVE_ASSOCIATED_FILES,
                           sickrage.srCore.CONFIG.USE_SUBTITLES and ep_obj.show.subtitles)
            elif self.process_method == "hardlink":
                self._hardlink(self.file_path, dest_path, new_base_name, sickrage.srCore.CONFIG.MOVE_ASSOCIATED_FILES,
                               sickrage.srCore.CONFIG.USE_SUBTITLES and ep_obj.show.subtitles)
            elif self.process_method == "symlink":
                if isFileLocked(self.file_path, True):
                    raise EpisodePostProcessingFailedException("File is locked for reading/writing")
                self._moveAndSymlink(self.file_path, dest_path, new_base_name, sickrage.srCore.CONFIG.MOVE_ASSOCIATED_FILES,
                                     sickrage.srCore.CONFIG.USE_SUBTITLES and ep_obj.show.subtitles)
            else:
                sickrage.srCore.LOGGER.error("Unknown process method: " + str(self.process_method))
                raise EpisodePostProcessingFailedException("Unable to move the files to their new home")
        except (OSError, IOError):
            raise EpisodePostProcessingFailedException("Unable to move the files to their new home")

        # download subtitles
        if sickrage.srCore.CONFIG.USE_SUBTITLES and ep_obj.show.subtitles:
            for cur_ep in [ep_obj] + ep_obj.relatedEps:
                with cur_ep.lock:
        ...
        try:
            ...
            sickrage.srCore.LOGGER.info("Some notifications could not be sent. Continuing with post-processing...")

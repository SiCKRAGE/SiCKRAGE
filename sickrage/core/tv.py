...
from sickrage.core.common import ARCHIVED, DOWNLOADED, FAILED, IGNORED, \
    NAMING_DUPLICATE, NAMING_EXTEND, NAMING_LIMITED_EXTEND, \
    NAMING_LIMITED_EXTEND_E_PREFIXED, NAMING_SEPARATED_REPEAT, Overview, \
    Quality, SKIPPED, SNATCHED, SNATCHED_BEST, SNATCHED_PROPER, UNAIRED, \
    UNKNOWN, WANTED, dateTimeFormat, statusStrings
from sickrage.core.databases import main_db
from sickrage.core.exceptions import EpisodeDeletedException, \
    EpisodeNotFoundException, MultipleEpisodesInDatabaseException, \
    MultipleShowObjectsException, MultipleShowsInDatabaseException, \
    NoNFOException, ShowDirectoryNotFoundException, ShowNotFoundException
from sickrage.core.helpers import findCertainShow, isMediaFile, listMediaFiles, \
    remove_extension, remove_non_release_groups, removetree, rename_ep_file, \
    replaceExtension, sanitizeFileName, sanitizeSceneName, touchFile, tryInt, \
...
from sickrage.core.scene_numbering import get_scene_absolute_numbering, \
    get_scene_numbering, xem_refresh
    ...
    class TVShow(object):
        ...
        def getEpisode(self, season=None, episode=None, file=None, noCreate=False, absolute_number=None,
            ...
            if not episode in self.episodes[season] or self.episodes[season][episode] is None:
                ...
                if file:
                    ep = TV.TVEpisode(self, season, episode, file)
                else:
                    ep = TV.TVEpisode(self, season, episode)
        ...
        def mapIndexers(self):
            ...
            for curResult in sqlResults:
                ...
                if len(nlist) >= 4:
                    logging.debug("Found indexer mapping in cache for show: " + self.name)
                ...
                for indexer in indexerApi().indexers:
                    ...
                    try:
                        ...
                        logging.debug("Unable to map " + indexerApi(self.indexer).name + "->" + indexerApi(
                                indexer).name + " for show: " + self.name + ", skipping it")
                        continue

                    if mapped_show and len(mapped_show) == 1:
                        logging.debug("Mapping " + indexerApi(self.indexer).name + "->" + indexerApi(
                                indexer).name + " for show: " + self.name)

                        mapped[indexer] = int(mapped_show[0][b'id'])

                        logging.debug("Adding indexer mapping to DB for show: " + self.name)

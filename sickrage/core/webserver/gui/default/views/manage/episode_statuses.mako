...
<form action="${parent.web_root()}/manage/episodeStatuses" method="get">
...
<form action="${parent.web_root()}/manage/changeEpisodeStatuses" method="post">
...
<br>
    ...
    <tr id="${cur_indexer_id}">
        ...
        <th colspan="2" style="width: 100%; text-align: left;"><a class="whitelink" href="${parent.web_root()}/home/displayShow?show=${cur_indexer_id}">${show_names[cur_indexer_id]}</a> (${ep_counts[cur_indexer_id]}) <input type="button" class="pull-right get_more_eps btn" id="${cur_indexer_id}" value="Expand" /></th>
    </tr>
    % endfor

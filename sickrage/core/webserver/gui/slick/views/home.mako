...
<%block name="content">
    ...
    <div id="HomeLayout" class="pull-right hidden-print" style="margin-top: -40px;">
        % if sickrage.HOME_LAYOUT != 'poster':
        ...
        <select name="layout" class="form-control form-control-inline input-sm"
            ...
            <option value="${srRoot}/setHomeLayout/?layout=poster" ${('', 'selected="selected"')[sickrage.HOME_LAYOUT == 'poster']}>
                Poster
            </option>
            <option value="${srRoot}/setHomeLayout/?layout=small" ${('', 'selected="selected"')[sickrage.HOME_LAYOUT == 'small']}>
                Small Poster
            </option>
            <option value="${srRoot}/setHomeLayout/?layout=banner" ${('', 'selected="selected"')[sickrage.HOME_LAYOUT == 'banner']}>
                Banner
            </option>
            <option value="${srRoot}/setHomeLayout/?layout=simple" ${('', 'selected="selected"')[sickrage.HOME_LAYOUT == 'simple']}>
                Simple
            </option>
        </select>
            % if sickrage.HOME_LAYOUT != 'poster':
                Search:
            <input class="search form-control form-control-inline input-sm input200" type="search" data-column="2"
    ...
    <span> Sort By:
        <select id="postersort" class="form-control form-control-inline input-sm">
            <option value="name"
                    data-sort="${srRoot}/setPosterSortBy/?sort=name" ${('', 'selected="selected"')[sickrage.POSTER_SORTBY == 'name']}>
                Name
            </option>
            <option value="date"
                    data-sort="${srRoot}/setPosterSortBy/?sort=date" ${('', 'selected="selected"')[sickrage.POSTER_SORTBY == 'date']}>
                Next Episode
            </option>
            <option value="network"
                    data-sort="${srRoot}/setPosterSortBy/?sort=network" ${('', 'selected="selected"')[sickrage.POSTER_SORTBY == 'network']}>
                Network
            </option>
            <option value="progress"
                    data-sort="${srRoot}/setPosterSortBy/?sort=progress" ${('', 'selected="selected"')[sickrage.POSTER_SORTBY == 'progress']}>
                Progress
            </option>
        </select>
    </span>
    ...
    <span> Sort Order:
        <select id="postersortdirection" class="form-control form-control-inline input-sm">
            <option value="true"
                    data-sort="${srRoot}/setPosterSortDir/?direction=1" ${('', 'selected="selected"')[sickrage.POSTER_SORTDIR == 1]}>
                Asc
            </option>
            <option value="false"
                    data-sort="${srRoot}/setPosterSortDir/?direction=0" ${('', 'selected="selected"')[sickrage.POSTER_SORTDIR == 0]}>
                Desc
            </option>
        </select>
    </span>
        ...
        % if sickrage.HOME_LAYOUT == 'poster':
            <div id="${('container', 'container-anime')[curListType == 'Anime' and sickrage.HOME_LAYOUT == 'poster']}"
                ...
                <div class="posterview">
                    ...
                    %>
                        <div class="show" id="show${curShow.indexerid}" data-name="${curShow.name}"
                             data-date="${data_date}" data-network="${curShow.network}"
                             data-progress="${progressbar_percent}">
                            <div class="show-image">
                                <a href="${srRoot}/home/displayShow?show=${curShow.indexerid}"><img alt=""
                                                                                                    class="show-image"
                                                                                                    src="${srRoot}${showImage(curShow.indexerid, 'poster_thumb')}"/></a>
                            </div>
    ...
    <%
                            ...
                            <table width="100%" cellspacing="1" border="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td class="show-table">
                                        % if sickrage.HOME_LAYOUT != 'simple':
                                            % if curShow.network:
                                                <span title="${curShow.network}"><img class="show-network-image"
                                                                                      src="${srRoot}${showImage(curShow.indexerid, 'network')}"
                                                                                      alt="${curShow.network}"
                                                                                      title="${curShow.network}"/></span>
                                            % else:
                                                <span title="No Network"><img class="show-network-image"
                                                                              src="${srRoot}/images/network/nonetwork.png"
                                                                              alt="No Network"
                                                                              title="No Network"/></span>
                                            % endif
                                        % else:
            ...
            <table id="showListTable${curListType}" class="tablesorter" cellspacing="1" border="0" cellpadding="0">
                ...
                <tr>
                    <th rowspan="1" colspan="1" align="center"><a
                            href="${srRoot}/home/addShows/">Add ${('Show', 'Anime')[curListType == 'Anime']}</a></th>
                    <th>&nbsp;</th>
                    <th>&nbsp;</th>
                    <th>&nbsp;</th>
                    <th>&nbsp;</th>
                ...
                <tbody>
                    ...
                    % for curShow in myShowList:
                        ...
                        <tr>
                            ...
                            % if sickrage.HOME_LAYOUT == 'small':
                                <td class="tvShow">
                                    <div class="imgsmallposter ${sickrage.HOME_LAYOUT}">
                                        <a href="${srRoot}/home/displayShow?show=${curShow.indexerid}"
                                           title="${curShow.name}">
                                            <img src="${srRoot}${showImage(curShow.indexerid, 'poster_thumb')}"
                                                 class="${sickrage.HOME_LAYOUT}"
                            ...
                            % elif sickrage.HOME_LAYOUT == 'banner':
                                <td>
                                    <span style="display: none;">${curShow.name}</span>
                                    <div class="imgbanner ${sickrage.HOME_LAYOUT}">
                                        <a href="${srRoot}/home/displayShow?show=${curShow.indexerid}">
                                            <img src="${srRoot}${showImage(curShow.indexerid, 'banner')}"
                                                 class="${sickrage.HOME_LAYOUT}"
                                                 alt="${curShow.indexerid}" title="${curShow.name}"/>
                                        </a>
                                    </div>
                                </td>
                            % elif sickrage.HOME_LAYOUT == 'simple':
                                <td class="tvShow"><a
                                        href="${srRoot}/home/displayShow?show=${curShow.indexerid}">${curShow.name}</a>
                                </td>
                            % endif
                            ...
                            % if sickrage.HOME_LAYOUT != 'simple':
                                <td align="center">
                                    % if curShow.network:
                                        <span title="${curShow.network}" class="hidden-print"><img id="network"
                                                                                                   width="54"
                                                                                                   height="27"
                                                                                                   src="${srRoot}${showImage(curShow.indexerid, 'network')}"
                                                                                                   alt="${curShow.network}"
                                    ...
                                    % else:
                                        <span title="No Network" class="hidden-print"><img id="network" width="54"
                                                                                           height="27"
                                                                                           src="${srRoot}/images/network/nonetwork.png"
                                                                                           alt="No Network"
                                                                                           title="No Network"/></span>
                            ...
                            <td align="center">
                                ...
                                <img src="${srRoot}/images/${('no16.png', 'yes16.png')[bool(paused)]}"

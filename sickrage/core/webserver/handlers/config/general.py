...
class SaveAddShowDefaultsHandler(BaseHandler, ABC):
    def get(self, *args, **kwargs):
        ...
        best_qualities = self.get_query_argument('bestQualities', '')
        default_flatten_folders = self.get_query_argument('defaultFlattenFolders', None)
        subtitles = self.get_query_argument('subtitles', None)
        anime = self.get_query_argument('anime', None)
        scene = self.get_query_argument('scene', None)
        default_status_after = self.get_query_argument('defaultStatusAfter', common.WANTED)
        skip_downloaded = self.get_query_argument('skip_downloaded', None)
        add_show_year = self.get_query_argument('add_show_year', None)
...
class SaveGeneralHandler(BaseHandler, ABC):
    def post(self, *args, **kwargs):
        ...
        web_port = self.get_body_argument('web_port', None)
        web_ipv6 = self.get_body_argument('web_ipv6', None)
        trash_remove_show = self.get_body_argument('trash_remove_show', None)
        trash_rotate_logs = self.get_body_argument('trash_rotate_logs', None)
        update_frequency = self.get_body_argument('update_frequency', None)
        skip_removed_files = self.get_body_argument('skip_removed_files', None)
        indexerDefaultLang = self.get_body_argument('indexerDefaultLang', 'en')
        ep_default_deleted_status = self.get_body_argument('ep_default_deleted_status', None)
        launch_browser = self.get_body_argument('launch_browser', None)
        showupdate_hour = self.get_body_argument('showupdate_hour', 3)
        api_key = self.get_body_argument('api_key', None)
        indexer_default = self.get_body_argument('indexer_default', None)
        timezone_display = self.get_body_argument('timezone_display', None)
        cpu_preset = self.get_body_argument('cpu_preset', 'NORMAL')
        version_notify = self.get_body_argument('version_notify', None)
        enable_https = self.get_body_argument('enable_https', None)
        https_cert = self.get_body_argument('https_cert', None)
        https_key = self.get_body_argument('https_key', None)
        handle_reverse_proxy = self.get_body_argument('handle_reverse_proxy', None)
        sort_article = self.get_body_argument('sort_article', None)
        auto_update = self.get_body_argument('auto_update', None)
        notify_on_update = self.get_body_argument('notify_on_update', None)
        proxy_setting = self.get_body_argument('proxy_setting', None)
        proxy_indexers = self.get_body_argument('proxy_indexers', None)
        anon_redirect = self.get_body_argument('anon_redirect', None)
        git_path = self.get_body_argument('git_path', None)
        pip3_path = self.get_body_argument('pip3_path', None)
        calendar_unprotected = self.get_body_argument('calendar_unprotected', None)
        calendar_icons = self.get_body_argument('calendar_icons', None)
        debug = self.get_body_argument('debug', None)
        ssl_verify = self.get_body_argument('ssl_verify', None)
        no_restart = self.get_body_argument('no_restart', None)
        coming_eps_missed_range = self.get_body_argument('coming_eps_missed_range', None)
        filter_row = self.get_body_argument('filter_row', None)
        fuzzy_dating = self.get_body_argument('fuzzy_dating', None)
        trim_zero = self.get_body_argument('trim_zero', None)
        date_preset = self.get_body_argument('date_preset', None)
        time_preset = self.get_body_argument('time_preset', None)
        indexer_timeout = self.get_body_argument('indexer_timeout', None)
        download_url = self.get_body_argument('download_url', None)
        theme_name = self.get_body_argument('theme_name', None)
        default_page = self.get_body_argument('default_page', None)
        git_username = self.get_body_argument('git_username', None)
        git_password = self.get_body_argument('git_password', None)
        git_autoissues = self.get_body_argument('git_autoissues', None)
        gui_language = self.get_body_argument('gui_language', None)
        display_all_seasons = self.get_body_argument('display_all_seasons', None)
        showupdate_stale = self.get_body_argument('showupdate_stale', None)
        notify_on_login = self.get_body_argument('notify_on_login', None)
        allowed_video_file_exts = self.get_body_argument('allowed_video_file_exts', '')

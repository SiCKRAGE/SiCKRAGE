...
class SaveNotificationsHandler(BaseHandler, ABC):
    ...
    def post(self, *args, **kwargs):
        ...
        kodi_notify_onsnatch = self.get_body_argument('kodi_notify_onsnatch', None)
        kodi_notify_ondownload = self.get_body_argument('kodi_notify_ondownload', None)
        kodi_notify_onsubtitledownload = self.get_body_argument('kodi_notify_onsubtitledownload', None)
        kodi_update_onlyfirst = self.get_body_argument('kodi_update_onlyfirst', None)
        kodi_update_library = self.get_body_argument('kodi_update_library', None)
        kodi_update_full = self.get_body_argument('kodi_update_full', None)
        kodi_host = self.get_body_argument('kodi_host', None)
        kodi_username = self.get_body_argument('kodi_username', None)
        kodi_password = self.get_body_argument('kodi_password', None)
        use_plex = self.get_body_argument('use_plex', None)
        plex_notify_onsnatch = self.get_body_argument('plex_notify_onsnatch', None)
        plex_notify_ondownload = self.get_body_argument('plex_notify_ondownload', None)
        plex_notify_onsubtitledownload = self.get_body_argument('plex_notify_onsubtitledownload', None)
        plex_update_library = self.get_body_argument('plex_update_library', None)
        plex_server_host = self.get_body_argument('plex_server_host', None)
        plex_server_token = self.get_body_argument('plex_server_token', None)
        plex_host = self.get_body_argument('plex_host', None)
        plex_username = self.get_body_argument('plex_username', None)
        plex_password = self.get_body_argument('plex_password', None)
        use_emby = self.get_body_argument('use_emby', None)
        emby_notify_onsnatch = self.get_body_argument('emby_notify_onsnatch', None)
        emby_notify_ondownload = self.get_body_argument('emby_notify_ondownload', None)
        emby_notify_onsubtitledownload = self.get_body_argument('emby_notify_onsubtitledownload', None)
        emby_host = self.get_body_argument('emby_host', None)
        emby_apikey = self.get_body_argument('emby_apikey', None)
        use_growl = self.get_body_argument('use_growl', None)
        growl_notify_onsnatch = self.get_body_argument('growl_notify_onsnatch', None)
        growl_notify_ondownload = self.get_body_argument('growl_notify_ondownload', None)
        growl_notify_onsubtitledownload = self.get_body_argument('growl_notify_onsubtitledownload', None)
        growl_host = self.get_body_argument('growl_host', None)
        growl_password = self.get_body_argument('growl_password', None)
        use_freemobile = self.get_body_argument('use_freemobile', None)
        freemobile_notify_onsnatch = self.get_body_argument('freemobile_notify_onsnatch', None)
        freemobile_notify_ondownload = self.get_body_argument('freemobile_notify_ondownload', None)
        freemobile_notify_onsubtitledownload = self.get_body_argument('freemobile_notify_onsubtitledownload', None)
        freemobile_id = self.get_body_argument('freemobile_id', None)
        freemobile_apikey = self.get_body_argument('freemobile_apikey', None)
        use_telegram = self.get_body_argument('use_telegram', None)
        telegram_notify_onsnatch = self.get_body_argument('telegram_notify_onsnatch', None)
        telegram_notify_ondownload = self.get_body_argument('telegram_notify_ondownload', None)
        telegram_notify_onsubtitledownload = self.get_body_argument('telegram_notify_onsubtitledownload', None)
        telegram_id = self.get_body_argument('telegram_id', None)
        telegram_apikey = self.get_body_argument('telegram_apikey', None)
        use_join = self.get_body_argument('use_join', None)
        join_notify_onsnatch = self.get_body_argument('join_notify_onsnatch', None)
        join_notify_ondownload = self.get_body_argument('join_notify_ondownload', None)
        join_notify_onsubtitledownload = self.get_body_argument('join_notify_onsubtitledownload', None)
        join_id = self.get_body_argument('join_id', None)
        join_apikey = self.get_body_argument('join_apikey', None)
        use_prowl = self.get_body_argument('use_prowl', None)
        prowl_notify_onsnatch = self.get_body_argument('prowl_notify_onsnatch', None)
        prowl_notify_ondownload = self.get_body_argument('prowl_notify_ondownload', None)
        prowl_notify_onsubtitledownload = self.get_body_argument('prowl_notify_onsubtitledownload', None)
        prowl_api = self.get_body_argument('prowl_api', None)
        prowl_priority = self.get_body_argument('prowl_priority', 0)
        use_twitter = self.get_body_argument('use_twitter', None)
        twitter_notify_onsnatch = self.get_body_argument('twitter_notify_onsnatch', None)
        twitter_notify_ondownload = self.get_body_argument('twitter_notify_ondownload', None)
        twitter_notify_onsubtitledownload = self.get_body_argument('twitter_notify_onsubtitledownload', None)
        twitter_usedm = self.get_body_argument('twitter_usedm', None)
        twitter_dmto = self.get_body_argument('twitter_dmto', None)
        use_twilio = self.get_body_argument('use_twilio', None)
        twilio_notify_onsnatch = self.get_body_argument('twilio_notify_onsnatch', None)
        twilio_notify_ondownload = self.get_body_argument('twilio_notify_ondownload', None)
        twilio_notify_onsubtitledownload = self.get_body_argument('twilio_notify_onsubtitledownload', None)
        twilio_phone_sid = self.get_body_argument('twilio_phone_sid', None)
        twilio_account_sid = self.get_body_argument('twilio_account_sid', None)
        twilio_auth_token = self.get_body_argument('twilio_auth_token', None)
        twilio_to_number = self.get_body_argument('twilio_to_number', None)
        use_boxcar2 = self.get_body_argument('use_boxcar2', None)
        boxcar2_notify_onsnatch = self.get_body_argument('boxcar2_notify_onsnatch', None)
        boxcar2_notify_ondownload = self.get_body_argument('boxcar2_notify_ondownload', None)
        boxcar2_notify_onsubtitledownload = self.get_body_argument('boxcar2_notify_onsubtitledownload', None)
        boxcar2_accesstoken = self.get_body_argument('boxcar2_accesstoken', None)
        use_pushover = self.get_body_argument('use_pushover', None)
        pushover_notify_onsnatch = self.get_body_argument('pushover_notify_onsnatch', None)
        pushover_notify_ondownload = self.get_body_argument('pushover_notify_ondownload', None)
        pushover_notify_onsubtitledownload = self.get_body_argument('pushover_notify_onsubtitledownload', None)
        pushover_userkey = self.get_body_argument('pushover_userkey', None)
        pushover_apikey = self.get_body_argument('pushover_apikey', None)
        pushover_device = self.get_body_argument('pushover_device', None)
        pushover_sound = self.get_body_argument('pushover_sound', None)
        use_libnotify = self.get_body_argument('use_libnotify', None)
        libnotify_notify_onsnatch = self.get_body_argument('libnotify_notify_onsnatch', None)
        libnotify_notify_ondownload = self.get_body_argument('libnotify_notify_ondownload', None)
        libnotify_notify_onsubtitledownload = self.get_body_argument('libnotify_notify_onsubtitledownload', None)
        use_nmj = self.get_body_argument('use_nmj', None)
        nmj_host = self.get_body_argument('nmj_host', None)
        nmj_database = self.get_body_argument('nmj_database', None)
        nmj_mount = self.get_body_argument('nmj_mount', None)
        use_synoindex = self.get_body_argument('use_synoindex', None)
        use_nmjv2 = self.get_body_argument('use_nmjv2', None)
        nmjv2_host = self.get_body_argument('nmjv2_host', None)
        nmjv2_dbloc = self.get_body_argument('nmjv2_dbloc', None)
        nmjv2_database = self.get_body_argument('nmjv2_database', None)
        use_trakt = self.get_body_argument('use_trakt', None)
        trakt_username = self.get_body_argument('trakt_username', None)
        trakt_remove_watchlist = self.get_body_argument('trakt_remove_watchlist', None)
        trakt_sync_watchlist = self.get_body_argument('trakt_sync_watchlist', None)
        trakt_remove_show_from_sickrage = self.get_body_argument('trakt_remove_show_from_sickrage', None)
        trakt_method_add = self.get_body_argument('trakt_method_add', None)
        trakt_start_paused = self.get_body_argument('trakt_start_paused', None)
        trakt_use_recommended = self.get_body_argument('trakt_use_recommended', None)
        trakt_sync = self.get_body_argument('trakt_sync', None)
        trakt_sync_remove = self.get_body_argument('trakt_sync_remove', None)
        trakt_default_indexer = self.get_body_argument('trakt_default_indexer', None)
        trakt_remove_serieslist = self.get_body_argument('trakt_remove_serieslist', None)
        trakt_timeout = self.get_body_argument('trakt_timeout', None)
        trakt_blacklist_name = self.get_body_argument('trakt_blacklist_name', None)
        use_synologynotifier = self.get_body_argument('use_synologynotifier', None)
        synologynotifier_notify_onsnatch = self.get_body_argument('synologynotifier_notify_onsnatch', None)
        synologynotifier_notify_ondownload = self.get_body_argument('synologynotifier_notify_ondownload', None)
        synologynotifier_notify_onsubtitledownload = self.get_body_argument('synologynotifier_notify_onsubtitledownload', None)
        use_pytivo = self.get_body_argument('use_pytivo', None)
        pytivo_notify_onsnatch = self.get_body_argument('pytivo_notify_onsnatch', None)
        pytivo_notify_ondownload = self.get_body_argument('pytivo_notify_ondownload', None)
        pytivo_notify_onsubtitledownload = self.get_body_argument('pytivo_notify_onsubtitledownload', None)
        pytivo_update_library = self.get_body_argument('pytivo_update_library', None)
        pytivo_host = self.get_body_argument('pytivo_host', None)
        pytivo_share_name = self.get_body_argument('pytivo_share_name', None)
        pytivo_tivo_name = self.get_body_argument('pytivo_tivo_name', None)
        use_nma = self.get_body_argument('use_nma', None)
        nma_notify_onsnatch = self.get_body_argument('nma_notify_onsnatch', None)
        nma_notify_ondownload = self.get_body_argument('nma_notify_ondownload', None)
        nma_notify_onsubtitledownload = self.get_body_argument('nma_notify_onsubtitledownload', None)
        nma_api = self.get_body_argument('nma_api', None)
        nma_priority = self.get_body_argument('nma_priority', 0)
        use_pushalot = self.get_body_argument('use_pushalot', None)
        pushalot_notify_onsnatch = self.get_body_argument('pushalot_notify_onsnatch', None)
        pushalot_notify_ondownload = self.get_body_argument('pushalot_notify_ondownload', None)
        pushalot_notify_onsubtitledownload = self.get_body_argument('pushalot_notify_onsubtitledownload', None)
        pushalot_authorizationtoken = self.get_body_argument('pushalot_authorizationtoken', None)
        use_pushbullet = self.get_body_argument('use_pushbullet', None)
        pushbullet_notify_onsnatch = self.get_body_argument('pushbullet_notify_onsnatch', None)
        pushbullet_notify_ondownload = self.get_body_argument('pushbullet_notify_ondownload', None)
        pushbullet_notify_onsubtitledownload = self.get_body_argument('pushbullet_notify_onsubtitledownload', None)
        pushbullet_api = self.get_body_argument('pushbullet_api', None)
        pushbullet_device_list = self.get_body_argument('pushbullet_device_list', None)
        use_email = self.get_body_argument('use_email', None)
        email_notify_onsnatch = self.get_body_argument('email_notify_onsnatch', None)
        email_notify_ondownload = self.get_body_argument('email_notify_ondownload', None)
        email_notify_onsubtitledownload = self.get_body_argument('email_notify_onsubtitledownload', None)
        email_host = self.get_body_argument('email_host', None)
        email_port = self.get_body_argument('email_port', 25)
        email_from = self.get_body_argument('email_from', None)
        email_tls = self.get_body_argument('email_tls', None)
        email_user = self.get_body_argument('email_user', None)
        email_password = self.get_body_argument('email_password', None)
        email_list = self.get_body_argument('email_list', None)
        use_slack = self.get_body_argument('use_slack', None)
        slack_notify_onsnatch = self.get_body_argument('slack_notify_onsnatch', None)
        slack_notify_ondownload = self.get_body_argument('slack_notify_ondownload', None)
        slack_notify_onsubtitledownload = self.get_body_argument('slack_notify_onsubtitledownload', None)
        slack_webhook = self.get_body_argument('slack_webhook', None)
        use_discord = self.get_body_argument('use_discord', False)
        discord_notify_onsnatch = self.get_body_argument('discord_notify_onsnatch', None)
        discord_notify_ondownload = self.get_body_argument('discord_notify_ondownload', None)
        discord_notify_onsubtitledownload = self.get_body_argument('discord_notify_onsubtitledownload', None)
        discord_webhook = self.get_body_argument('discord_webhook', None)
        discord_name = self.get_body_argument('discord_name', None)
        discord_avatar_url = self.get_body_argument('discord_avatar_url', None)
        discord_tts = self.get_body_argument('discord_tts', None)

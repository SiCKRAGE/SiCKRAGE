...
class SaveSearchHandler(BaseHandler, ABC):
    ...
    def post(self, *args, **kwargs):
        ...
        nzb_dir = self.get_body_argument('nzb_dir', None)
        sab_username = self.get_body_argument('sab_username', None)
        sab_password = self.get_body_argument('sab_password', None)
        sab_apikey = self.get_body_argument('sab_apikey', None)
        sab_category = self.get_body_argument('sab_category', None)
        sab_category_anime = self.get_body_argument('sab_category_anime', None)
        sab_category_backlog = self.get_body_argument('sab_category_backlog', None)
        sab_category_anime_backlog = self.get_body_argument('sab_category_anime_backlog', None)
        sab_host = self.get_body_argument('sab_host', None)
        nzbget_username = self.get_body_argument('nzbget_username', None)
        nzbget_password = self.get_body_argument('nzbget_password', None)
        nzbget_category = self.get_body_argument('nzbget_category', None)
        nzbget_category_backlog = self.get_body_argument('nzbget_category_backlog', None)
        nzbget_category_anime = self.get_body_argument('nzbget_category_anime', None)
        nzbget_category_anime_backlog = self.get_body_argument('nzbget_category_anime_backlog', None)
        nzbget_priority = self.get_body_argument('nzbget_priority', None)
        nzbget_host = self.get_body_argument('nzbget_host', None)
        nzbget_use_https = self.get_body_argument('nzbget_use_https', None)
        backlog_frequency = self.get_body_argument('backlog_frequency', None)
        dailysearch_frequency = self.get_body_argument('dailysearch_frequency', None)
        nzb_method = self.get_body_argument('nzb_method', None)
        torrent_method = self.get_body_argument('torrent_method', None)
        usenet_retention = self.get_body_argument('usenet_retention', None)
        download_propers = self.get_body_argument('download_propers', None)
        check_propers_interval = self.get_body_argument('check_propers_interval', None)
        allow_high_priority = self.get_body_argument('allow_high_priority', None)
        sab_forced = self.get_body_argument('sab_forced', None)
        randomize_providers = self.get_body_argument('randomize_providers', None)
        use_failed_snatcher = self.get_body_argument('use_failed_snatcher', None)
        failed_snatch_age = self.get_body_argument('failed_snatch_age', None)
        torrent_dir = self.get_body_argument('torrent_dir', None)
        torrent_username = self.get_body_argument('torrent_username', None)
        torrent_password = self.get_body_argument('torrent_password', None)
        torrent_host = self.get_body_argument('torrent_host', None)
        torrent_label = self.get_body_argument('torrent_label', None)
        torrent_label_anime = self.get_body_argument('torrent_label_anime', None)
        torrent_path = self.get_body_argument('torrent_path', None)
        torrent_verify_cert = self.get_body_argument('torrent_verify_cert', None)
        torrent_seed_time = self.get_body_argument('torrent_seed_time', None)
        torrent_paused = self.get_body_argument('torrent_paused', None)
        torrent_high_bandwidth = self.get_body_argument('torrent_high_bandwidth', None)
        torrent_rpcurl = self.get_body_argument('torrent_rpcurl', None)
        torrent_auth_type = self.get_body_argument('torrent_auth_type', None)
        ignore_words = self.get_body_argument('ignore_words', None)
        require_words = self.get_body_argument('require_words', None)
        ignored_subs_list = self.get_body_argument('ignored_subs_list', None)
        enable_rss_cache = self.get_body_argument('enable_rss_cache', None)
        torrent_file_to_magnet = self.get_body_argument('torrent_file_to_magnet', None)
        download_unverified_magnet_link = self.get_body_argument('download_unverified_magnet_link', None)

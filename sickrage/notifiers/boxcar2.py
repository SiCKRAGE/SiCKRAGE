...
class Boxcar2Notifier(srNotifiers):
    ...
    def _sendBoxcar2(self, msg, title, accesstoken):
        ...
        try:
            ...
            # HTTP status 404
            if e.code == 404:
                sickrage.srCore.LOGGER.warning("Access token is invalid. Check it.")
                return False

            # If you receive an HTTP status code of 400, it is because you failed to send the proper parameters
            elif e.code == 400:
                sickrage.srCore.LOGGER.error("Wrong data send to boxcar2")
                return False

        sickrage.srCore.LOGGER.debug("Boxcar2 notification successful.")
        return True

    def _notify_snatch(self, ep_name, title=notifyStrings[NOTIFY_SNATCH]):
        if sickrage.srCore.CONFIG.BOXCAR2_NOTIFY_ONSNATCH:
            self._notifyBoxcar2(title, ep_name)

    def _notify_download(self, ep_name, title=notifyStrings[NOTIFY_DOWNLOAD]):
        if sickrage.srCore.CONFIG.BOXCAR2_NOTIFY_ONDOWNLOAD:
            self._notifyBoxcar2(title, ep_name)

    def _notify_subtitle_download(self, ep_name, lang, title=notifyStrings[NOTIFY_SUBTITLE_DOWNLOAD]):
        if sickrage.srCore.CONFIG.BOXCAR2_NOTIFY_ONSUBTITLEDOWNLOAD:
            self._notifyBoxcar2(title, ep_name + ": " + lang)

    def _notify_version_update(self, new_version="??"):
        if sickrage.srCore.CONFIG.USE_BOXCAR2:
    ...
    def _notifyBoxcar2(self, title, message, accesstoken=None):
        ...
        if not sickrage.srCore.CONFIG.USE_BOXCAR2:
            sickrage.srCore.LOGGER.debug("Notification for Boxcar2 not enabled, skipping this notification")
            return False

        # if no username was given then use the one from the config
        if not accesstoken:
            accesstoken = sickrage.srCore.CONFIG.BOXCAR2_ACCESSTOKEN

...
class KODINotifier(srNotifiers):
    ...
    def _get_kodi_version(self, host, username, password):
        ...
        socket.setdefaulttimeout(sickrage.srCore.CONFIG.SOCKET_TIMEOUT)
    ...
    def _notify_kodi(self, message, title="SiCKRAGE", host=None, username=None, password=None, force=False):
        ...
        if not username:
            username = sickrage.srCore.CONFIG.KODI_USERNAME
        if not password:
            password = sickrage.srCore.CONFIG.KODI_PASSWORD

        # suppress notifications if the notifier is disabled but the notify options are checked
        if not sickrage.srCore.CONFIG.USE_KODI and not force:
            sickrage.srCore.LOGGER.debug("Notification for KODI not enabled, skipping this notification")
            return False

        result = ''
        for curHost in [x.strip() for x in host.split(",")]:
            sickrage.srCore.LOGGER.debug("Sending KODI notification to '" + curHost + "' - " + message)

            kodiapi = self._get_kodi_version(curHost, username, password)
            if kodiapi:
                if kodiapi <= 4:
                    sickrage.srCore.LOGGER.debug("Detected KODI version <= 11, using KODI HTTP API")
                    ...
                    sickrage.srCore.LOGGER.debug("Detected KODI version >= 12, using KODI JSON API")
                    ...
                    if notifyResult and notifyResult.get('result'):
                        result += curHost + ':' + notifyResult[b"result"].decode(sickrage.srCore.CONFIG.SYS_ENCODING)
            else:
                if sickrage.srCore.CONFIG.KODI_ALWAYS_ON or force:
    ...
    def _send_update_library(self, host, showName=None):
        ...
        sickrage.srCore.LOGGER.debug("Sending request to update library for KODI host: '" + host + "'")

        kodiapi = self._get_kodi_version(host, sickrage.srCore.CONFIG.KODI_USERNAME, sickrage.srCore.CONFIG.KODI_PASSWORD)
        if kodiapi:
            if kodiapi <= 4:
                # try to update for just the show, if it fails, do full update if enabled
                if not self._update_library(host, showName) and sickrage.srCore.CONFIG.KODI_UPDATE_FULL:
                    sickrage.srCore.LOGGER.debug("Single show update failed, falling back to full update")
                    return self._update_library(host)
                else:
                    return True
            else:
                # try to update for just the show, if it fails, do full update if enabled
                if not self._update_library_json(host, showName) and sickrage.srCore.CONFIG.KODI_UPDATE_FULL:
                    sickrage.srCore.LOGGER.debug("Single show update failed, falling back to full update")
                    return self._update_library_json(host)
                else:
                    return True
        elif sickrage.srCore.CONFIG.KODI_ALWAYS_ON:
            sickrage.srCore.LOGGER.warning("Failed to detect KODI version for '" + host + "', check configuration and try again.")
    ...
    def _send_to_kodi(self, command, host=None, username=None, password=None):
        ...
        if not host:
            sickrage.srCore.LOGGER.warning('No KODI host passed, aborting update')
            return False
        ...
        sickrage.srCore.LOGGER.debug("KODI encoded API command: " + enc_command)
        ...
        try:
            ...
            if password:
                ...
                sickrage.srCore.LOGGER.debug("Contacting KODI (with auth header) via url: " + url)
            else:
                sickrage.srCore.LOGGER.debug("Contacting KODI via url: " + url)

            try:
                response = urllib2.urlopen(req)
            except (httplib.BadStatusLine, urllib2.URLError) as e:
                sickrage.srCore.LOGGER.debug("Couldn't contact KODI HTTP at %r : %r" % (url, e))
                return False

            result = response.read().decode(sickrage.srCore.CONFIG.SYS_ENCODING)
            response.close()

            sickrage.srCore.LOGGER.debug("KODI HTTP response: " + result.replace('\n', ''))
            return result
    ...
    def _update_library(self, host=None, showName=None):
        ...
        sickrage.srCore.LOGGER.debug("Updating KODI library via HTTP method for host: " + host)

        # if we're doing per-show
        if showName:
            sickrage.srCore.LOGGER.debug("Updating library in KODI via HTTP method for show " + showName)
            ...
            if not sqlXML:
                sickrage.srCore.LOGGER.debug("Invalid response for " + showName + " on " + host)
                return False
            ...
            if not paths:
                sickrage.srCore.LOGGER.debug("No valid paths found for " + showName + " on " + host)
                return False

            for path in paths:
                # we do not need it double-encoded, gawd this is dumb
                unEncPath = urllib.unquote(path.text).decode(sickrage.srCore.CONFIG.SYS_ENCODING)
                sickrage.srCore.LOGGER.debug("KODI Updating " + showName + " on " + host + " at " + unEncPath)
                ...
                if not request:
                    sickrage.srCore.LOGGER.warning("Update of show directory failed on " + showName + " on " + host + " at " + unEncPath)
                    return False
        ...
        # do a full update if requested
            ...
            sickrage.srCore.LOGGER.debug("Doing Full Library KODI update on host: " + host)
            updateCommand = {'command': 'ExecBuiltIn', 'parameter': 'KODI.updatelibrary(video)'}
            request = self._send_to_kodi(updateCommand, host)

            if not request:
                sickrage.srCore.LOGGER.warning("KODI Full Library update failed on: " + host)
                return False
    ...
    def _send_to_kodi_json(self, command, host=None, username=None, password=None):
        ...
        sickrage.srCore.LOGGER.debug("KODI JSON command: " + command)
        ...
        try:
            ...
            try:
                ...
                response.close()
                sickrage.srCore.LOGGER.debug("KODI JSON response: " + str(result))
                return result  # need to return response for parsing
            except ValueError as e:
                sickrage.srCore.LOGGER.warning("Unable to decode JSON: " + str(response.read()))
                return False
    ...
    def _update_library_json(self, host=None, showName=None):
        ...
        sickrage.srCore.LOGGER.debug("Updating KODI library via JSON method for host: " + host)
        ...
        if showName:
            ...
            sickrage.srCore.LOGGER.debug("Updating library in KODI via JSON method for show " + showName)
            ...
            if showsResponse and "result" in showsResponse and "tvshows" in showsResponse[b"result"]:
                ...
                if showsResponse and "result" in showsResponse and "tvshows" in showsResponse[b"result"]:
                    ...
                    sickrage.srCore.LOGGER.debug("KODI: No tvshows in KODI TV show list")
                    return False
            ...
            if tvshowid == -1:
                sickrage.srCore.LOGGER.debug('Exact show name not matched in KODI TV show list')
                return False
            ...
            sickrage.srCore.LOGGER.debug("Received Show: " + showName + " with ID: " + str(tvshowid) + " Path: " + path)

            if not len(path):
                sickrage.srCore.LOGGER.warning("No valid path found for " + showName + " with ID: " + str(tvshowid) + " on " + host)
                return False

            sickrage.srCore.LOGGER.debug("KODI Updating " + showName + " on " + host + " at " + path)
            ...
            if not request:
                sickrage.srCore.LOGGER.warning("Update of show directory failed on " + showName + " on " + host + " at " + path)
                return False
    ...
    def update_library(self, showName=None):
        ...
        if sickrage.srCore.CONFIG.USE_KODI and sickrage.srCore.CONFIG.KODI_UPDATE_LIBRARY:
            if not sickrage.srCore.CONFIG.KODI_HOST:
                sickrage.srCore.LOGGER.debug("No KODI hosts specified, check your settings")
                return False

            # either update each host, or only attempt to update until one successful result
            result = 0
            for host in [x.strip() for x in sickrage.srCore.CONFIG.KODI_HOST.split(",")]:
                if self._send_update_library(host, showName):
                    if sickrage.srCore.CONFIG.KODI_UPDATE_ONLYFIRST:
                        sickrage.srCore.LOGGER.debug("Successfully updated '" + host + "', stopped sending update library commands.")
                        return True
                else:
                    if sickrage.srCore.CONFIG.KODI_ALWAYS_ON:
                        sickrage.srCore.LOGGER.warning(

...
class PLEXNotifier(srNotifiers):
    def _send_to_plex(self, command, host, username=None, password=None):
        ...
        if not username:
            username = sickrage.srCore.CONFIG.PLEX_CLIENT_USERNAME
        if not password:
            password = sickrage.srCore.CONFIG.PLEX_CLIENT_PASSWORD

        if not host:
            sickrage.srCore.LOGGER.warning('PLEX: No host specified, check your settings')
            return False
        ...
        sickrage.srCore.LOGGER.debug('PLEX: Encoded API command: ' + enc_command)
        ...
        try:
            ...
            if password:
                ...
                sickrage.srCore.LOGGER.debug('PLEX: Contacting (with auth header) via url: ' + url)
            else:
                sickrage.srCore.LOGGER.debug('PLEX: Contacting via url: ' + url)
            ...
            sickrage.srCore.LOGGER.debug('PLEX: HTTP response: ' + result.replace('\n', ''))
            # could return result response = re.compile('<html><li>(.+\w)</html>').findall(result)
            return 'OK'

        except (urllib2.URLError, IOError) as e:
            sickrage.srCore.LOGGER.warning('PLEX: Warning: Couldn\'t contact Plex at ' + url + ' ' + e)
            return False
    ...
    def _notify_pmc(self, message, title='SiCKRAGE', host=None, username=None, password=None, force=False):
        ...
        if not sickrage.srCore.CONFIG.USE_PLEX_CLIENT and not force:
            return False

        # fill in omitted parameters
        if not host:
            host = sickrage.srCore.CONFIG.PLEX_HOST
        if not username:
            username = sickrage.srCore.CONFIG.PLEX_CLIENT_USERNAME
        if not password:
            password = sickrage.srCore.CONFIG.PLEX_CLIENT_PASSWORD
        ...
        for curHost in [x.strip() for x in host.split(',')]:
            sickrage.srCore.LOGGER.debug('PLEX: Sending notification to \'%s\' - %s' % (curHost, message))
    ...
    def update_library(self, ep_obj=None, host=None, username=None, password=None, plex_server_token=None, force=True):
        ...
        if sickrage.srCore.CONFIG.USE_PLEX and sickrage.srCore.CONFIG.PLEX_UPDATE_LIBRARY:

            if not sickrage.srCore.CONFIG.PLEX_SERVER_HOST:
                sickrage.srCore.LOGGER.debug('PLEX: No Plex Media Server host specified, check your settings')
                return False

            if not host:
                host = sickrage.srCore.CONFIG.PLEX_SERVER_HOST
            if not username:
                username = sickrage.srCore.CONFIG.PLEX_USERNAME
            if not password:
                password = sickrage.srCore.CONFIG.PLEX_PASSWORD

            if not plex_server_token:
                plex_server_token = sickrage.srCore.CONFIG.PLEX_SERVER_TOKEN
            ...
            if plex_server_token:
                ...
                sickrage.srCore.LOGGER.debug('PLEX: fetching plex.tv credentials for user: ' + username)
                ...
                req.add_header('X-Plex-Client-Identifier', sickrage.srCore.CONFIG.USER_AGENT)
                ...
                try:
                    ...
                    sickrage.srCore.LOGGER.debug('PLEX: Error parsing plex.tv response: ' + e)
            ...
            for cur_host in host_list:
                ...
                if not sections:
                    sickrage.srCore.LOGGER.debug('PLEX: Plex Media Server not running on: ' + cur_host)
                    hosts_failed.append(cur_host)
                    continue
            ...
            if hosts_match:
                sickrage.srCore.LOGGER.debug('PLEX: Updating hosts where TV section paths match the downloaded show: ' + ', '.join(
                    set(host_list)))
            else:
                sickrage.srCore.LOGGER.debug('PLEX: Updating all hosts with TV sections: ' + ', '.join(set(host_list)))

...
class ProwlNotifier(srNotifiers):
    ...
    def _notify_version_update(self, new_version="??"):
        if sickrage.srCore.CONFIG.USE_PROWL:
    ...
    def _sendProwl(self, prowl_api=None, prowl_priority=None, event=None, message=None, force=False):
        ...
        if not sickrage.srCore.CONFIG.USE_PROWL and not force:
            return False

        if prowl_api is None:
            prowl_api = sickrage.srCore.CONFIG.PROWL_API

        if prowl_priority is None:
            prowl_priority = sickrage.srCore.CONFIG.PROWL_PRIORITY

        title = "SiCKRAGE"

        sickrage.srCore.LOGGER.debug("PROWL: Sending notice with details: event=\"%s\", message=\"%s\", priority=%s, api=%s" % (
        ...
        try:
            ...
            sickrage.srCore.LOGGER.error("Prowl notification failed.")
            return False
        response = http_handler.getresponse()
        request_status = response.status

        if request_status == 200:
            sickrage.srCore.LOGGER.info("Prowl notifications sent.")
            return True
        elif request_status == 401:
            sickrage.srCore.LOGGER.error("Prowl auth failed: %s" % response.reason)
            return False
        else:
            sickrage.srCore.LOGGER.error("Prowl notification failed.")
            return False

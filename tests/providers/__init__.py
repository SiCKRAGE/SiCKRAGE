...
disabled_providers = ['BitCannon', 'Newpct', 'LimeTorrent', 'HorribleSubs', 'EliteTorrent']
    ...
    class ProviderTest(VCRMixin, SiCKRAGETestDBCase):
        ...
        def magic_skip(func):
            ...
            def magic(self, *args, **kwargs):
                if self.provider.name in disabled_providers:
                    self.skipTest('All tests are programmatically disabled for provider {}'.format(self.provider.name))
        ...
        @magic_skip
        def test_url(self):
            resp = WebSession().get(self.provider.urls['base_url'], timeout=30)
            code = resp.status_code
            self.assertTrue(self.provider.urls['base_url'] in resp.url)
            self.assertTrue(code == 200, '{} returned a status code of {}'.format(resp.url, code))

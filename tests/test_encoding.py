...
class EncodingTests(SiCKRAGETestCase):
    def test_encoding(self):
        ...
        sickrage.srCore.CONFIG.SYS_ENCODING = None

        try:
            locale.setlocale(locale.LC_ALL, "")
            sickrage.srCore.CONFIG.SYS_ENCODING = locale.getpreferredencoding()
        except (locale.Error, IOError):
            pass

        # For OSes that are poorly configured I'll just randomly force UTF-8
        if not sickrage.srCore.CONFIG.SYS_ENCODING or sickrage.srCore.CONFIG.SYS_ENCODING in ('ANSI_X3.4-1968', 'US-ASCII', 'ASCII'):
